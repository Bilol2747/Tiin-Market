(function () {
  ...
  window.TiinDataAPI = {
    async bootstrap() {
      // meta.json (52 bayt) - shu nashrning vaqti: sahifa "jonli ma'lumot eskirdi" ogohlantirishini ko'rsatishi
      // uchun (2026-09-26, Bilol). Xato bo'lsa OLDINGI muvaffaqiyatli qiymat saqlanadi - vaqt o'tgani sari eskiradi.
      const [inventory, meta] = await Promise.all([
        requestCdn("/invdata.json"),
        requestCdn("/meta.json").catch(() => null)
      ]);
      const pub = meta && Date.parse(meta.published_at);
      if (pub > 0) window.__liveMeta = { ok: true, publishedAt: pub, fetchedAt: Date.now() };
      else if (!window.__liveMeta) window.__liveMeta = { ok: false, fetchedAt: Date.now() };
      return { inventory };
    },
    async kirimdata() {
