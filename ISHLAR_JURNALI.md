...
**Nazorat: Invan'da qo'lda berilgan buyurtma bilan to'qnashuv yopildi (2026-09-25, Bilol):** yuborish kaliti yoqiq bo'lsa, `zakas/auto_control.js` har ishga tushishda Invan'dan eng yangi buyurtmalarni REAL VAQTDA oladi (`POST api.7i.uz/integration/v1/supplier_order?page=1&limit=300`, statik `INVAN_API_TOKEN` - live_data.yml ishlatadigan secret, faqat o'qish; yangi->eski, 300 ta ~7 kunni qamraydi). Jonli ma'lumot snapshot vaqtidan 15 daq oldindan keyin berilgan buyurtmasi bor firma (`supplier.id` `api/_supplier_id_map.json` orqali) shu safar o'tkazib yuboriladi - keyingi safar u saytda Open bo'lib ko'rinadi. FAIL-CLOSED: `INVAN_API_TOKEN` yo'q / Invan javob bermasa / ro'yxat vaqtni qamramasa - HECH NARSA yuborilmaydi (hisobotda sabab). Kalit o'chiq bo'lsa Invan'ga so'rov ham bormaydi. Xarita haqiqiy ma'lumotda tekshirildi: 300 buyurtmadagi 159 ta'minotchining 96% i xaritada. Sinov: 24 + 26 + yangi M1-M7 (soxta GitHub/Invan).
