...
**avg30sa anchor: Invan qoldig'i -> hisoblangan qoldiq (2026-09-26, Bilol: "hisob ancha aniqroq ishlaydi") - LOKAL COMMIT, PUSH KUTILMOQDA:** `build_prev_avg.recompute_avg30_from_history` kunlik "javonda bor edimi" tiklashni Invan `a` dan emas `calcStock` dan boshlaydi (calcStock yo'q bo'lsa `a`). Sabab: Egoiste kofe (Invan 12 / hisob 0) - 60 kunlik oynada 60 kunning HAMMASI "mavjud kun" bo'lib, tezlik 0.12 (haqiqiy 0.35) -> zakas 3 (kerak 6). `build_all_from_api.py`: `build_avg30sa` va `build_c_keep` ENDI `build_calc_stock`DAN KEYIN (avval oldin edi) - ikkalasi ham data_inv_new.json'ni o'zi o'qib-yozadi va HTML embed'ni o'zi yangilaydi, calcStock'ga tegmaydi (e2e sinaldi). Jonli ma'lumotda: tezligi o'zgargan 4042 SKU (2499 oshgan, 1543 kamaygan), C belgisi (`ck`) 100 SKU'da o'zgardi (anchor C filtriga ham tegadi; oyna 30 kunda qoldi). Natija: zakas 2345 (asl) -> 2312 (60 kun, Invan anchor) -> 2322 mln (60 kun, hisob anchor); Heinz 1 -> 8 -> 13, Egoiste 1 -> 3 -> 6, Tushenka 1 -> 3. OCHIQ: mavsumi tugagan tovarlar (mars muzqaymoq 0 -> 5184) - oxirgi 14 kunda sotilmagan 324 ta tovarga +190 mln; keyinga qoldirilgan.

**Zakas tezligi oynasi 30 -> 60 kun (2026-09-26, Bilol) - LOKAL COMMIT, PUSH KUTILMOQDA:** `build_prev_avg.py` `AVG30_SPEED_WINDOW=60` (yangi konstanta; `_compute_avg30_stock_aware`/`recompute_avg30_from_history` endi `win` parametrini oladi) - `avg30sa` shu oynadan hisoblanadi. Kun turlari (sotilgan / javonda bor edi / tugagan) va 3x kalendar chegarasi O'ZGARMAGAN, faqat oyna uzaydi. **C tovarlar filtri (`ck`, `recompute_c_keep`) ATAYLAB 30 kunda qoldi** (Bilol: "C 30 kunda qolaversin") - jonli ma'lumotda tekshirildi: 1 ta SKU'dan boshqasida `ck` o'zgarmadi. `sales_runtime.js` `ZKA_LAST_KIRIM_MIN_DAYS=5 -> 15`: kirim 15 kundan yosh bo'lsa kirimdan-keyingi tezlik ishlatilmaydi (kunlar kamligidan tasodifiy), 60 kunlik o'rtacha qoladi.

SABAB va o'lchovlar: Heinz ketchup/Egoiste kofe kabi kam sotiladigan tovarlarda 30 kunlik oyna tezlikni 0.03/kun ga tushirib, zakasni 1 donaga qisqartirardi. Haqiqiy kunlik stok arxivi (`data_stock_snapshot.json`) bilan backtest (7868 tovar, kesim 2026-08-28, haqiqat = keyingi 30 kunda FAQAT javonda bo'lgan kunlardagi sotuv): kam baholash 15.9% -> 15.1%, ortiqcha 16.4% -> 15.4%, to'g'ri 67.7% -> 69.5%; kam sotiladigan guruhda kam baholash 36.1% -> 32.1%; kirimi eski guruhda 19.1% -> 17.3%. Bilolning boshqa 2 varianti (kirimdan-beri tezlik; kirimdan-beri faqat 60 kun ichida) ham o'lchandi - ikkinchisi Heinz'ni umuman tuzatmasdi (kirimi 108 kun oldin). Kirim qoidasi chegarasi: kirim 5-9 kun oldin bo'lganda tezlik uzoq muddatli o'rtachadan 26% tovarda 2+ baravar farq qilgan, 15-30 kun oralig'ida esa 7% - shuning uchun 15.

JONLI MA'LUMOTDA TA'SIRI: zakas qatorlari 2500 -> 2515, jami 2345.4 -> 2312 mln (-33.4); tezligi o'zgargan 9002 tovar, holati (MUST/CAN) o'zgargan 703; oshgan 547 (+273.5 mln) va yangi 177 (+169.5 mln), kamaygan 649 (-350.6 mln) va chiqib ketgan 170 (-129.4 mln). Heinz 1 -> 8, Egoiste 1 -> 3. **OCHIQ XAVF:** mavsumi tugagan tovarlarda (muzqaymoq) 60 kunlik oyna eski tezlikni qaytaradi - oxirgi 14 kunda umuman sotilmagan 274 ta tovarga +157.9 mln qo'shiladi (mars icecream 0 -> 5184). "14 kunda sotilmasa tegma" himoyasi sinaldi va RAD ETILDI: u Heinz'ni ham o'chirib qo'yadi (u ham tugagani uchun sotilmagan). Mavsumiy kategoriyalar qoidasi keyinga qoldirildi.

**Takroriy zakasdan ikki himoya (2026-09-26, Bilol) - LOKAL COMMIT, PUSH KUTILMOQDA:** (1) `api/invan-order.js` yangi `action:"recent"` (faqat o'qish, shaxsiy token, `POST /api/v1/supplier_order?page=1&limit=300`): shu ta'minotchiga oxirgi `minutes` (standart 30, eng ko'pi 240) daqiqada ochilgan buyurtmalar (qaytarish/bekor turlari hisobga olinmaydi) -> `{no,status,mins_ago,total}`. Frontend `_zkRecentOrdersInfo()` katakcha oqimida (`zkAutoToggleSupplier`) va `zkSendToInvan`/`zkSendFileToInvan` (prod) tasdiqlash oynasi tepasiga "⛔ DIQQAT: N daqiqa oldin buyurtma ochilgan (№..)" qo'shadi; tekshirib bo'lmasa (xato/timeout 15s) to'smaydi, "tekshirib bo'lmadi" deydi. (2) `sales_api_client.js` `bootstrap()` endi `meta.json`ni ham oladi -> `window.__liveMeta.publishedAt`; `sales_runtime.js` `_liveBannerPaint()` Buyurtma (p7) tepasida sariq banner: ma'lumot >40 daqiqa eski (`ZK_LIVE_STALE_MIN`) yoki meta yuklanmagan bo'lsa; renderZakas() va har 60s yangilanadi. Sinov: soxta Invan 17/17, frontend 22/22, banner (soxta DOM) 8/8, HAQIQIY Invan o'qish testi (240 daq: topildi, boshqa firma: 0, yaroqsiz token: to'smaydi); zakas hisobi o'zgarmagan (jonli ma'lumotda 2500 qator, farq 0), test_send_phase 24/24, test_pr2 40/41 (ma'lum dedlayn artefakti). `sales_runtime.js?v=`/`sales_api_client.js?v=` yangilandi.

**Yaqinda kelgan tovarga yana zakas chiqishi tuzatildi (2026-09-26, Bilol):** sabab - `lkQty/lkDate` sync paytida (kuniga 2 marta) qotadi, jonli 15 daqiqalik yangilanish (`backend/app.py` `_live_invdata`) esa yangi kirimda faqat `lkSold`ni kamaytirardi (0 dan pastga tushmaydi) - tovar eski kirim bo'yicha "tugayapti" ko'rinardi (cavendish леденцы: bugun 54 ta kelgan, saytda 18/18 -> +36 zakas). Endi `pipeline_adapter.newer_last_kirim()`: bazaviy vaqtdan (`data_calc_baseline.json`) KEYIN qabul qilingan Received kirim bo'lsa `lkQty/lkDate` shu kirimga almashadi, `lkSold` = bazaviy vaqtdan beri sotuv. Mezon calcStock'ga qo'shiladigan kirim (`received_since`) bilan AYNAN BIR XIL - stok va "oxirgi kirim" doim birga o'zgaradi (jonli ma'lumotda 129 SKU, received_since bilan nomuvofiqlik 0). Bu 15 daqiqalik jonli yugurish ichida (0.7 daq, og'ir hisob qo'shilmagan). Jonli ma'lumotda sinov: 129 SKU'da lk yangilandi, zakasdan 3 ta qator chiqdi (2 ta cavendish + lavazza, 3.3 mln), boshqa hech narsa o'zgarmadi, yangi qator paydo bo'lmadi. `sales_runtime.js` o'zgarmagan - tuzatish live_data.yml keyingi yugurishida (har 15 daq) kuchga kiradi.

**Doimiy nazorat: firma sahifasidagi qator belgisi = BUGUNGI istisno (2026-09-25, Bilol):** nazoratdagi firma sahifasi ochiq bo'lsa, qator/sarlavha katakchasi o'zgargach (`zkSaveManual()` ichidan - `zkRowChecked` o'zgaradigan hamma joy shuni chaqiradi) ~1.5s debounce bilan `api/zakas-auto-firms.js` action `"exclude"` ({firm, skus}) yuboriladi: `_zkAutoRowsFor(sup)` dan belgisi olinganlar. Server `auto-data` branch'idagi `zakas_auto_exclude.json` ({day, firms}) da firmaning bugungi ro'yxatini ALMASHTIRADI (bo'sh = o'chadi; `day` - server, Toshkent kuni; eski kun -> bo'sh xarita; faqat nazoratdagi firma; <=500 sku, <=60 belgi; 409/422 -> 3 urinish). `zakas/auto_control.js` faqat `day===bugun` bo'lsa istisnoni `evaluateFirm()`dan OLDIN chiqaradi (holat/summa/max_sum/ledger qolgan tovarlar bo'yicha; hammasi istisno -> yuborilmaydi); hisobotda "🚫 Bugungi istisno" bo'limi (istisno MUST tovarlar ro'yxati bilan, Telegramga ham). Faylni o'qib bo'lmasa - shu safar yuborilmaydi. Ertasiga o'zi qaytadi. Sinov: mock fetch (API 11 holat, auto_control, frontend debounce). `sales_runtime.js?v=` yangilandi.

**Karobkaga yuqoriga yaxlitlash chegarasi (2026-09-25, Bilol):** ikki shart, ikkalasi ham bajarilsa yuqoriga yaxlitlanadi: (1) `ZKA_BOX_UP_MAX_DAYS=30` - karobkagacha QO'SHISH natijasida zaxira 30 kundan ko'p bo'lmasin (tezlik noma'lum bo'lsa ham yaxlitlanmaydi), (2) `ZKA_BOX_UP_MAX_PCT=20` - qo'shilgan miqdor xom miqdorning 20% idan oshmasin (xom 70, karobka 40 -> 80 mumkin; xom 25 -> 40 yo'q). Aks holda xom miqdor (kamida 1) qoladi. Pastga yaxlitlash (kamaytirish) ham shunday cheklangan: `ZKA_BOX_DOWN_MAX_PCT=20` - xom miqdorning 20% idan ko'p kamaymaydi (xom 5, karobka 30 -> 0 emas, 5 qoladi; MUST himoya qoidasi avvalgidek). Natijada karobkaga yaxlitlash faqat ±20% ichida. O'lchov (lokal ma'lumot): jami zakas 2516.5 -> 2509.1 mln, yuqoriga yaxlitlangan qator 159 -> 92 (ortiqcha 18.2 -> 5.1 mln), kamaytirilgan qator 157 -> 134 (15.6 -> 11.5 mln), zakasdan butunlay tushib qolgan 11 ta qator qaytdi. Sabab: tushenka Ansor - kuniga 0.03 dona, xom 1 dona, karobka 24 -> MUST xavfsizlik qoidasi +23 qo'shib 24 dona (800 kunlik zaxira) berardi. Pastga yaxlitlash (kamaytirish) o'zgarmagan.

**ZAKAS OCHILGANDA TELEGRAMGA XABAR (2026-09-25, Bilol):** zakas Open bo'lgach (sayt katakchasi VA avtomat nazorat) 'Tiin Analitika' guruhiga SENING nomingdan buyurtma PDF'i va ostida xabar ketadi: firma nomi + 'Hurmatli menejer! Shu firma kimga tegishli bo'lsa, shu postavshikka avtozakaz ochildi.' + SMS holati (raqam bor: 'SMS yuborildi: +998...', yo'q: 'SMS yuborilmadi - sababi: telefon raqami kiritilmagan') + 'Iltimos, postavshikka xabar bering.' (buyurtma raqami/tovar soni YO'Q - PDF'dan ko'rinadi). Tuzilma: `zakas/tg_notify.py` (Telethon; Invan'dan buyurtma+PDF+telefon, guruhga yuboradi, takror yuborilmasin deb `auto-data/zakas_tg_posted.json`), `.github/workflows/telegram_notify.yml` (workflow_dispatch: order_no, sms, target=group|me; concurrency bitta - Telegram sessiyasi ikki joyda bir vaqtda ishlamasin), ishga tushiruvchilar: `api/invan-order.js` (`dispatchTelegramNotify`, GITHUB_PAT) va `zakas/auto_control.js` (GITHUB_TOKEN, `actions: write`). Telegram sessiyasi ALOHIDA yaratilgan StringSession GitHub secret'larda: `TELEGRAM_SESSION`, `TELEGRAM_API_ID`, `TELEGRAM_API_HASH` (lokal `claude.session` bilan aralashmasin). O'chirish: env `TG_NOTIFY=off` (Vercel va workflow). Sinov: target=me -> 'Saqlangan xabarlar'. Telefon raqami logga/dispatch kiritishiga TUSHMAYDI (repo ochiq).

**Ortiqcha zakasga qarshi 2 qoida (2026-09-25, Bilol):** (1) `ZKA_REF_MAX_DAYS` 60 -> 45: oxirgi kirim 45 kundan uzoqqa yetsa, zakas ~30 kunlik miqdorga tushadi (1 kunlik ma'lumotda ~200 mln, 6.6% kamayadi; 45+ kunlik zaxira beradigan qatorlar 292 -> 39). (2) QOLDIQ TEKSHIRUVI `ZKA_STOCK_GUARD_DAYS=30`: Invan VA hisoblangan qoldiq (`calcStock`) IKKALASI ham 30+ kunlik zaxira ko'rsatsa, kirim modeli 'tugagan' desa ham zakas berilmaydi (`zkaStockOk`; sabab: oxirgi yozilgan kirim eski, sotuv undan bir necha barobar ko'p - yozilmagan kirimlar/eski zaxira; Jacobs 3in1: kirim 6.5 oy oldin 1008, sotilgan 4594, Invan 6160). Ikkalasi kelishishi shart - bittasi xato bo'lsa tegilmaydi. Ko'rinish: shunday qatorda QOLGAN KUN stokdan, HOLAT yashil (izoh bilan); hisobotda 'Stok yetarli deb zakas berilmagan' soni. Ta'siri (1 kunlik ma'lumotda): qoldiq tekshiruvi 633 qatorga (~367 mln), chegara 60->45 - 11 qator/~200 mln; ikkalasi birga 3302 -> 2659 qator, 3051 -> 2516 mln so'm. TESKARISI (kirim 'yetarli' deydi, Invan VA hisob <3 kunlik, 159 tovar/89 tasi A-B): zakas BERILMAYDI, faqat nazorat hisobotida ogohlantirish. Sayt, Export va nazorat bir xil hisobdan foydalanadi.

**BELGISI OLIB TASHLANGAN TOVAR - 2-VARIANT (2026-09-25, Bilol; avvalgi 'faqat bugun' qoidasini ALMASHTIRDI):** firma sahifasida qatordan belgi olib tashlangan tovar O'ZI SABABLI avtomat buyurtma chiqarmaydi (firmani 'zakasga tushdi' qilmaydi: holat faqat qolgan tovarlardan), lekin firmaga boshqa tovar sababli buyurtma ketsa - hamma zakas tovari (belgisizlar ham) birga ketadi. Belgi firmaga BUYURTMA berilgach (ledger / sayt katakchasi `at` / Invan real vaqt) 'ishlatilgan' bo'ladi va tovar oddiy tartibga qaytadi (nazorat `zakas_auto_exclude.json`dan tozalaydi); unutilgan belgi 14 kundan keyin tugaydi. Fayl: `{"firms":{firma:{sku:qachondan_ISO}}}` (kunlik format YO'Q). API: `exclude` (ro'yxatni almashtiradi, mavjud tovarning vaqtini saqlaydi), `exclude_get`. Sayt sahifa ochilganda faol belgilarni qatorlarga qo'llaydi (brauzer kunlik tozalanishi belgini yo'qotmasin).

**XATO TUZATILDI: katakcha bilan yuborishda qator katakchalari hisobga olinmagan (2026-09-25, Bilol topilmasi):** `zkAutoToggleSupplier()` `_zkAutoRowsFor()` ning HAMMA (orderQty>0) tovarini yuborardi, qatordagi belgisi olib tashlangan tovarlarni ham (HORECA EXPERT (Kotanyi), buyurtma 21597: 2 ta belgisiz tovar ham ketgan). Endi faqat belgilangan (`_zkIsChecked`) qatorlar yuboriladi; belgisiz tovar soni tasdiq oynasida ko'rsatiladi; hammasidan belgi olingan bo'lsa yuborilmaydi. ESLATMA: doimiy nazorat (auto_control.js) qator katakchalarini bilmaydi (ular faqat brauzerda) - avtomat buyurtmaga firmaning HAMMA zakas tovari kiradi; alohida tovarni avtomatdan chiqarish uchun serverda 'istisno' ro'yxati kerak bo'ladi (hali yo'q).

**Nazorat: Invan'da qo'lda berilgan buyurtma bilan to'qnashuv yopildi (2026-09-25, Bilol):** yuborish kaliti yoqiq bo'lsa, `zakas/auto_control.js` har ishga tushishda Invan'dan eng yangi buyurtmalarni REAL VAQTDA oladi (`POST api.7i.uz/integration/v1/supplier_order?page=1&limit=300`, statik `INVAN_API_TOKEN` - live_data.yml ishlatadigan secret, faqat o'qish; yangi->eski, 300 ta ~7 kunni qamraydi). Jonli ma'lumot snapshot vaqtidan 15 daq oldindan keyin berilgan buyurtmasi bor firma (`supplier.id` `api/_supplier_id_map.json` orqali) shu safar o'tkazib yuboriladi - keyingi safar u saytda Open bo'lib ko'rinadi. FAIL-CLOSED: `INVAN_API_TOKEN` yo'q / Invan javob bermasa / ro'yxat vaqtni qamramasa - HECH NARSA yuborilmaydi (hisobotda sabab). Invan buyurtmalari (faqat o'qish) `INVAN_API_TOKEN` bor bo'lsa har ishga tushishda so'raladi (belgilar ishlatilganligini bilish uchun ham); kalit o'chiq bo'lsa yuborilmaydi. Xarita haqiqiy ma'lumotda tekshirildi: 300 buyurtmadagi 159 ta'minotchining 96% i xaritada. Sinov: 24 + 26 + yangi M1-M7 (soxta GitHub/Invan).
...
- **1. Qo'lda buyurtmadan keyin:** `firms[sup].at` (API `set true` vaqti) Toshkent kuni == bugun -> firma bugun avtomatik yuborilmaydi. (Bot/Invan orqali qo'lda buyurtma `at`ni yangilamaydi - bu hali qamralmagan.)
- **2. Ledger tartibi:** buyurtmadan OLDIN `status:'inflight'` yoziladi, natija `results`ga ledger'dan OLDIN, `ledgerUpsert()` 409/422'da qayta o'qib 3 martagacha urinadi. Inflight yozilmasa buyurtma ketmaydi; buyurtmadan keyin ledger yozilmasa inflight qoladi (bugunga blok), job qizil.
- **3.** Kalit har buyurtmadan oldin qayta o'qiladi (`readMode()`), `send!==true` -> to'xtaydi; `max_sum` ham yangilanadi.
- **4.** Frontend: "hammasini olib tashlash" faqat serverdagi nomlarni yuboradi (`_zkAutoServerFirms`), 400 talik bo'laklar; `set` javobidagi `firms` haqiqat (`_zkAutoApplyServerFirms`).
- **5.** Ketma-ket 2 tarmoq xatosi -> to'xtaydi; umumiy dedlayn 10 daq (`ZAKAS_AUTO_DEADLINE_MS`); telefon so'rovi 30s; `invanCall` timeout javob tanasini ham qamraydi; Telegram 20s.
- **6.** `!j.ok` va `token_expired` ham ledger'ga `failed` (bugunga blok). Oxirgi 7 kundagi `inflight/unknown/partial/failed` har run hisobotda "Hal qilinmagan" bo'limida.
- **7.** `!(ageMin<=90)` (NaN = eskirgan). Kalit yoqiq, lekin yuborib bo'lmasa (token yo'q/eskirgan) exit 1.
- **8.** Token bo'sh -> `sendPhase` istisno; dedup nom YOKI supplier_id bo'yicha (workflow sparse-checkout'ga `api/_supplier_id_map.json` qo'shildi); summa chegarasi `sumMax` (tannarx yo'q -> sotuv narxi), narxsiz tovar bo'lsa va chegara qo'yilgan bo'lsa yuborilmaydi; API `firms[..].uid` va `mode.uid` imzolangan tokendan; `data_mahsulotlar.json` yangiligi eng oxirgi `ld` bo'yicha (2 kundan eski -> yuborilmaydi).
- **Hisobot (buyurtma mantig'iga tegilmadi):** `dropped` endi faqat `zabc=C && ck=0` ni chiqaradi (qatorga `ck` qo'shildi); yangi bo'limlar: kirim ma'lumoti yo'q + sotuv bor + qoldiq<=0; ochiq PO bor lekin <=0 kun; C-filtr tufayli tugaganlar soni. Real ma'lumotda (530 firma, `--all`): buyurtma natijalari o'zgarmadi (0 farq), dropped=0 (MUST'dagi zakassizlar hammasi PO'da yoki C-filtrda), 145 / 415 / 580.
- **Sinov (hammasi soxta fetch, haqiqiy so'rov 0):** sendPhase 25 tekshiruv (oddiy, telefon yo'q, kuniga 1 ta nom+sid, qo'lda buyurtma, inflight, ko'prik tarmoq xatosi, javobsiz buyurtma, token yo'q, ledger 409 x2/x10, ledger buyurtmadan keyin yozilmadi, kalit run o'rtasida o'chdi, !ok, partial, token_expired, summa chegarasi, cap, hisobot); `main()` 4 holat (kalit o'chiq -> exit 0; noto'g'ri `published_at` va token yo'q -> exit 1, Invan'ga 0 so'rov; dedlayn); API `uid`; frontend 730 o'zgarish -> 400+330. Sinov skriptlari repo'ga qo'shilmagan.
- `sales_runtime.js?v=` yangilandi.

**Kalitni yoqishdan oldin qolgan:** bot/Invan orqali qo'lda buyurtma (1-band to'liq emas), `partial/failed` yozuvlarni "hal qilindi" deb belgilash mexanizmi yo'q (7 kun hisobotda turadi).
...
**Yuborilgach:** shu firma tovarlariga darhol "Open" (P8'ga vaqtincha "New" kirim yozuvi, `_zkAutoReapplySent()`, buyurtma raqami haqiqiy kirimda kelgach o'zi tushadi) va "Zakas berildi" (`zkSetConfirmed`) avtomatik. Belgi olib tashlansa ikkalasi tozalanadi.

**"Qabul boshlangan buyurtma" qoidasi (`krPendingQty` + `_krOrderHasReceipt`):** buyurtmadan kamida bitta tovar qabul qilingan (qty>0) bo'lsa, butun buyurtma "Open" hisoblanmaydi - kelmagan tovarlar qayta kelmaydi, keyingi zakasga qaytadi. Buyurtma (order_id) darajasida - bir firmaning boshqa buyurtmalariga ta'sir qilmaydi. Real ma'lumotda ~108 SKU ta'sirlanadi.

**Katakcha endi buyurtmani OCHADI ham va ta'minotchiga SMS yuboradi (2026-09-24):** `api/invan-order.js`ga `body.finalize` (faqat prod): buyurtma+tovarlardan keyin `PUT /api/v1/supplier_order_status/{id}` (New -> Open, shaxsiy token), keyin `GET /api/v1/supplier/{id}` dagi `phone_number` (bo'shlari tashlanadi) bo'yicha `POST /api/v1/supplier_order_sms/{id}` `{phone_number:[...]}` - qadamlar `zakas/order_finalize.js` bilan bir xil. SMS FAQAT Open bo'lgach; telefon yo'q/xato bo'lsa buyurtma qoladi va sabab `finalize.note`da qaytadi, frontend qizil xabar beradi. Tasdiqlash oynasi SMS haqida ogohlantiradi. `vercel.json`: `api/invan-order.js` maxDuration 60. Sinov: soxta fetch bilan 6 holat (telefon bor/yo'q, Open xatosi, SMS xatosi, eski oqim, demo) - real SMS yuborilmadi.

**Zakas chegarasi: oxirgi kirimning 3 baravaridan ORTIQ bo'lmaydi (2026-09-24, Bilol):** `_zkaCompute()`da `ZKA_MAX_ORDER_MULT=3` (kg emas -> butun, kg -> aniq), karobka yaxlitlashdan keyin ham chegaradan oshsa - chegaradan oshmaydigan eng katta karobka karralisiga tushiriladi. Sabab: oxirgi kirim 15 kundan kam yetsa formula uni 'kichik' deb 25-30 kunlik sotuvga oshirardi (6 kelgan tovarga 36 zakas). Real ma'lumotda 449 ta qator (10%) 3x dan ortiq edi; jami miqdor ~14% kamayadi. Lokal `backend_p_zakas_auto.py`da `MAX_ORDER_MULT=3` (bir xil). Tekshirildi: sayt 4464 qator, Python 4191 qator - eng katta nisbat 3.0, oshgani 0. Oxirgi kirimi 1-2 dona bo'lgan ~106 qator (ehtimol ma'lumot xatosi) ham shu chegaraga tushadi.

**"Oxirgi kirim yetadimi" tekshiruvi kirimdan beri sotuv tezligi bilan (2026-09-24, Bilol):** `_zkaCompute()`da kirim 15-60 kunga yetadimi degan sanity-tekshiruv endi 30 kunlik o'rtacha emas, `lkSold/o'tgan_kun` tezligi bilan (ZKA_LAST_KIRIM_MIN_DAYS=5, MAX=30; kirim hali tugamagan 0<lkSold<lkQty). Sabab: makfa makaron (60 kirim, 12 kunda 37 sotilgan = 3.1/kun ~19 kunga yetadi) 30 kunlik 4.46 bilan 13.5 kun chiqib 'kichik' deb 111 zakas berardi. Kirim 30 kundan eski/5 kundan yangi/tugagan bo'lsa 30 kunlik qoladi (eski kirimda lkSold>lkQty - yozilmagan kirim - tezlik ishonchsiz, 85-232 kunlik holatlar zakasni 2000 ga ko'tarardi). Xuddi shu tezlik (vCheck) 'qolgan kun'da ham ishlatiladi (ikki qadam zid bo'lmasligi uchun); buyurtma HAJMI (resetDays*velocity) 30 kunlikda qoladi. Kirim 30 kundan eski bo'lsa hamma joyda 30 kunlik o'rtacha. Lokal `backend_p_zakas_auto.py` (LAST_KIRIM_MIN/MAX_DAYS) bir xil. Natija (butun katalog): 170 qator o'zgaradi (94 kamayadi, 76 ko'payadi - tez sotilganlar), 75 tovar zakasdan chiqadi, 119 tovar yangi zakasga kiradi, jami -1.8%; SKU 2076: 120 -> 40.

**C tovarlar filtri - kam sotiladigan C ga zakas berilmaydi (2026-09-24, Bilol):** zakas jadvalidagi ABC 'C' (zabc) tovar 'Oxirgi kirim' rejimida FAQAT quyidagilardan biri bajarilsa zakasda qoladi: (1) oxirgi 30 kundan >=10 kunida sotilgan, (2) tezlik >=1 dona/kun, (3) tez-tez tugagan (javonda 6-19 kun) va shu kunlarning >=yarmida sotilgan (yaxshi sotilardi, lekin tugab qolgan). Aks holda zakas 0 (sonlar - 'zakas kerak', Tezkor ro'yxat, firmalar ro'yxati - o'zi kamayadi, chunki orderQty>0 dan sanaladi). Hisob BACKEND'da: `build_prev_avg.py` `c_keep_decision()`/`build_c_keep()` (chegaralar C_KEEP_* faqat shu yerda), har build'da (kuniga 2 marta) qayta hisoblanadi -> `data_inv_new.json` `ck`=1/0 (faqat C tovarlarda); ABC ham har build'da qayta hisoblanadi, shuning uchun sotuv o'zgarsa tovar o'zi qaytadi/chiqadi (qattiq yozilgan ro'yxat YO'Q). `ck` `_HEAVY_FIELDS` (backend/app.py, publish_metrics.py) orqali jonli ma'lumotga ham o'tadi; sayt `v.ck`ni o'qiydi (`ck` yo'q bo'lsa filtr ishlamaydi - xavfsiz). Lokal `backend_p_zakas_auto.py` ham `ck`ni o'qiydi. Real ma'lumotda: 1125 C qatordan 328 qoladi, 797 chiqadi (7,381 dona = jami zakasning 3.0%). Saytda `ck` keyingi build'dan (14:00) keyin paydo bo'ladi.

**Firma telefon raqami yuborishdan OLDIN ko'rinadi (2026-09-24, Bilol):** `api/invan-order.js` `action:"phones"` (faqat shaxsiy Invan tokeni bilan; `GET /api/v1/supplier/{id}`.phone_number, bo'shlari tashlanadi; umumiy token bilan qaytarilmaydi - endpoint ochiq). Sayt: firma sahifasi sarlavhasida badge (`_zkPhoneBadgeHtml`: '📞 SMS: raqam' / '📵 Invan'da raqam YO'Q - SMS ketmaydi' / '❔ firma topilmadi' / tokensiz - 'SMS raqamini tekshirish' tugmasi), katakcha bosilganda tasdiqlash oynasida ham raqam holati yoziladi (raqam yo'q bo'lsa qizil ogohlantirish). Kesh 10 daqiqa. Sinov: soxta Invan javoblari bilan (raqam bor/yo'q/topilmadi), buyurtma yaratilmadi.

**Avto-belgi va "zakasga muhtoj" soni (2026-09-24, Bilol):** (1) Muntazam zakasi bor (orderQty>0) tovar SUKUT bo'yicha belgilangan (`_zkIsChecked`) - firma sahifasiga kirganda "Belgilangan summa" darhol ko'rinadi; faqat MUNTAZAM (`normal:` kalit) - Chuqur qatorlar avvalgidek qo'lda (aks holda summa Chuqur bilan shishardi: 188 mln o'rniga 147 mln). Qo'lda olib tashlash saqlanadi, lekin har KUNI yangidan boshlanadi (`zk_row_checked_day`). (2) Ro'yxat sarlavhasidagi "zakasga muhtoj" endi FAQAT Muntazam zakasi bor firmalarni sanaydi (640 -> 364): Muntazam zakas berilgan firma faqat Chuqur tufayli 'muhtoj' bo'lib qolmasin. Ro'yxatning o'zi o'zgarmaydi.

**DOIMIY ZAKAS NAZORATI (2026-09-24, Bilol; main'ga chiqarildi, hisobot rejimida ISHLAB TURADI, yuborish KALITI o'chiq):** katakcha belgilangan (buyurtma yuborilgan) firmalar nazoratga olinadi, `zakas/auto_control.js` ularni har ~3 soatda tekshiradi (Toshkent 08:17, 11:17, 14:17, 17:17, 20:17 - kechasi SMS ketmasin). Minuti :17 - GitHub cron soat boshida (:00) tez-tez kechikadi/tushib qoladi (2026-09-25: 08:00 run ishga tushmagan), :17 ishonchliroq va jonli ma'lumot (:00/:15/:30/:45) yangilangandan keyin ishlaydi. QOIDA (Bilol: "zakas berishga tushmaguncha berilmasin, tushgach tugab qolmasin"): firma "zakas berishga tushdi" = kamida bitta tovari MUST zonasida (`zkaDays < ZKA_MUST_ORDER_DAYS`=7 kun; `zkaDays` = qatorga chiqarilgan yangi maydon); faqat CAN (7-14 kun) zonasidagi firma KUTADI (CAN'da ham yuborilsin desa - `evaluateFirm()`da `must.length` -> `order.length`); <=3 kun = SHOSHILINCH.

**YUBORISH KALITI (Bilol: "men buyruq beraman, u kuchga kiradi va SMS bilan zakas jo'natishni boshlaydi"):** `auto-data` branch'idagi `zakas_auto_mode.json` {send, max_sum}; sukut `send:false` = faqat hisobot. Kalitni faqat ADMIN o'zgartiradi: sayt zakas sahifasi -> Avtomatik zakas paneli -> "Yoqish/O'chirish" tugmasi (yoki `api/zakas-auto-firms.js` `mode_set`). Yuborish faqat: kalit yoqilgan + `INVAN_PERSONAL_TOKEN` GitHub secret'i bor + jonli ma'lumot yangi (<90 daq) + ledger yozish mumkin bo'lsa. Yo'l sayt katakchasi bilan AYNAN bir xil (`api/invan-order.js`, `finalize:true` -> Open + SMS, Bilolning shaxsiy Invan hisobidan). Xavfsizlik: firma kuniga 1 ta buyurtma (ledger `zakas_auto_ledger.json`, Toshkent kuni); Invan'da telefon YO'Q firmaga yuborilmaydi; `max_sum` (0=chegarasiz) oshsa - o'tkazib yuboriladi (qo'lda tasdiq); bir ishga tushishda ko'pi bilan `ZAKAS_AUTO_MAX_ORDERS`=15; token muddati o'tsa/Invan ko'prigi javob bermasa hammasi to'xtaydi va job qizil; javob kelmasa "unknown" yozuv (bugun qayta yuborilmaydi); ledger yozilmasa job yiqiladi. KEYINGI QADAMLAR (Bilol keyin so'raydi): summa chegarasi (`mode_set` `max_sum`), Telegram nishoni (`ZAKAS_AUTO_REPORT_CHAT_ID` repo variable).

Qismlari: (1) `api/zakas-auto-firms.js` - nazorat ro'yxati + rejim kaliti GitHub `auto-data` branch'ida (Turso/Blob EMAS, sessiya tokeni HMAC bilan tekshiriladi, konfliktda qayta urinadi); (2) `sales_runtime.js`: `_zkAutoFirmsLoad/_zkAutoFirmsSync` (belgilash+yuborish muvaffaqiyatli -> nazoratga qo'shiladi, belgi olinsa chiqadi; "Hammasini tanlash" nazoratga HECH NARSA qo'shmaydi; sahifa ochilganda server ro'yxati lokal belgilar ustidan g'olib), `_zkAutoModeLoad/zkAutoModeToggle` (panel holat satri + admin tugmasi), `_zkAutoAllRowsMap()`, qatorga `zkaDays`; (3) `zakas/auto_control.js` - hisob `sales_runtime.js`dan chaqiriladi (qayta yozilmagan), ma'lumot GitHub'dan (live-data-latest + repo), qo'lda miqdorlar YUKLANMAYDI; (4) `.github/workflows/zakas_auto.yml` (schedule yoqilgan, `contents: write` ledger uchun); (5) `vercel.json`: `auto-data` branch'ida deploy o'chirilgan. Sinov: 530 firma 1.5 soniyada; ro'yxat API 15 + rejim API 11 tekshiruv (soxta GitHub + Python `make_token` bilan mos); yuborish bosqichi 24 tekshiruv (soxta GitHub+Invan: kalit o'chiq, telefon yo'q, kuniga 1 ta, summa chegarasi, cap, token muddati, eskirgan ma'lumot, timeout, ledger xatosi); brauzerda belgilash/olib tashlash/hammasini tanlash/kalit tugmasi. Ma'lum cheklov: `zkaDays<=0` ("tugagan") tovarlar ko'p (~56%) - oxirgi kirimdan ko'p sotilgan, formulaning o'zi shunday.

**Lokal zaxira tizimi** (Vercel'siz, repo'da YO'Q): `local_zakas_server.py` + `backend_p_zakas_auto.py` (kg bo'lmagan tovar butunga `ceil` - 4.3 xatosi tuzatilgan).

---

## 2026-09-21 — Zakas Excel'da Сумма formula + soni ustunida birlik ("10 шт")

**Saytdan yuklangan zakas Excel'i = Excel JADVALI (Сумма/ИТОГО avtomatik):** `sales_runtime.js` dagi ikkala eksport (`_zkExportInvanTemplate()` — `zakas_<supplier>_<sana>.xlsx`, `zkFileExportXLSX()` — `buyurtma_...xlsx`) endi bitta umumiy `_zkBuildOrderBuffer()` orqali quriladi: tovarlar jadval (ListObject "Zakas"), Сумма = `[@Кол-во]*[@Цена]` formula, ИТОГО = jadvalning o'z jami qatori (`SUBTOTAL(109,[Сумма])`), tovarlardan KEYIN bevosita (bo'sh qator yo'q). Sabab (foydalanuvchi so'rovi): Excel'da soni/narx o'zgartirilsa VA ИТОГО'dan oldin/orasiga qator qo'shilsa Сумма o'zi to'lib ИТОГО yangilanishi kerak — oddiy diapazonda Excel qator qo'shganda formulani ko'chirmaydi va SUM chegarasi kengaymaydi. ExcelJS jadval ustunining `calculatedColumnFormula` belgisini yozmaydi, shuning uchun `_zkPatchCalcColumn()` (JSZip) `xl/tables/table1.xml`ga uni qo'shadi (bo'lmasa butun-qator Insert'da formula ko'chmaydi; JSZip yuklanmasa fayl o'zgarishsiz qaytadi). Haqiqiy Excel'da 4 usul sinalgan (ИТОГО'dan oldin/orasiga butun-qator Insert, Table Rows Above, oxiriga Tab) — hammasida Сумма to'ldi va ИТОГО to'g'ri. `order_agent.js` bunday faylni to'g'ri o'qiydi (ИТОГО qatori tovar hisoblanmaydi). `sales_runtime.js?v=` `index.html` va `sales.html`da yangilandi. **Push qilinmagan.**

**Soni ustunida birlik (`zakas/build_shablon.js`):** `parseQty()` — "10 шт", "4 kg", "4sht" → raqam (faqат шт/штук/sht/pcs/dona/дона/кг/kg). "2 коробки" ATAYLAB olinmaydi (quti ≠ dona). Sabab: Надежда Мирзоян "Заказ тиин.xlsx" shunday keldi va butun fayl "soni yozilgan qator topilmadi" bergan edi. Ikkala yo'l (lokal `zakas_tez.py` va Telegram bot) bitta `build_shablon.js` dan foydalanadi. **Push qilinmagan.**
