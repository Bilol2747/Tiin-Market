...
AVG30_WINDOW = 30            # C tovarlar filtri (`ck`) oynasi — o'zgarmaydi
# ZAKAS TEZLIGI OYNASI (2026-09-26, Bilol): 30 -> 60 kun. Sabab: 30 kunlik oyna kam sotiladigan
# tovarda tezlikni yerga urib yuborardi (Heinz ketchup: 150 kunda 36 dona sotilgan, lekin oxirgi
# 30 kunda 1 dona -> tezlik 0.03/kun -> zakas 1 dona; 60 kun bilan 0.28/kun -> 8 dona). Haqiqiy
# kunlik stok arxivi bilan backtest (7868 tovar, kesim 2026-08-28): kam baholash 15.9% -> 15.1%,
# ortiqcha baholash 16.4% -> 15.4%; kam sotiladigan guruhda kam baholash 36.1% -> 32.1%.
# C tovarlar filtri (`ck`, recompute_c_keep) ATAYLAB 30 kunda qoladi - u "30 kunda kamida 10 kun
# sotilgan" degan qoidaga tayanadi, oyna uzaysa filtr o'z-o'zidan yumshab ketardi.
AVG30_SPEED_WINDOW = 60      # zakas tezligi (avg30sa) oynasi
...
def recompute_pav_by_sku(verbose=True):
    """Invan'dan barcha orderlarni olib, to'liq tarix breakdown quradi.
    sku->pav va sku->pavm (davr summary) qaytaradi.

    DIQQAT: bu OXIRGI CHORA yo'li — odatda `recompute_pav_from_history()`
    (data_history.json, arzon) ishlatiladi va bu funksiya umuman
    chaqirilmaydi. Jan 1 dan butun tarix so'raladi, ya'ni juda uzoq.
    2026-08-15: manba Turso emas, Invan API (Turso butunlay olib tashlandi)."""
    from invan_orders import fetch_range
    from build_sales_demand import api_records, build as build_daily
    today = date.today()
    date_from = HISTORY_BASE_DATE.isoformat()
    date_to = (today + timedelta(days=1)).isoformat()
    if verbose:
        print(f"  Invan'dan {date_from}..{today} orderlar (chuqur zakas)...")
...
def _compute_avg30_stock_aware(d_arr, rt_arr, wi_arr, cur_stock, kirim_by_date, hist_base, n, we_arr=None, win=None):
    """Oxirgi `win` (sukut: AVG30_WINDOW) kunlik kunlik o'rtacha — stok-asosli.
    ...
    win = win or AVG30_WINDOW
    if n < win or cur_stock is None:
        return None
    s = n - win
    combo = []
    for i in range(s, n):
...
def recompute_avg30_from_history(root=ROOT, verbose=True, details=None, win=None):
    ...
    for iv in inv.values():
        if isinstance(iv, dict) and iv.get("sku") not in (None, ""):
            # ANCHOR (2026-09-26, Bilol: "hisob ancha aniqroq ishlaydi"): kunlik "javonda bor edimi" tiklash
            # Invan qoldig'idan (`a`) EMAS, HISOBLANGAN qoldiqdan (`calcStock`, kirim+sotuv oqimi) boshlanadi.
            # Invan ba'zan xato (yo'q tovarni bor deb ko'rsatadi): Heinz ketchup Invan 15 / hisob 0, Egoiste kofe
            # Invan 12 / hisob 0 - Invan bilan 30-60 kunning HAMMASI "javonda bor edi, sotilmadi" bo'lib, tezlik
            # 0.03-0.12/kun ga tushib, zakas 1-3 dona chiqardi (haqiqiy tezlik 0.3-0.35/kun). calcStock yo'q
            # (hisoblanmagan) tovarda - Invan qoldig'i. Shuning uchun build_all_from_api.py'da bu qadam
            # build_calc_stock'DAN KEYIN chaqiriladi.
            try:
                _cs = iv.get("calcStock")
                cur_stock_by_sku[str(iv["sku"])] = float(_cs) if _cs is not None else float(iv.get("a") or 0)
    ...
    for key, d_arr in d_map.items():
        ...
        res = _compute_avg30_stock_aware(d_arr or [], rt_arr, wi_arr, cur,
                                          ...
                                          we_arr=we_arr, win=win)
        if res and res[0] > 0:
            avg30_by_sku[sku] = res[0]
            if details is not None:
                details[sku] = res  # (avg, sotilgan kunlar, javonda bo'lgan kunlar)
    if verbose:
...
def build_avg30sa(root=ROOT, verbose=True):
    ...
    try:
        avg30_by_sku = recompute_avg30_from_history(root=root, verbose=verbose, win=AVG30_SPEED_WINDOW)
...
# ─── C TOVARLAR: ZAKASDA QOLISH BELGISI `ck` (2026-09-24, Bilol) ───
# Zakas jadvalidagi ABC "C" (zabc) tovarlarning ko'pi kam sotiladi. "Oxirgi kirim" rejimida C
# tovar zakasda FAQAT quyidagilardan kamida bittasi bajarilsa qoladi (`ck`=1), aks holda `ck`=0
# (sayt shu tovarga zakas bermaydi):
#   1) oxirgi AVG30_WINDOW (30) kunning kamida C_KEEP_MIN_SOLD_DAYS kunida sotilgan (muntazam talab);
#   2) tezligi (avg30sa) kuniga C_KEEP_MIN_VELOCITY dona va undan ko'p;
#   3) tez-tez tugab qolgan (javonda C_KEEP_STOCKOUT_MIN_DAYS..C_KEEP_STOCKOUT_MAX_DAYS-1 kun)
#      va javonda bo'lgan kunlarining kamida C_KEEP_STOCKOUT_RATIO qismida sotilgan -
#      "yaxshi sotilardi, lekin tugab qolgani uchun kam ko'rinyapti".
# Faqat C tovarlarga qo'yiladi (boshqalarda `ck` yo'q). Sayt: sales_runtime.js (v.ck), lokal
# avtomatika: backend_p_zakas_auto.py - ikkalasi shu belgini o'qiydi (chegaralar FAQAT shu yerda).
C_KEEP_MIN_SOLD_DAYS = 10
C_KEEP_MIN_VELOCITY = 1.0
C_KEEP_STOCKOUT_MIN_DAYS = 6
C_KEEP_STOCKOUT_MAX_DAYS = 20
C_KEEP_STOCKOUT_RATIO = 0.5


def c_keep_decision(avg, active, effective):
    """(avg30sa, sotilgan kunlar, javonda bo'lgan kunlar) -> 1 (zakasda qoladi) / 0 (zakas bermaymiz)."""
    if active >= C_KEEP_MIN_SOLD_DAYS or avg >= C_KEEP_MIN_VELOCITY:
        return 1
    if C_KEEP_STOCKOUT_MIN_DAYS <= effective < C_KEEP_STOCKOUT_MAX_DAYS \
            and active / effective >= C_KEEP_STOCKOUT_RATIO:
        return 1
    return 0


def recompute_c_keep(root=ROOT, verbose=True):
    """{sku: 0/1} - faqat zabc=="C" va avg30sa hisoblangan (Muntazam qamrov) tovarlar uchun.

    DIQQAT: bu yerda oyna ATAYLAB AVG30_WINDOW (30 kun) - zakas tezligi 60 kunga o'tgan bo'lsa ham
    (2026-09-26, Bilol: "C 30 kunda qolaversin")."""
    details = {}
    recompute_avg30_from_history(root=root, verbose=False, details=details)
    inv_path = root / "data_inv_new.json"
    if not details or not inv_path.exists():
        return {}
    inv = json.loads(inv_path.read_text(encoding="utf-8"))
    out = {}
    for iv in inv.values():
        if not isinstance(iv, dict) or iv.get("zabc") != "C":
            continue
        sku = str(iv.get("sku") or "").strip()
        d = details.get(sku)
        if sku and d:
            out[sku] = c_keep_decision(d[0], d[1], d[2])
    if verbose:
        kept = sum(out.values())
        print(f"  C tovarlar (Muntazam qamrov): {len(out):,} ta - zakasda qoladi {kept:,}, zakas bermaymiz {len(out) - kept:,}")
    return out


def build_c_keep(root=ROOT, verbose=True):
    """`ck` belgisini data_inv_new.json (+ HTML embed) ga yozadi. zabc (build_zakas_abc)
    hisoblangandan KEYIN chaqiriladi."""
    ck = recompute_c_keep(root=root, verbose=verbose)
    inv_path = root / "data_inv_new.json"
    if not inv_path.exists():
        return 0
    inv = json.loads(inv_path.read_text(encoding="utf-8"))
    ...
    for fn in ("index.html", "sales.html"):
        ...
        f.write_text(txt[:start] + inv_json + txt[end:], encoding="utf-8")
    if verbose:
        print(f"  {n:,} ta C tovarga ck belgisi qo'llandi (inv + embed)")
    return n
...
def recompute_current_cost(root=ROOT, verbose=True):
    ...
    2026-08-19 (Bilol topilmasi, real misol: SKU 512 "Bulyon Galina" - 2
    marta izchil ~723.79 so'mdan HAQIQATAN qabul qilingan, lekin BITTA
    "Received" yozuvda operatordan xato ketib 33,730 so'm yozib qo'yilgan
    (ehtimol quti/karton narxi dona narxi o'rniga kiritilgan) - shu xato
    yozuv sana bo'yicha eng so'nggi bo'lgani uchun "joriy tannarx" bo'lib
    qolgan edi. Bu "Open" buyurtma emas (haqiqatan Received), shuning
    uchun avvalgi tuzatish (faqat Received/Partially received) buni
    tutmaydi - bu YANGI, alohida himoya: agar HAQIQATAN qabul qilingan
    narx ham sotilish narxidan SUSPICIOUS_COST_MULTIPLIER (3x) baravardan
    ko'p bo'lsa - bu ham "shubhali" (backend_p6_suppliers.py'dagi oylik
    COGS'dagi bilan bir xil chegara/mantiq) deb, taxminiy (katalog
    narxi) ga tushiladi.

    Qaytaradi: {sku: (cost, approx)}."""
    from backend_p6_suppliers import kirim_cost_breakpoints, cost_at, SUSPICIOUS_COST_MULTIPLIER
    kirim_path = root / "data_kirim.json"
    inv_path = root / "data_inv_new.json"
    ...
    for iv in inv.values():
        ...
        approx = c is None
        if not approx:
            sell_p = iv.get("p") or 0
            if sell_p and c > sell_p * SUSPICIOUS_COST_MULTIPLIER:
                approx = True
                n_suspicious += 1
        if approx:
    ...
    if verbose:
        ...
        print(f"  {len(cost_by_sku):,} mahsulotga rcost hisoblandi (shundan {n_approx:,} taxminiy - {n_suspicious:,} tasi shubhali kirim narxi sabab)")
    return cost_by_sku
