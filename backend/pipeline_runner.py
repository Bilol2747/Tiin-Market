...
from backend_p_zakas_abc import build_zakas_abc                    # noqa: E402
from build_prev_avg import build_current_cost                      # noqa: E402
from backend_p_calc_stock import build_calc_stock                  # noqa: E402
...
def _apply_zakas_postprocessing(invdata, history, kirimdata, verbose=True):
    """Zakas bo'limiga tegishli UCHTA QO'SHIMCHA maydonni qo'llaydi — aynan
    build_all_from_api.py:864-895 dagi TARTIBDA (rcost -> zabc -> calcStock):

      1) rcost/rcost_approx  — build_prev_avg.build_current_cost()
         "Narx"/Summa ustunlari uchun eng ishonchli joriy tannarx
      2) zabc                — backend_p_zakas_abc.build_zakas_abc()
         kategoriya-ichi ABC (2026-08-06)
      3) calcStock va h.k.   — backend_p_calc_stock.build_calc_stock()
         "Hisoblangan stok" (arvoh-stok, Invan qoldig'idan mustaqil)

    HISOB BU YERDA YO'Q — uchala funksiya ham ROOT'dagi original fayllardan
    BIR HARF HAM O'ZGARTIRILMAGAN holda chaqiriladi (avvalroq boshqa hisobni
    qayta yozganda 46x xato chiqqan edi — shu sababli bu tamoyil qat'iy).

    NEGA VAQTINCHALIK PAPKA: bu uch funksiya ham `data_*.json` FAYLLARINI
    o'qib/yozib ishlaydi (disk-asosidagi eski pipeline uchun yozilgan, bir-
    birining natijasini KETMA-KET o'sha bitta faylga yozib boradi). Bizda bu
    ma'lumot xotirada tayyor turibdi — shuning uchun funksiyalarning o'zini
    o'zgartirish o'rniga, xotiradagi tuzilmani vaqtinchalik papkaga yozamiz,
    uch funksiyani ASL TARTIBIDA ishga tushiramiz (har biri avvalgisi
    yozgan faylni o'qib, ustiga o'z maydonini qo'shadi), yakuniy natijani
    o'qib olamiz, papka avtomatik tozalanadi.

    `calcStock` yana bitta ESKI faylga muhtoj — `data_sales_2025h2.json`
    (2025-yil ikkinchi yarmi tarixi, hali bizning bazamizda yo'q — to'liq
    tarixiy backfill qilinmaguncha). Shu fayl ROOT'dan o'zgarishsiz
    nusxalanadi — jonli sayt ham AYNAN shu faylni ishlatadi.
    """
    if not history:
        if verbose:
            log("zakas qo'shimchalari: tarix yo'q (birinchi ishga tushirish?) — o'tkazib yuborildi")
        return

    import shutil
    import tempfile
    with tempfile.TemporaryDirectory(prefix="zkpost_") as tmp:
        tmp_path = Path(tmp)
        (tmp_path / "data_history.json").write_text(
            json.dumps(history, ensure_ascii=False), encoding="utf-8")
        (tmp_path / "data_inv_new.json").write_text(
            json.dumps(invdata, ensure_ascii=False), encoding="utf-8")
        (tmp_path / "data_kirim.json").write_text(
            json.dumps(kirimdata, ensure_ascii=False), encoding="utf-8")
        old_2025h2 = ROOT / "data_sales_2025h2.json"
        if old_2025h2.exists():
            shutil.copy(old_2025h2, tmp_path / "data_sales_2025h2.json")

        build_current_cost(root=tmp_path, verbose=verbose)
        n_zabc = build_zakas_abc(root=tmp_path, verbose=verbose)
        build_calc_stock(root=tmp_path, verbose=verbose)

        final = json.loads((tmp_path / "data_inv_new.json").read_text(encoding="utf-8"))

    # `final` — nom bo'yicha kalitlangan (invdata bilan bir xil shakl);
    # joriy invdata'ning O'ZIGA yangi maydonlarni qo'shamiz (obyekt
    # identifikatorini almashtirmaymiz, chaqiruvchi tomonda ishlatilgan
    # bo'lishi mumkin).
    NEW_KEYS = ("rcost", "rcost_approx", "zabc",
    ...
    for name, entry in invdata.items():
        ...
        for k in NEW_KEYS:
            if k in src:
                entry[k] = src[k]
    if verbose:
        log(f"zakas qo'shimchalari qo'llandi: rcost, zabc({n_zabc:,}), calcStock")
...
def compute(con, window_days=SITE_WINDOW_DAYS, long_history=True, verbose=True):
    ...
    # `history` shu yerda BIR MARTA hisoblanadi (avval qaytish bloki ichida
    # ikkinchi marta chaqirilardi) — kirim, rcost/zabc/calcStock va yakuniy
    # natijaning barchasi aynan shu tuzilmaga muhtoj.
    history = _build_history(dailydata_long)
    kirimdata = build_kirimdata(supplier_orders_from_db(con))

    # rcost -> zabc -> calcStock, AYNAN build_all_from_api.py:864-895 dagi
    # tartibda (har biri avvalgisining ustiga yozadi).
    _apply_zakas_postprocessing(invdata, history, kirimdata, verbose=verbose)
