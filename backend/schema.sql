...
CREATE TABLE IF NOT EXISTS sup_orders (
    id            TEXT PRIMARY KEY,
...
CREATE INDEX IF NOT EXISTS ix_so_d      ON sup_orders(d);
CREATE INDEX IF NOT EXISTS ix_so_status ON sup_orders(status);

CREATE TABLE IF NOT EXISTS arrivals (
    id            INTEGER PRIMARY KEY,
    order_id      TEXT NOT NULL REFERENCES sup_orders(id) ON DELETE CASCADE,
    ...
    status        TEXT NOT NULL DEFAULT '', -- denorm: sup_orders.status
    sku           TEXT NOT NULL DEFAULT '',
...
CREATE INDEX IF NOT EXISTS ix_clients_tin ON biz_clients(tin);
...
-- ─── Og'ir hisob natijalari (SEKIN QATLAM) ─────────────────────────────────
--
-- IKKI QATLAMLI ARXITEKTURANING KALITI.
--
-- `invdata` (zakas/zaxira uchun asosiy tuzilma) ikki xil ma'lumotdan iborat:
--   1) ARZON, JONLI: stok, narx, ta'minotchi, kategoriya, shtrix-kod —
--      to'g'ridan-to'g'ri `products`/`arrivals` dan bir so'rovda olinadi
--      (backend_p5_stock.build_invdata faqat shularni quradi);
--   2) OG'IR HISOB: avg30sa, pav, zabc, rcost, calcStock va h.k. —
--      butun sotuv tarixini qayta ishlashni talab qiladi (~217 s), Vercel
--      funksiyasining vaqt chegarasiga umuman sig'maydi.
--
-- Shuning uchun (2) qismi GitHub Actions'da kuniga bir necha marta
-- hisoblanadi va SHU JADVALGA yoziladi (backend/publish_metrics.py), API esa
-- (1) ni jonli o'qib, ustiga shu jadvalni birlashtiradi. Natijada zakas
-- ko'rsatkichlari (stok, kirim, sotuv) HAR DOIM jonli bo'ladi, sekin
-- o'zgaradigan tasniflar esa kuniga bir necha marta yangilanadi.
--
-- Maydonlar JSON sifatida saqlanadi: formulalar rivojlanганда (yangi maydon
-- qo'shilganda) sxemani o'zgartirish shart bo'lmasin.
