...
_HEAVY_FIELDS = (
    ...
    "lkQty", "lkSold", "lkDate", "ld60", "lsd", "ck",
)
...
def _live_invdata():
    ...
    from .pipeline_adapter import (
        ...
        sold_events_since, sum_events_since, newer_last_kirim,
    )
    ...
    for entry in inv.values():
        ...
        if payload:
            ...
            # "OXIRGI KIRIM" ZAKAS REJIMI JONLANISHI (2026-09-22, Bilol so'rovi:
            # "sotuv har 15 daqiqada yangilansa, unda ham yangilansin" - Zakas
            # sahifasidagi "Oxirgi kirim" tugmasi, sales_runtime.js `_zkaCompute()`).
            # calcStock'dagi bilan AYNAN bir xil sold_since/received_delta signali -
            # lekin ASOSIY qiymatga (lkQty - "qachon/qancha kirim kelgani", bu voqea,
            # o'zgarmaydi) emas, SOTILGAN tarafga (lkSold) qo'llanadi: sotuv bo'lsa
            # lkSold ko'payadi (calcStock kamayganidek), kirim kelsa lkSold kamayadi
            # (calcStock ko'payganidek) - natijada frontend'dagi lkQty-lkSold
            # ("qolgan_miqdor") calcStock kabi bir xil ishonchli harakatni oladi.
            if "lkSold" in payload and payload.get("lkQty") is not None and (sold_since or received_delta):
                lk_adj = payload["lkSold"] + sold_since.get(sku, 0.0) - received_delta.get(sku, 0.0)
                entry["lkSold"] = max(0.0, round(lk_adj, 3))
            # YANGI KIRIM (2026-09-26, Bilol: "yaqinda kelgan tovarga yana zakas yozyapti"):
            # sync'dan KEYIN kelgan kirim bo'lsa lkQty/lkDate shu kirimga ALMASHADI (yuqoridagi
            # lkSold tuzatishi faqat eski kirimni kamaytirardi va tovar "tugayapti" ko'rinardi).
            # MEZON calcStock tuzatishi (received_since) bilan BIR XIL: bazaviy vaqtdan KEYIN
            # qabul qilingan kirim - stokka qo'shiladigan kirim va "oxirgi kirim" birga o'zgaradi.
            # lkSold = shu kirimdan keyingi sotuv (bazaviy vaqtdan beri).
            try:   # xato bo'lsa eski xatti-harakat qoladi - jonli ma'lumot nashri to'xtab qolmasin
                _nk = newer_last_kirim(((kirim_dict.get("skus") or {}).get(sku) or {}).get("arrivals"), entry.get("lkDate"), baseline_at) if baseline_at else None
                if _nk:
                    entry["lkQty"], entry["lkDate"] = _nk
                    entry["lkSold"] = max(0.0, round(sold_since.get(sku, 0.0), 3))
            except Exception:
                pass
...
_KIRIM_PRUNE_DAYS = 120       # bundan eski buyurtma persistlangan holicha qoladi,
                              # qayta so'ralmaydi (OA_P8/p9'dagi bilan bir xil chegara)
_KIRIM_RECENT_HOURS = 5       # ODATIY holatda HAR safar qayta tekshiriladigan oyna
                              # (2026-09-10, Bilol: "5 soatni so'rasa ham xavfsiz
                              # bo'ladimi" - ha, chunki to'g'irlik bu raqamdan emas,
                              # pastdagi O'ZINI-O'ZI TUZATISH mexanizmidan keladi;
                              # bu funksiya juda tez-tez (har 15 daqiqada)
                              # chaqirilgani uchun 5 soat ODATIY holatda yetarli).
                              #
                              # O'ZINI-O'ZI TUZATISH (sync_worker.py'dagi
                              # KIRIM_DEEP_OVERLAP_PAGES bilan bir xil g'oya): agar
                              # oxirgi muvaffaqiyatli saqlashdan beri 5 soatdan KO'P
                              # vaqt o'tgan bo'lsa (masalan tunda yoki sayt uzoq
                              # ochilmagan bo'lsa), oyna AVTOMATIK shu bo'shliqni
                              # to'liq qoplaydigan qilib kengaytiriladi - hech qanday
                              # Open->Received o'tishi qochib ketmaydi.
_KIRIM_BLOB_PATH = "kirim-live-cache.json"


def _kirim_blob_token():
    return os.environ.get("BLOB_READ_WRITE_TOKEN", "").strip() or None


def _kirim_blob_get():
    """Persistlangan (oldingi so'rovlardan yig'ilgan) buyurtmalar. Vercel
    cold-start/qayta deploy bo'lsa ham YO'QOLMAYDI (Blob'da), shuning uchun
    har safar noldan boshlash shart emas."""
    token = _kirim_blob_token()
    if not token:
        return None
    try:
        import requests
        resp = requests.get(
            ...
            headers={"Authorization": f"Bearer {token}", "x-api-version": "7"},
            params={"prefix": _KIRIM_BLOB_PATH, "limit": 10}, timeout=10)
        resp.raise_for_status()
        url = next((b.get("url") for b in resp.json().get("blobs", [])
                    if b.get("pathname") == _KIRIM_BLOB_PATH), None)
        if not url:
            return None
...
def _kirim_blob_put(data):
    ...
    try:
        ...
        requests.put(
            f"https://blob.vercel-storage.com/{_KIRIM_BLOB_PATH}", data=body, timeout=15,
            headers={
        ...
        pass  # keshga yozib bo'lmasa ham natija baribir qaytadi - keyingi safar
              # yana to'liq (yoki qisman) qayta so'raladi, ma'lumot yo'qolmaydi


def _live_kirimdata():
    """`kirimdata`ni JONLI quradi — Invan `/supplier_order`dan TO'G'RIDAN-TO'G'RI
    (`supplier_orders_from_invan()`, pipeline_adapter.py) + `build_kirimdata()`
    (backend_p8_kirim.py, O'ZGARISHSIZ).

    2026-09-10 TUZATISH (Vercel CPU limiti kutilganidan TEZ tugashi sababi
    topildi): ilgari bu funksiya HAR safar (har sahifa ochilganda VA har
    15 daqiqada, HAR ochiq brauzerda) 2026-01-01'dan HOZIRGACHA BARCHA
    ta'minotchi buyurtmalarini Invan'dan qayta so'rardi - oy o'tgani sayin
    (endi 8+ oy) BIR XIL trafikda ham so'rov TOBORA SEKINROQ/OG'IRROQ bo'lib,
    CPU sarfi doimiy o'sib borgan (avval 3 oyga yetgan limit ~10 kunda
    tugagan sababi shu edi).

    Endi ikki qatlamli: (1) 120 kundan eski buyurtmalar Vercel Blob'da
    PERSISTLANADI (bir marta olingach qayta so'ralmaydi - status boshqa
    o'zgarmasligi Bilol tomonidan tasdiqlangan: "ochilgan zakas uzog'i
    bilan 30 kunda keladi, kelmasa qayta ochiladi"), (2) faqat so'nggi
    ~5 kun HAR safar qayta so'raladi (yangi buyurtmalar + Open->Received
    o'tgan buyurtmalar shu oynada - bu funksiya juda tez-tez chaqirilgani
    uchun 5 kun xavfsizlik zaxirasi bilan yetarli), uzoq bo'shliqdan keyin
    esa avtomatik kengayadi. Natija: har chaqiruv endi 120 kun o'rniga
    atigi ~5 kunlik ma'lumot so'raydi, ustiga Blob orqali cold-start/
    deploy'dan keyin ham noldan boshlanmaydi."""
    from datetime import datetime, timedelta, timezone
    from .pipeline_adapter import supplier_orders_from_invan
    from backend_p8_kirim import build_kirimdata

    now = datetime.now(timezone.utc)
    prune_cutoff = (now - timedelta(days=_KIRIM_PRUNE_DAYS)).strftime("%Y-%m-%dT%H:%M:%SZ")
    recent_cutoff = (now - timedelta(hours=_KIRIM_RECENT_HOURS)).strftime("%Y-%m-%dT%H:%M:%SZ")

    stored = _kirim_blob_get() or {}
    stored_orders = stored.get("orders") or {}

    if not stored_orders:
        # Birinchi marta (yoki Blob hali bo'sh/token yo'q) - to'liq 120 kunlik
        # bazani bir marta o'qiymiz.
        fetch_since = prune_cutoff
    else:
        # O'ZINI-O'ZI TUZATISH: agar oxirgi muvaffaqiyatli saqlashdan beri
        # 5 kundan KO'P vaqt o'tgan bo'lsa (sayt uzoq vaqt ochilmagan), oyna
        # shu bo'shliqni to'liq qoplaydigan qilib kengaytiriladi - hech qanday
        # Open->Received o'tishi qochib ketmaydi.
        last_at = str(stored.get("updated_at") or "")
        try:
            last_dt = datetime.fromisoformat(last_at.replace("Z", "+00:00")) if last_at else None
        except ValueError:
            last_dt = None
        fetch_since = min(recent_cutoff, last_dt.strftime("%Y-%m-%dT%H:%M:%SZ")) if last_dt else recent_cutoff
        fetch_since = max(fetch_since, prune_cutoff)  # 120 kundan orqaga hech qachon o'tmaydi
    fresh = supplier_orders_from_invan(since_iso=fetch_since)

    merged = dict(stored_orders)
    for o in fresh:
        oid = str(o.get("id") or "")
        if oid:
            merged[oid] = o
    merged = {k: v for k, v in merged.items() if str(v.get("created_at") or "") >= prune_cutoff}

    _kirim_blob_put({"orders": merged, "updated_at": now.isoformat()})
    return build_kirimdata(list(merged.values()))
...
_kirim_recent_cache: dict = {}   # days -> {"src_at": float, "raw": bytes, "gz": bytes}


def _kirim_recent_response(request, days):
    """Kirimning FAQAT so'nggi `days` kunlik yozuvlari (2026-09-19, Bilol:
    "har yuklanganda butun davrni emas, oxirgi yangilangandan keyingisini
    ol"). Frontend birinchi marta to'liq (120 kunlik) javobni oladi, keyin har
    15 daqiqada faqat shu qisqa oynani so'raydi va o'zidagi ma'lumot ustiga
    order_id bo'yicha yozadi (hajm ~2.7MB -> ~0.3-0.6MB).

    Invan'ga QO'SHIMCHA so'rov YO'Q: mavjud `_kirim_cache` (Blob bilan
    persistlanadigan `_live_kirimdata()` natijasi) dan qirqib beriladi, keshning
    yangilanishi `_serve_swr` orqali odatdagidek. Qator sanasi (`date`) =
    qabul qilingan sana yoki yaratilgan sana, shuning uchun yaqinda
    "Received"ga o'tgan (lekin ancha oldin yaratilgan) buyurtmalar ham
    oynaga tushadi. SKU agregatlari (total_received va h.k.) YUBORILMAYDI -
    frontend ularni birlashtirilgan qatorlardan qayta hisoblaydi."""
    import time as _time
    from datetime import datetime, timezone
    _serve_swr(request, _kirim_cache, _KIRIM_CACHE_TTL, _build_kirim_payload)  # kesh yangiligini ta'minlaydi
    d = _kirim_cache["dict"] or {}
    src_at = _kirim_cache["at"]
    ent = _kirim_recent_cache.get(days)
    if ent is None or ent["src_at"] != src_at:
        cutoff = (datetime.now(timezone.utc) - timedelta(days=days)).strftime("%Y-%m-%dT%H:%M:%SZ")
        skus = {}
        for sk, e in (d.get("skus") or {}).items():
            rows = [a for a in (e.get("arrivals") or []) if str(a.get("date") or "") >= cutoff]
            if rows:
                skus[sk] = {"arrivals": rows}
        raw, gz = _pack({"skus": skus, "delta_days": days})
        ent = {"src_at": src_at, "raw": raw, "gz": gz}
        _kirim_recent_cache.clear()          # faqat oxirgi variant saqlanadi (xotira)
        _kirim_recent_cache[days] = ent
    if "gzip" in request.headers.get("accept-encoding", "").lower():
        return Response(content=ent["gz"], media_type="application/json",
                        headers={"Content-Encoding": "gzip"})
    return Response(content=ent["raw"], media_type="application/json")


@app.get("/api/v1/kirimdata")
def kirimdata(request: Request, sku: str | None = None, days: int | None = Query(None, ge=1, le=120)):
    ...
    if db.USE_TURSO:
        if days and not sku:
            return _kirim_recent_response(request, days)
