...
def newer_last_kirim(arrivals, lk_date, since_ts=None):
    """Sync paytidagi "oxirgi kirim" (`lkDate`)dan YANGIROQ Received kirim bo'lsa
    (sync'dan keyin, hatto BUGUN kelgan) `(qty, "YYYY-MM-DD")` qaytaradi, aks holda None.

    SABAB (2026-09-26, Bilol topilmasi): `lkQty/lkDate` sync paytida (kuniga 2 marta)
    qotadi, jonli yangilanish esa faqat `lkSold`ni kirim miqdoriga kamaytirardi
    (0 dan pastga tushmaydi) - yangi kirim kelgach tovar eski kirim bo'yicha "tugayapti"
    ko'rinib, yana zakas chiqib ketardi (cavendish: 54 ta kelib, 18/18 -> +36 zakas).
    Aniqlik `backend_p_calc_stock.py` dagi lk bilan bir xil: faqat Received, qty>0,
    eng oxirgi KUNdagi hujjatlar QO'SHILADI. `since_ts` (ISO vaqt, bazaviy vaqt) berilsa,
    FAQAT undan KEYIN qabul qilingan kirimlar olinadi - `received_since()` (calcStock'ga
    qo'shiladigan kirim) bilan AYNAN BIR XIL mezon: stokka qo'shilgan kirim va "oxirgi kirim"
    doim birga o'zgaradi (eski, oynadan tashqari kirimga soxta lk ham yaratilmaydi)."""
    base = str(lk_date or "")[:10]
    by_day = {}
    for a in arrivals or []:
        ...
        q = float(a.get("qty") or 0)
        if not d or q <= 0 or d <= base or (since_ts and str(a.get("date")) <= since_ts):
            continue
        by_day[d] = by_day.get(d, 0.0) + q
    if not by_day:
        return None
    d = max(by_day)
    return round(by_day[d], 2), d
