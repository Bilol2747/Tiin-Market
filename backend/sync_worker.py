...
def _diff_write(con, table, keycols, valcols, current, computed):
    """`current`/`computed`: {key_tuple: value_tuple}. Faqat haqiqatan
    o'zgargan/yangi/yo'qolgan qatorlarni yozadi — Turso yozish kvotasini
    tejash uchun (2026-08-11: `refresh_rollups()` har tikda butun kunni
    qayta yozib, kvotani ~26 kunda tugatib qo'ygan edi)."""
    to_write = [k + v for k, v in computed.items() if current.get(k) != v]
    to_delete = [k for k in current if k not in computed]
    if not to_write and not to_delete:
        return
    allcols = keycols + valcols
    placeholders = ",".join("?" * len(allcols))
    keyclause = " AND ".join(f"{c}=?" for c in keycols)
    with con:
        if to_delete:
            con.executemany(f"DELETE FROM {table} WHERE {keyclause}", to_delete)
        if to_write:
            con.executemany(
                f"INSERT INTO {table} ({','.join(allcols)}) VALUES ({placeholders}) "
                f"ON CONFLICT({','.join(keycols)}) DO UPDATE SET "
                + ",".join(f"{c}=excluded.{c}" for c in valcols),
                to_write)


def refresh_rollups(con, days):
    """Faqat ta'sirlangan kunlarning jamlanmasini qayta hisoblaydi VA faqat
    haqiqatan o'zgargan qatorlarni yozadi (avvalgi versiya kun ichidagi
    BARCHA SKU qatorini har tikda o'chirib-qayta yozardi — 2000+ qator ×
    kuniga yuzlab tik = Turso yozish kvotasini tez tugatgan asosiy sabab)."""
    if not days:
        return
    marks = ",".join("?" * len(days))
    p = tuple(days)

    cur_sku = {(r["d"], r["sku"]): (r["qty"], r["revenue"], r["cost"], r["receipts"])
               for r in con.execute(
                   f"SELECT d, sku, qty, revenue, cost, receipts FROM daily_sku WHERE d IN ({marks})", p)}
    new_sku = {(r["d"], r["sku"]): (r["qty"], r["revenue"], r["cost"], r["receipts"])
               for r in con.execute(f"""
        SELECT d, sku, SUM(qty*sign) AS qty, SUM(revenue*sign) AS revenue,
               SUM(supply_price*qty*sign) AS cost, COUNT(DISTINCT receipt_id) AS receipts
        FROM receipt_items WHERE d IN ({marks}) AND sku <> '' GROUP BY d, sku""", p)}
    _diff_write(con, "daily_sku", ["d", "sku"], ["qty", "revenue", "cost", "receipts"], cur_sku, new_sku)

    cur_tot = {(r["d"],): (r["revenue"], r["cost"], r["receipts"], r["items_qty"], r["refund_total"], r["wholesale_rev"])
               for r in con.execute(
                   f"SELECT d, revenue, cost, receipts, items_qty, refund_total, wholesale_rev "
                   f"FROM daily_totals WHERE d IN ({marks})", p)}
    new_tot = {(r["d"],): (r["revenue"], r["cost"], r["receipts"], r["items_qty"], r["refund_total"], r["wholesale_rev"])
               for r in con.execute(f"""
        SELECT i.d AS d, SUM(i.revenue*i.sign) AS revenue, SUM(i.supply_price*i.qty*i.sign) AS cost,
               COUNT(DISTINCT i.receipt_id) AS receipts, SUM(i.qty*i.sign) AS items_qty,
               SUM(CASE WHEN i.sign < 0 THEN i.revenue ELSE 0 END) AS refund_total,
               SUM(CASE WHEN r.is_wholesale = 1 THEN i.revenue*i.sign ELSE 0 END) AS wholesale_rev
        FROM receipt_items i JOIN receipts r ON r.id = i.receipt_id
        WHERE i.d IN ({marks}) GROUP BY i.d""", p)}
    _diff_write(con, "daily_totals", ["d"],
                ["revenue", "cost", "receipts", "items_qty", "refund_total", "wholesale_rev"], cur_tot, new_tot)

    cur_emp = {(r["d"], r["employee"]): (r["revenue"], r["receipts"])
               for r in con.execute(
                   f"SELECT d, employee, revenue, receipts FROM daily_employee WHERE d IN ({marks})", p)}
    new_emp = {(r["d"], r["employee"]): (r["revenue"], r["receipts"])
               for r in con.execute(f"""
        SELECT r.d AS d, r.employee AS employee, SUM(i.revenue*i.sign) AS revenue, COUNT(DISTINCT r.id) AS receipts
        FROM receipts r JOIN receipt_items i ON i.receipt_id = r.id
        WHERE r.d IN ({marks}) AND r.employee <> ''
        GROUP BY r.d, r.employee""", p)}
    _diff_write(con, "daily_employee", ["d", "employee"], ["revenue", "receipts"], cur_emp, new_emp)
...
def sync_catalog(con, token, verbose=False):
    ...
    # Faqat O'ZGARGAN qatorlarni yozamiz — Turso yozish kvotasi cheklangan
    # (2026-07-16'da aynan "har safar hammasini qayta yozish" sababli bir
    # hisob bloklangan edi). `updated_at`siz solishtiramiz — u har doim
    # farq qilardi va hech narsani "o'zgarmagan" deb topmasdi.
    current = {r["sku"]: (r["product_id"], r["name"], r["name_raw"], r["cat"], r["cat_top"],
                          r["supplier"], r["supplier_id"], r["unit"], r["price"],
                          r["supply_price"], r["stock"], r["is_active"])
               for r in con.execute(
                   "SELECT sku, product_id, name, name_raw, cat, cat_top, supplier,"
                   " supplier_id, unit, price, supply_price, stock, is_active FROM products")}
    changed = [r for r in rows if current.get(r[0]) != r[1:13]]
    ...
    with con:
        if changed:
            con.executemany(
                ...
                " is_active=excluded.is_active, updated_at=excluded.updated_at", changed)
            con.executemany("INSERT INTO product_barcodes (sku, barcode) VALUES (?,?) "
                            "ON CONFLICT DO NOTHING", bcs)
            # Qidiruv indeksini qayta qurish — FAQAT haqiqiy o'zgarish bo'lganda
            # (21k qatorni har safar qayta yozish shart emas). Kontentsiz FTS5
            # jadvali DELETE ni qo'llab-quvvatlamaydi — 'delete-all' kerak.
...
def _kirim_latest_created_at(con):
    r = con.execute("SELECT MAX(created_at) FROM sup_orders").fetchone()
...
def sync_kirim(con, token, verbose=False):
    ...
    while True:
        ...
        if not data:
            break
        orders, arrivals_by_id = [], {}
        for o in data:
            order_row, arr_rows = parse_supplier_order(o)
            if order_row is None:
                continue
            orders.append(order_row)
            arrivals_by_id[order_row[0]] = arr_rows
        if orders:
            # Faqat HAQIQATAN o'zgargan (yoki yangi) buyurtmalarni yozamiz —
            # avval har sahifada BARCHASI qayta yozilardi (o'zgarmagan bo'lsa
            # ham), bu Turso yozish kvotasini keraksiz tez tugatgan edi
            # (2026-08-11, `refresh_rollups` bilan bir xil muammo).
            ids0 = [(r[0],) for r in orders]
            current = {r["id"]: tuple(r[c] for c in
                       ("id", "external_id", "created_at", "d", "expected_date",
                        "received_at", "status", "supplier", "supplier_id", "total_price"))
                       for r in con.execute(
                           f"SELECT id, external_id, created_at, d, expected_date, received_at,"
                           f" status, supplier, supplier_id, total_price FROM sup_orders"
                           f" WHERE id IN ({','.join('?' * len(ids0))})", [i for (i,) in ids0])}
            orders = [r for r in orders if current.get(r[0]) != r]
            arrivals = [a for r in orders for a in arrivals_by_id[r[0]]]
        if orders:
            with con:
