...
class _TursoCursor:
    """`sqlite3.Cursor`ning sync_worker.py ishlatadigan qismini (fetchone/
    fetchall) taqlid qiladi. libsql Row allaqachon `r[0]`/`r["col"]`
    ikkalasini ham qo'llab-quvvatlaydi — sqlite3.Row bilan bir xil."""
    __slots__ = ("_rs",)
...
class _TursoWriteConn:
    ...
    def executemany(self, sql, seq_of_params):
        ...
        for i in range(0, len(stmts), self._BATCH):
            self._client.batch(stmts[i:i + self._BATCH])
