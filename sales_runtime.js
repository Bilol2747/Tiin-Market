...
async function zkFileExportXLSX(){
  ...
  const headerRow=ws.addRow(["Наименование","Штрих код","Кол-во","Цена"]);
  ...
  rows.forEach(r=>{
    const row=ws.addRow([r.name,r.bcRaw||r.bc||"",r.qty,r.price||0]);
    row.getCell(3).numFmt=Number.isInteger(r.qty)?"#,##0":"#,##0.##";
    row.getCell(4).numFmt=Number.isInteger(r.price)?"#,##0":"#,##0.##";
  });
  ws.columns=[{width:42},{width:18},{width:10},{width:12}];
  const buf=await wb.xlsx.writeBuffer();
  const a=document.createElement("a");
  a.href=URL.createObjectURL(new Blob([buf],{type:"application/vnd.openxmlformats-officedocument.spreadsheetml.sheet"}));
...
function _zkBuildSuppliers(depth){
  ...
  const out=supNames.map(sup=>{
    ...
    const rows=bySup[sup].map(v=>{
      ...
      const useCalcStock=zkRowStockMode[key]==="calc"&&v.calcStock!=null;
      const stock=(useCalcStock?v.calcStock:v.stock)||0;
      const _da=eda(v);
...
function zkOpenSupplier(sup){zkMode="detail";zkSupFilter=sup;zkLastSup=sup;zkQuery="";zkDetailQuery="";zkPage=1;zkCatFilter="";zkSubFilter="";zkBcFilter=null;zkBcShowAll=false;zkBcStats=null;zkDepth=_zkPickDepthFor(sup);const inp=document.getElementById("zk-search");if(inp)inp.value="";const x=document.getElementById("zk-search-x");if(x)x.style.display="none";const di=document.getElementById("zk-detail-search");if(di)di.value="";const dx=document.getElementById("zk-detail-search-x");if(dx)dx.style.display="none";_zkRefreshCatFilters();_zkSaveViewState();renderZakas();}
...
function zkSetRowStockMode(ri,mode){
  const r=_ZK_ALLROWS[ri];if(!r)return;
  if(mode==="calc"){if(r.calcStock==null)return;zkRowStockMode[r.key]="calc";}
  else delete zkRowStockMode[r.key];
  zkSaveManual();
  renderZakas();
}
...
function zkSetSupplierStockMode(si,mode){
  ...
  s.rows.forEach(r=>{
    ...
    if(mode==="calc")zkRowStockMode[r.key]="calc";else delete zkRowStockMode[r.key];
  });
  zkSaveManual();
  renderZakas();
}
...
function renderZakas(){
  ...
  shownSups.forEach(s=>{
    ...
    h+=`<div class="zk-sup-block"><div class="zk-sup-name"><span style="display:flex;align-items:center;gap:10px;min-width:0;flex:1 1 auto"><input type="checkbox" class="zk-chk zk-sup-chk"${supChkAttrs} onchange="zkToggleSupplier(${s._si})"><span class="zk-sup-title" title="${esc(s.sup)}">${esc(s.sup)}</span></span><span class="zk-sup-meta">${needBadge} &nbsp;·&nbsp; ${t("zk_total_label")} <b>${totTxt||"0"}</b><span class="zk-target-edit">${t("zk_target_label")} <input class="zk-target-inp" type="number" min="0" max="365" value="${s.target}" onchange="zkSetTarget(${s._si},this.value)"></span></span></div><div class="zk-tbl-wrap"><table class="zk-ktbl"><colgroup><col style="width:4%"><col style="width:3%"><col style="width:16%"><col style="width:5%"><col style="width:17%"><col style="width:6%"><col style="width:6%"><col style="width:7%"><col style="width:5%"><col style="width:9%"><col style="width:13%"><col style="width:9%"></colgroup><thead><tr><th style="text-align:center"><input type="checkbox" class="zk-chk zk-sup-chk"${supChkAttrs} onchange="zkToggleSupplier(${s._si})"></th><th>#</th>${_zkTh(t("zk_col_product"),"name","left")}${_zkTh("ABC","abc","center")}${stockTh}${_zkTh(t("zk_col_daily"),"dailyAvg")}${_zkTh(t("zk_col_days_left"),"daysLeft")}<th style="text-align:right">${t("zk_col_extra_days")}</th><th style="text-align:center">${t("zk_col_status")}</th>${_zkTh(t("zk_col_cost"),"rcost")}${_zkTh(t("zk_col_order"),"orderQty")}<th style="text-align:right">${t("zk_col_sum")}</th></tr></thead><tbody>`;
    if(!visRows.length){
    ...
    visRows.forEach((r,i)=>{
      ...
      if(r.calcStock==null){
        ...
        const confCls=r.calcConf==="yuqori"?"zk-stock-conf-hi":r.calcConf==="eskirgan"?"zk-stock-conf-stale":"zk-stock-conf-mid";
        const calcTxt=_fmt(r.calcStock);
        const tt=esc(`${t("zk_stock_tt_calc")}: ${calcTxt} — ${t("zk_stock_tt_conf_"+(r.calcConf==="yuqori"?"hi":r.calcConf==="eskirgan"?"stale":"mid"))}, ${t("zk_stock_tt_evidence")} ${r.calcEvidence}, ${t("zk_stock_tt_anchor")} ${r.calcAnchor}`);
        const isCalc=r.stockMode==="calc";
        const calcStyle=isCalc?"font-weight:700":"font-weight:600;opacity:.72";
        ...
        const _chk=(r.calcRule&&r.calcRule!=="oddiy")
          ?`<span class="zk-stock-chk" title="${esc(t("zk_stock_tt_checked"))}"></span>`:"";
        calcCell=`<span class="zk-stock-half zk-stock-calc ${confCls}" style="${calcStyle}" onclick="event.stopPropagation();zkSetRowStockMode(${r._ri},'calc')" title="${tt}">${_chk}${calcTxt}</span>`;
      ...
      h+=`<tr><td style="text-align:center"><input type="checkbox" class="zk-chk" ${_checked?"checked":""} onchange="zkToggleRow(${r._ri})" onclick="event.stopPropagation()"></td><td style="color:#bbb;font-size:12px">${i+1}</td><td><div class="zk-prod-link" onclick="zkOpenProduct(${r._ri})">${esc(r.name)}</div>${r.sku?`<div style="font-size:11px;color:#aaa">${esc(r.sku)}</div>`:""}</td><td style="text-align:center"><span style="font-size:11.5px;font-weight:700;padding:3px 8px;border-radius:4px;background:${r.abc==="A"?"#d7f3ea":r.abc==="B"?"#e3ddf9":"#fce8c8"};color:${r.abc==="A"?"#1D9E75":r.abc==="B"?"#534AB7":"#c97f14"}">${r.abc||"—"}</span></td><td style="text-align:right">${stTxt}</td><td style="text-align:right;color:#666">${dTxt}</td><td style="text-align:right;color:#666">${dlTxt}</td><td style="text-align:right"><input class="zk-adj-inp${r.adj?" nonzero":""}" type="number" value="${r.adj}" onchange="zkSetAdj(${r._ri},this.value)" onclick="event.stopPropagation()"></td><td style="text-align:center">${statusCell}</td><td style="text-align:right">${costCell}</td><td style="text-align:right;padding:2px 4px">${r.minAdd>0&&!isManual?`<span style="color:#EF9F27;font-size:11px;margin-right:3px">+${r.minAdd}</span>`:"" }<input class="zk-adj-inp${isManual?' nonzero':''}" type="number" min="0" step="${r.kg?'0.1':'1'}" value="${oqRaw}" onchange="zkSetQty(${r._ri},this.value)" onclick="event.stopPropagation()"> <span style="color:#888;font-size:12px">${u}</span></td><td style="text-align:right;font-weight:600">${sumTxt}</td></tr>`;
    });
...
async function _zkExportInvanTemplate(sups){
  ...
  const headers=["Наименование","Штрих код","Кол-во","Цена"];
  let first=true;
  for(const [sup,rows] of bySup){
    ...
    rows.forEach(r=>{
      ...
      const row=ws.addRow([r.name,bcTxt,r.orderQty,r.rcost||0]);
...
let FM=null,fmFilt="all",fmQ="",fmFrom="",fmTo="",fmOpenId=null;
...
function _fmCalc(f){
  ...
  // Guruh ustunlari (0-15..45+) - FAQAT bizning sinxron davrimizdagi ("cheklar")
  // qarzga sotuv/qaytarish faoliyati, informatsion (qachon qarz olingani).
  (f.cheklar||[]).forEach(c=>{if(c.d<fmFrom||c.d>fmTo)return;o[_fmBucket(_fmAge(c.d))]+=c.s;o.n++;});
  // Jami qarz / Puli bor - Invan'ning YAGONA `balans` maydonidan (butun tarix
  // bo'yicha joriy holat, biz ko'rmaydigan to'lovlarni ham hisobga oladi).
  // Ikkisi hech qachon BIRGA chiqmaydi - bittasi doim nol (2026-08-01 tuzatildi,
  // avval ikki mustaqil hisob bir firma uchun ham qarz, ham kredit ko'rsatib
  // chalkashlik keltirib chiqargan edi).
  o.jami=f.balans<0?-f.balans:0;
  o.pb=f.balans>0?f.balans:0;
  return o;
}
function _fmRows(){
...
function fmRender(){
  ...
  if(cnt)cnt.textContent=rows.length+" "+t("fm_firma_cnt")+" · "+t("fm_jami").toLowerCase()+" "+(_fmNum(T.jami)||"0")+" so'm";
