...
const I18N={
  ...
  zk_calcmode_stock:{uz:"Stok",en:"Stock",ru:"Склад"},
  zk_calcmode_kirim:{uz:"Oxirgi kirim",en:"Last delivery",ru:"Последняя поставка"},
...
// ZAKAS HISOB REJIMI (2026-09-22, Bilol so'rovi): ikkita MUSTAQIL formula, sahifa
// tepasidagi "Stok"/"Oxirgi kirim" tugmalari bilan almashtiriladi. "stock" = mavjud
// calcStock-asosli formula (o'zgarmagan, sukut bo'yicha). "kirim" = YANGI, Invan
// qoldig'iga UMUMAN tayanmaydigan formula: oxirgi bilingan kirimdan (lkQty) beri
// necha dona sotilgani (lkSold) - backend_p_zakas_auto.py'dagi bilan AYNAN bir xil
// filtrlar (MUST/CAN_ORDER_DAYS, 15-60 kunlik reference sanity, bootstrap-only -
...
{const _td=new Date().toLocaleDateString("sv-SE");   // YYYY-MM-DD (mahalliy sana)
 if(_zkLoadStr("zk_row_checked_day","")!==_td){zkRowChecked={};try{localStorage.setItem("zk_row_checked_day",_td);localStorage.removeItem("zk_row_checked");}catch(e){}}}
...
function _zkSyncCalcModeBtns(){
  const sb=document.getElementById("zk-calcmode-stock"),kb=document.getElementById("zk-calcmode-kirim");
  if(sb)sb.classList.toggle("active",zkCalcMode==="stock");
  if(kb)kb.classList.toggle("active",zkCalcMode==="kirim");
}
...
// AVTOMATIK BELGI (2026-09-24, Bilol): zakasi bor (orderQty>0) tovar SUKUT bo'yicha belgilangan -
// firma sahifasiga kirganda "Belgilangan summa" darhol ko'rinadi. Foydalanuvchi qo'lda belgini
// olib tashlasa/qo'ysa - shu saqlanadi (zkRowChecked), lekin har KUNI yangidan boshlanadi
// (quyida zk_row_checked_day), aks holda kechagi qo'lda olib tashlangan/yuborilgan belgilar
// yangi ehtiyojni yashirib qolardi.
// Faqat MUNTAZAM zakas (kalit "normal:" bilan boshlanadi) - Chuqur zakas avvalgidek qo'lda belgilanadi
// (aks holda firmaning "Belgilangan summa"si Chuqur qatorlar bilan shishib ketadi).
function _zkIsChecked(r){const v=zkRowChecked[r.key];return v!=null?v:(r.orderQty>0&&String(r.key).indexOf("normal:")===0);}
...
async function zkSendToInvan(target){
  ...
  if(isProd){const _rc=await _zkRecentOrdersInfo(sup,myToken);if(_rc.line)confirmMsg=_rc.line+"\n\n"+confirmMsg;}
...
async function zkFileExportXLSX(){
  ...
  if(!rows.length){alert(t("zk_no_selection"));return;}
  // Supplier nomi + Сумма (formula) + ИТОГО - _zkBuildOrderBuffer (jadval), _zkExportInvanTemplate bilan bir xil.
  const buf=await _zkBuildOrderBuffer(zkFileSupplier||"",rows.map(r=>({
    name:r.name,bc:r.bcRaw||r.bc||"",qty:r.qty,price:r.price||0,
    qtyFmt:Number.isInteger(r.qty)?"#,##0":"#,##0.##",priceFmt:Number.isInteger(r.price)?"#,##0":"#,##0.##"})));
  const a=document.createElement("a");
  a.href=URL.createObjectURL(new Blob([buf],{type:"application/vnd.openxmlformats-officedocument.spreadsheetml.sheet"}));
...
// darajasida (hamma qator bir xil status oladi), shuning uchun qoida ham buyurtma
// (order_id) bo'yicha - bir ta'minotchining BOSHQA buyurtmalariga (masalan suv va
// shirinlik alohida) ta'sir qilmaydi. Xarita P8 yangilanganda qayta quriladi.
let _krRcvP8=null,_krRcvSync=null,_krRcvMap=null;
function _krOrderHasReceipt(orderId){
  const sync=(typeof _p8SyncAt!=="undefined")?_p8SyncAt:0;
  if(!_krRcvMap||_krRcvP8!==P8||_krRcvSync!==sync){
    _krRcvMap={};
    for(const s in P8.skus)for(const a of (P8.skus[s].arrivals||[]))if(a.order_id&&(a.qty||0)>0)_krRcvMap[a.order_id]=true;
    _krRcvP8=P8;_krRcvSync=sync;
  }
  return !!_krRcvMap[orderId];
}
function krPendingQty(sku){
  ...
  if(latest.order_id&&!latest._local&&_krOrderHasReceipt(latest.order_id))return 0;
...
// KIRIM-OQIM ZAKAS FORMULASI (2026-09-22, Bilol so'rovi) - backend_p_zakas_auto.py'ning
// _bootstrap_sku()/_apply_reference_sanity()/_order_qty_for() JS ko'chirmasi (bir xil
// bo'lishi SHART - alohida hisoblab, saytdan boshqa raqam chiqmasligi kk). Invan qoldig'iga
// (calcStock ham shunga bog'liq) UMUMAN tayanmaydi - faqat oxirgi bilingan kirimdan (lkQty,
// backend_p_calc_stock.py, har buildda qaytadan hisoblanadi) beri necha dona sotilganini
// (lkSold) ko'radi. Backend'dagi kabi HOLAT saqlanmaydi - lkQty/lkSold ALLAQACHON har
// build'da "toza" qayta hisoblangani uchun frontend ham har chaqiriqda "bootstrap"
// qiladi, alohida fayl/state kerak emas.
const ZKA_MUST_ORDER_DAYS=7,ZKA_CAN_ORDER_DAYS=14,ZKA_REF_MIN_DAYS=15,ZKA_REF_MAX_DAYS=45,ZKA_REF_RESET_DAYS=30;
// 2026-09-25 (Bilol): ZKA_REF_MAX_DAYS 60 -> 45. Oxirgi kirim shu kundan uzoqqa yetadigan bo'lsa (sekin sotiladigan tovar) zakas
// ZKA_REF_RESET_DAYS (~30) kunlik miqdorga tushiriladi - bir vaqtda ortiqcha ko'p zakas berilmasin (1 kunlik ma'lumotda ~200 mln kamayadi).
// QOLDIQ TEKSHIRUVI: Invan VA hisoblangan qoldiq IKKALASI ham shuncha kunlik zaxira ko'rsatsa, kirim modeli "tugagan" desa ham zakas berilmaydi
// (yozilmagan kirimlar / eski zaxira: Jacobs - kirim 6.5 oy oldin 1008, sotilgan 4594, Invan qoldig'i 97 kunlik). Ikkalasi kelishishi SHART.
const ZKA_STOCK_GUARD_DAYS=30;
// KARABKAGA YUQORIGA YAXLITLASH CHEGARASI (2026-09-25, Bilol): karobka miqdorigacha QO'SHISH natijasida zaxira shuncha kundan ko'p bo'lib ketsa
// (yoki tezlik noma'lum bo'lsa) - yuqoriga yaxlitlanmaydi, xom miqdor (kamida 1) qoladi. Sekin sotiladigan tovarga butun karobka berilmasin
// (tushenka: kuniga 0.03 dona, xom 1 dona, karobka 24 -> 24 dona = 800 kunlik zaxira bo'lib ketardi).
// Bundan tashqari qo'shilgan miqdor xom miqdorning ZKA_BOX_UP_MAX_PCT foizidan ko'p bo'lmasin (xom 70, karobka 40 -> 80 (+14%) mumkin, xom 25 -> 40 (+60%) yo'q).
const ZKA_BOX_UP_MAX_DAYS=30;
const ZKA_BOX_UP_MAX_PCT=20;
// Kamaytirish (pastga yaxlitlash) tomoni ham xuddi shunday: xomning ZKA_BOX_DOWN_MAX_PCT% idan ko'p kamaymaydi (xom 5, karobka 30 -> 0 emas, 5 qoladi).
const ZKA_BOX_DOWN_MAX_PCT=20;
// ZAKAS CHEGARASI (2026-09-24, Bilol): zakas oxirgi kirim miqdorining ZKA_MAX_ORDER_MULT baravaridan
// ORTIQ bo'lmasin (istisnosiz). Sabab: oxirgi kirim 15 kundan kam yetgan bo'lsa formula uni "juda
...
// "kichik" deb 111 zakas berilardi). Faqat kirim oxirgi ZKA_LAST_KIRIM_MAX_DAYS kun ichida bo'lsa,
// kamida ZKA_LAST_KIRIM_MIN_DAYS kun o'tgan bo'lsa va kirim hali tugamagan (0<lkSold<lkQty) bo'lsa;
// aks holda 30 kunlik o'rtacha qoladi. Buyurtma HAJMI (resetDays*velocity) va qolgan-kunlar 30
// kunlikda qoladi. backend_p_zakas_auto.py LAST_KIRIM_MIN/MAX_DAYS bilan bir xil.
// 2026-09-26 (Bilol): MIN 5 -> 15 kun. Kirim juda yaqinda bo'lgan bo'lsa kunlar soni kam va tezlik
// tasodifga bog'liq bo'lib qoladi - jonli ma'lumotda o'lchandi: kirim 5-9 kun oldin bo'lganda
// tovarlarning 26% ida bu tezlik uzoq muddatli o'rtachadan 2 baravardan ko'p farq qilgan, 15-30 kun
// oralig'ida esa atigi 7%. Endi 15 kundan yosh kirimda 60 kunlik o'rtacha (avg30sa) ishlatiladi.
const ZKA_LAST_KIRIM_MIN_DAYS=15,ZKA_LAST_KIRIM_MAX_DAYS=30;
// TA'MINOTCHIGA MOSLASHUVCHI RESET-MAQSAD (2026-09-23, Bilol so'rovi): "kichik" reference
// har doim 30 kunlikka qayta hisoblanganda, ta'minotchining HAQIQIY kelish tezligidan qat'i
// nazar bir xil (30) qo'llanardi - real misolda (сливки сваля, "OOO Leader Import") bu
// ta'minotchi ~14-16 kunda kelib turadi, lekin tizim 30 kunlik (47 dona) buyurtma berardi -
// haqiqiy kelish siklidan deyarli 2x ko'p. Endi ta'minotchining BARCHA tovarlari bo'yicha
// (bitta SKU emas) haqiqiy "Received" kirim sanalaridan mediana oraliq topiladi - bu orqali
// bitta ta'minotchining HAMMA tovari BIR XIL maqsadga (20 yoki 30 kun) moslashadi, ya'ni
// bir vaqtda "tugaydi" va ta'minotchi kelganda hammasi birga to'ldiriladi (foydalanuvchi
// aniq talabi: "Supplier birdan zakas olib kelganda hamma tugaganlar kelishi kk"). Mediana
// 25 kundan kichik bo'lsa - 20 kunlik, aks holda 30 kunlik maqsad tanlanadi (ikkalasi ham
// saytda allaqachon tanish sonlar - 20 Stok rejimining standart maqsadli kuni). HAR safar
// (P8 yangilanganda) qayta hisoblanadi - keshlanmaydi (avg30sa bilan bir xil tamoyil).
const ZKA_RESET_SHORT_DAYS=25,ZKA_RESET_LONG_DAYS=30,ZKA_RESET_SWITCH_DAYS=27.5;
let _zkaSupResetCache={at:0,map:null};
function _zkaBuildSupplierResetDays(){
  const stamp=(typeof _p8SyncAt!=="undefined"&&_p8SyncAt)||0;
  if(_zkaSupResetCache.map&&_zkaSupResetCache.at===stamp)return _zkaSupResetCache.map;
  const bySup={};
  if(P8&&P8.skus){
    for(const sku in P8.skus){
      const entry=P8.skus[sku];
      if(!entry||!entry.arrivals)continue;
      for(const a of entry.arrivals){
        if(a.status!=="Received"||!a.date||!a.supplier)continue;
        (bySup[a.supplier]=bySup[a.supplier]||new Set()).add(String(a.date).slice(0,10));
      }
    }
  }
  const out={};
  for(const sup in bySup){
    const dates=[...bySup[sup]].sort();
    if(dates.length<2){out[sup]=ZKA_RESET_LONG_DAYS;continue;}
    const gaps=[];
    for(let i=1;i<dates.length;i++){
      const g=Math.round((new Date(dates[i])-new Date(dates[i-1]))/86400000);
      if(g>0)gaps.push(g);
    }
    if(!gaps.length){out[sup]=ZKA_RESET_LONG_DAYS;continue;}
    gaps.sort((a,b)=>a-b);
    const mid=Math.floor(gaps.length/2);
    const median=gaps.length%2?gaps[mid]:(gaps[mid-1]+gaps[mid])/2;
    out[sup]=median<ZKA_RESET_SWITCH_DAYS?ZKA_RESET_SHORT_DAYS:ZKA_RESET_LONG_DAYS;
  }
  _zkaSupResetCache={at:stamp,map:out};
  return out;
}
function _zkaCompute(v,velocity,pendingQty,resetDays){
  ...
  // belgilaydi. "katta" (>60 kun - masalan 56 dona, 70 kunga yetadigan) holatda ham
  // "qolgan" (hozir qancha bor) HAR DOIM xom lkQty-lkSold'dan olinadi - "reference
  // ishonchsiz" degani "sotuv kuzatuvi ham ishonchsiz" degani EMAS (2026-09-23,
  // Bilol tuzatishi: avval "katta"da qolgan ham reference'ga tenglashtirilib, "to'liq
  // bor" deb noto'g'ri faraz qilinardi - haqiqiy misolda (bri сыр, SKU bilan
  ...
  if(days<ZKA_REF_MIN_DAYS||days>ZKA_REF_MAX_DAYS)reference=resetDays*velocity;
  const qolgan=lkQty-lkSold;
  // Ochiq (hali kelmagan) buyurtma bor - saytning mavjud "Open PO" qoidasi bilan bir
...
// Received kirimga qaraydi. Sabab (real misolda topilgan, brынза сыр fitaki): butun
// tarixni olsak, 1+ yil oldingi eskirgan/bir martalik miqdorlar (masalan 10, 10, 50)
// GCD'ni buzib, foydasiz kichik son (2) berardi - so'nggi 6 tasiga qarasak, ta'minotchi
// HOZIR haqiqatan qaysi o'lchamda yetkazayotgani (masalan 12) aniq chiqadi. BARQARORLIK
// tekshirildi: bizning o'zimiz shu karobka o'lchamining karralisida buyurtma qilgani
// uchun, keyingi safar "so'nggi 6"ga bizning YANGI buyurtmamiz qo'shilganda ham GCD
// o'zgarmay qoladi (o'z-o'zini buzmaydi).
function _zkaBoxSize(sku){
  if(!P8||!P8.skus||!sku)return 0;
  const entry=P8.skus[String(sku)];
  if(!entry||!entry.arrivals)return 0;
  const recent=entry.arrivals.filter(a=>a.status==="Received"&&a.qty>0)
    .slice().sort((a,b)=>(b.date||"").localeCompare(a.date||""))
    .slice(0,6)
    .map(a=>Math.round(a.qty));
  const qtys=[...new Set(recent)];
...
function _zkBuildSuppliers(depth){
  if(!ZITEMS)return [];
  _zkAutoReapplySent();  // avtomatik yuborilgan buyurtmalarning vaqtinchalik "Open" belgisi (P8 qayta yuklangan bo'lsa ham)
  ...
  // Faqat "Oxirgi kirim" rejimida kerak - Stok rejimida hisoblash behuda (_zkaCompute
  // umuman chaqirilmaydi), shuning uchun shu rejimda hisoblanadi.
  const _supResetDays=(zkCalcMode==="kirim"&&!chuqur)?_zkaBuildSupplierResetDays():null;
  const out=supNames.map(sup=>{
    ...
    const rows=bySup[sup].map(v=>{
      ...
      let _zkaQolganKunlar=null,_zkaSpeed=null,_zkaSignal=null,_zkaStockOk=false;
      if(zkCalcMode==="kirim"&&!chuqur){
        ...
        // (eski ma'lumot) filtr ishlamaydi. Sonlar (zakas kerak, Tezkor ro'yxat, firmalar ro'yxati)
        // orderQty>0 dan sanaladi, shuning uchun o'zi kamayadi. Qoidaning o'zi: build_prev_avg.py.
        if(v.zabc==="C"&&v.ck===0)orderQty=0;
        // QOLDIQ TEKSHIRUVI (2026-09-25): Invan VA hisoblangan qoldiq ikkalasi ZKA_STOCK_GUARD_DAYS kunlik zaxira ko'rsatsa - zakas yo'q.
        if(orderQty>0&&_zkaSpeed>0&&v.calcStock!=null&&Number.isFinite(Number(v.calcStock))){
          const _phys=Math.min(Number(v.stock||0),Number(v.calcStock));
          if(_phys>=ZKA_STOCK_GUARD_DAYS*_zkaSpeed){orderQty=0;_zkaStockOk=true;if(_zkaSignal)_zkaSignal="normal";}
        }
      }
      ...
      if(manualQty==null&&!v.kg&&orderQty>0&&zkCalcMode!=="kirim"){
      ...
      // "Oxirgi kirim" rejimidagi karobka yaxlitlash (2026-09-23, Bilol so'rovi) -
      // yuqoridagi (Stok rejimi) usulidan FARQI: faqat YUQORIGA emas, ENG YAQIN
      // karraliga (pastga HAM, yuqoriga HAM) yaxlitlaydi - `_zkaBoxSize()` (oxirgi
      // 6 marta Received kirimdan GCD). Real misolda tasdiqlangan (brынза сыр
      // fitaki): xom ehtiyoj 37, karobka 12 -> eng yaqin karrali 36 (-1), 48 emas.
      // Karobka topilmasa (2+ xil son yo'q) - xom son O'ZGARISHSIZ qoladi.
      else if(manualQty==null&&!v.kg&&orderQty>0&&zkCalcMode==="kirim"&&!chuqur){
        boxSize=_zkaBoxSize(v.sku);
        if(boxSize>1){
          const rem=orderQty%boxSize;
          if(rem!==0){
            boxAdd=(rem<=boxSize/2)?-rem:(boxSize-rem);
            // XAVFSIZLIK: pastga yaxlitlash 0GACHA tushirib yuborsa, lekin qolgan
            // kun ALLAQACHON shoshilinch (MUST_ORDER_DAYS'dan kam) bo'lsa - 0 o'rniga
            // BITTA karobkaga (yuqoriga) yaxlitlanadi, shoshilinch payt buyurtamasiz
            // qolmasin.
            if(orderQty+boxAdd<=0&&_zkaQolganKunlar!=null&&_zkaQolganKunlar<ZKA_MUST_ORDER_DAYS){
              boxAdd=boxSize-rem;
            }
            // YUQORIGA yaxlitlash ortiqcha zaxira bermasin: (orderQty+boxAdd)/tezlik > ZKA_BOX_UP_MAX_DAYS yoki tezlik noma'lum yoki qo'shilgan miqdor xomning ZKA_BOX_UP_MAX_PCT% idan ko'p -> xom miqdor.
            if(boxAdd>0&&(!(_zkaSpeed>0&&(orderQty+boxAdd)/_zkaSpeed<=ZKA_BOX_UP_MAX_DAYS)||boxAdd*100>ZKA_BOX_UP_MAX_PCT*orderQty))boxAdd=0;
            // PASTGA yaxlitlash ham xom miqdorning ZKA_BOX_DOWN_MAX_PCT% idan ko'p kamaytirmasin (mayda ehtiyoj 0 bo'lib zakasdan tushib qolmasin) -> xom miqdor.
            if(boxAdd<0&&-boxAdd*100>ZKA_BOX_DOWN_MAX_PCT*orderQty)boxAdd=0;
            orderQty+=boxAdd;
          }
          // Karobka yaxlitlash chegaradan (ZKA_MAX_ORDER_MULT x oxirgi kirim) oshirib yubormasin:
          // oshsa - chegaradan oshmaydigan ENG KATTA karobka karralisiga tushiriladi.
          const _capQ=v.lkQty>0?Math.floor(ZKA_MAX_ORDER_MULT*Number(v.lkQty)):0;
          if(_capQ>0&&orderQty>_capQ){
            const _fl=Math.floor(_capQ/boxSize)*boxSize;
            const _nq=_fl>0?_fl:_capQ;
            boxAdd+=_nq-orderQty;orderQty=_nq;
          }
        }
      }
...
function zkOpenSupplier(sup){
  const _q=zkQuery;
  // Ro'yxat darajasida kategoriya bo'yicha filtrlangan bo'lsa (masalan "suv"),
  // supplier ichiga kirilganda ham SHU filtr avtomatik saqlanadi (foydalanuvchi
  // so'rovi, 2026-09-18: "suvni tashqarida filtrlab, ichkariga kirganimda ham
  // suv turib qolsa osonlashadi") - qayta tanlash shart emas.
  zkMode="detail";zkSupFilter=sup;zkLastSup=sup;zkQuery="";zkPage=1;zkCatFilter=zkListCatFilter||"";zkSubFilter=zkListSubFilter||"";zkBcFilter=null;zkBcShowAll=false;zkBcStats=null;zkDepth=_zkPickDepthFor(sup);
...
function zkBackToList(){zkMode="list";zkSupFilter="";zkQuery="";zkDetailQuery="";zkPage=1;zkCatFilter="";zkSubFilter="";zkBcFilter=null;zkBcShowAll=false;zkBcStats=null;const inp=document.getElementById("zk-search");if(inp)inp.value="";const x=document.getElementById("zk-search-x");if(x)x.style.display="none";const di=document.getElementById("zk-detail-search");if(di)di.value="";const fp=document.getElementById("zk-fpop");if(fp)fp.classList.remove("open");_zkSaveViewState();
  ...
  if(_pendingBg){const _d=_pendingBg;_pendingBg=null;if(_d.inventory)INVDATA=_d.inventory;if(_d.kirim){P8=(_d.kirimDelta&&P8)?_kirimOverlay(P8,_d.kirim):_kirimApply(_d.kirim);_p8SyncAt=Date.now();}if(P2)_buildZItems();}
...
function _renderZkSupListMerged(supN,supC){
  ...
  // Ro'yxat darajasidagi Kategoriya/Subkategoriya filtri (2026-08-10, keyin
  // 2026-09-18'da TUZATILDI - foydalanuvchi topilmasi: "suv"ni tanlasam ham
  // ZAKAS/CHUQUR/JAMI supplierning BUTUN katalogi bo'yicha ko'rsatilardi, shu
  // sabab qaysi supplierda haqiqatan suv kamayganini bilib bo'lmasdi, ustiga
  // bitta eskirgan/o'lik qatori bor supplier ham ro'yxatga kirib qolardi).
  // Endi filtr faqat SUPPLIERNI saralab qo'ymaydi - shu supplierning qatorlarini
  // ham FAQAT tanlangan kategoriyaga qisqartiradi, ZAKAS/CHUQUR/JAMI o'sha
  // qisqartirilgan qatorlardan QAYTA hisoblanadi. Kategoriyada bironta ham
  // qatori yo'q supplier butunlay ro'yxatdan chiqadi.
  if(zkListCatFilter||zkListSubFilter){
    const _matchCat=r=>(!zkListCatFilter||r.catTop===zkListCatFilter)&&(!zkListSubFilter||r.cat===zkListSubFilter);
    all=all.map(s=>{
      const rN=s._rowsN.filter(_matchCat),rC=s._rowsC.filter(_matchCat);
      const needN=rN.filter(r=>r.orderQty>0).length,needC=rC.filter(r=>r.orderQty>0).length;
      return {...s,_rowsN:rN,_rowsC:rC,needN,needC,total:rN.length+rC.length,needAny:needN+needC};
    }).filter(s=>s.total>0);
  }
  ...
  // "zakasga muhtoj" soni FAQAT Muntazam zakasi bor firmalarni sanaydi (2026-09-24, Bilol) - Muntazam zakas
  // berilgan (0 bo'lgan) firma faqat Chuqur zakas tufayli "muhtoj" bo'lib qolmasin. Ro'yxatning o'zi o'zgarmaydi.
  const supCountEl=document.getElementById("zk-sup-count");
  if(supCountEl)supCountEl.innerHTML=`<div class="zk-sl-stat"><span class="zk-sl-stat-n">${all.length}</span><span class="zk-sl-stat-l">${t("zk_stat_all")}</span></div><div class="zk-sl-stat zk-sl-stat-red"><span class="zk-sl-stat-n">${withNeed.filter(s=>s.needN>0).length}</span><span class="zk-sl-stat-l">${t("zk_stat_need")}</span></div>${_zkMobMoreBtnHtml()}`;
...
async function zkSetSaveOverride(){
  ...
  try{
    ...
    // TUZATISH (2026-08-31, foydalanuvchi xabari): P2/ZITEMS'dagi `ovEffective`
    // ESKI tuzatishdan (yoki umuman bo'lmagan holatdan) hisoblangan holicha
    // qolib ketardi - renderZakas() ZITEMS'ni QAYTA QURMAYDI (faqat mavjud
    // massivni chizadi), shuning uchun jadval keyingi /api/v1/invdata
    // yangilanishigacha (≤15 daqiqa) ESKI sonni ko'rsatib turardi, garchi
    // "Joriy tuzatish" panelida yangi qiymat to'g'ri ko'ringan bo'lsa ham.
    // Yangi tuzatish `updated_at`="hozir" bilan kiritilgani uchun, shu ondagi
    // TO'G'RI jonli qiymat ANIQ `val`ning o'zi (hali hech qanday sotuv/kirim
    // o'tmagan) - shuning uchun ovEffective'ni to'g'ridan-to'g'ri shu bilan
    // yangilaymiz, keyingi jonli yangilanish esa buni tabiiy davom ettiradi.
    if(P2){const _p2it=P2.find(x=>String(x.sku)===sku);if(_p2it)_p2it.ovEffective=val;}
    if(ZITEMS)ZITEMS.forEach(z=>{if(String(z.sku)===sku)z.ovEffective=val;});
    zkSetMsg="✓ Saqlandi";
    renderZakas();
  }catch(e){
...
async function zkSetClearOverride(sku){
  ...
  try{
    ...
    if(!d||!d.ok)throw new Error((d&&d.error)||"Noma'lum xato");
    delete STOCK_OV[sku];
    // Xuddi saqlashdagi kabi - tuzatish o'chirilganda ham P2/ZITEMS'dagi
    // eski ovEffective'ni tozalash kk, aks holda jadval tuzatish o'chirilgan
    // bo'lsa ham eski (endi noto'g'ri) qiymatni ko'rsatib turaveradi.
    if(P2){const _p2it=P2.find(x=>String(x.sku)===sku);if(_p2it)_p2it.ovEffective=null;}
    if(ZITEMS)ZITEMS.forEach(z=>{if(String(z.sku)===sku)z.ovEffective=null;});
    zkSetMsg="";
    renderZakas();
  }catch(e){
    zkSetMsg="Xato: "+(e&&e.message||e);
...
// ─── AVTOMATIK ZAKAS: KATAKCHA BELGILANGANDA DARHOL YUBORISH (2026-09-24, foydalanuvchi so'rovi) ───
// Ta'minotchi belgilanganda shu ta'minotchining "Oxirgi kirim" formulasi bo'yicha hisoblangan
// (butun son/kg, karobka yaxlitlash bilan - saytda ko'ringan raqamning AYNAN o'zi) tovarlari
// Invan'ga qoralama ("New") buyurtma sifatida yuboriladi. Oqim mavjud zkSendToInvan bilan
// bir xil: shaxsiy Invan hisobi, dublikat himoyasi, api/invan-order.js - faqat tugma o'rniga
// katakcha. BIR MARTALIK: belgi turaversa ham o'zi qayta yubormaydi (qayta yuborish uchun
// belgini olib, yana belgilang). "Hammasini tanlash" HECH NARSA YUBORMAYDI (yuzlab firmaga
// birdan real buyurtma ketib qolmasligi uchun) - faqat belgilaydi. Belgilar ro'yxati faqat
// shu brauzerda (localStorage) - Turso/serverga bog'liq emas.
const ZK_AUTO_KEY="zk_auto_enabled",ZK_AUTO_SENT_KEY="zk_auto_sent_local";
let zkAutoOpen=false,zkAutoSaving=false,zkAutoMsg="",zkAutoQuery="";
let zkAutoEnabled=(()=>{try{return new Set(JSON.parse(localStorage.getItem(ZK_AUTO_KEY)||"[]"));}catch(e){return new Set();}})();
let _zkAutoSentList=(()=>{try{return JSON.parse(localStorage.getItem(ZK_AUTO_SENT_KEY)||"[]");}catch(e){return [];}})();
function _zkAutoSaveEnabled(){try{localStorage.setItem(ZK_AUTO_KEY,JSON.stringify([...zkAutoEnabled]));}catch(e){}}
function _zkAutoSaveSent(){try{localStorage.setItem(ZK_AUTO_SENT_KEY,JSON.stringify(_zkAutoSentList));}catch(e){}}
// ─── DOIMIY NAZORAT RO'YXATI (2026-09-24, Bilol) ───
// Firma yonidagi katakcha belgilanganda (va buyurtma muvaffaqiyatli yuborilganda) firma serverdagi
// nazorat ro'yxatiga qo'shiladi (api/zakas-auto-firms.js -> repo'ning `auto-data` branch'i).
// zakas/auto_control.js (GitHub Actions) FAQAT shu ro'yxatni tekshiradi. "Hammasini tanlash"
// ro'yxatga HECH NARSA qo'shmaydi. Server javob bermasa - lokal belgi ishlayveradi, xabarda aytiladi.
let _zkAutoFirmsLoaded=false;
function _zkAutoFirmsEndpoint(){return (location.protocol==="file:"?"https://tiin-market.vercel.app":"")+"/api/zakas-auto-firms";}
async function _zkAutoFirmsCall(action,extra){
  const tok=_authToken();if(!tok)return null;
  try{
    const r=await fetch(_zkAutoFirmsEndpoint(),{method:"POST",headers:{"Content-Type":"application/json"},
      body:JSON.stringify(Object.assign({action,token:tok,by:_zkManagerName()},extra||{}))});
    const j=await r.json();
...
// Kalit yoqilsa zakas/auto_control.js "zakas berishga tushgan" nazoratdagi firmalarga haqiqiy
// buyurtma yaratadi, Open qiladi va SMS yuboradi. Kalitni FAQAT admin o'zgartiradi (server ham
// tekshiradi: api/zakas-auto-firms.js "mode_set"). Kalit va summa chegarasi `auto-data` branch'ida.
let _zkAutoMode={known:false,send:false,max_sum:0};
function _zkIsAdmin(){try{return JSON.parse(localStorage.getItem("tiin_user")||"{}").role==="admin";}catch(e){return false;}}
async function _zkAutoModeLoad(){
  const j=await _zkAutoFirmsCall("mode_get");
  if(!j)return;
  _zkAutoMode={known:true,send:!!j.send,max_sum:Number(j.max_sum)||0};
  try{_zkRenderAutoPanel();}catch(e){}
}
async function zkAutoModeToggle(){
  if(!_zkIsAdmin()||!_zkAutoMode.known)return;
  const want=!_zkAutoMode.send;
  const msg=want
    ?`AVTOMATIK YUBORISH YOQILSINMI?\n\nNazoratdagi firmalar (${zkAutoEnabled.size} ta) har ~3 soatda tekshiriladi. Firmaning tovari tugashga yaqinlashsa (7 kundan kam qolsa) - Invan'da buyurtma OCHILADI va ta'minotchiga SMS yuboriladi. SMS qaytarib bo'lmaydi.\n\nYoqilsinmi?`
    :`Avtomatik yuborish O'CHIRILSINMI?\n\nNazorat faqat hisobot beradi, buyurtma va SMS yuborilmaydi.`;
  if(!confirm(msg))return;
  const j=await _zkAutoFirmsCall("mode_set",{send:want});
  if(!j){alert("Kalitni o'zgartirib bo'lmadi (server javob bermadi yoki sizda admin huquqi yo'q).");return;}
  _zkAutoMode={known:true,send:!!j.send,max_sum:Number(j.max_sum)||0};
  zkAutoMsg=_zkAutoMode.send?"Avtomatik yuborish YOQILDI.":"Avtomatik yuborish o'chirildi (faqat hisobot).";
  _zkRenderAutoPanel();
}
...
function _zkAutoAllSupplierNames(){
  if(!ZITEMS)return[];
  const set=new Set();
  ZITEMS.forEach(v=>set.add(v.sup||ZK_NO_SUPPLIER));
  return[...set].sort((a,b)=>a.localeCompare(b,"ru"));
}
function zkAutoToggle(e){
  if(e)e.stopPropagation();
  zkAutoOpen=!zkAutoOpen;
  _zkRenderAutoPanel();
}
function zkAutoFilterInput(v){zkAutoQuery=(v||"").toLowerCase().trim();_zkRenderAutoPanel();}
// Yuborilgan buyurtma tovarlariga DARHOL "Open" (P8'ga vaqtinchalik "New" kirim yozuvi) -
// haqiqiy kirim ma'lumoti (jonli ma'lumot, ~15 daqiqa) shu buyurtma raqami bilan kelgach,
// vaqtinchalik yozuv o'zi tushib qoladi. P8 qayta yuklanganda ham qayta qo'llanadi.
function _zkAutoReapplySent(){
  if(!_zkAutoSentList.length||!P8||!P8.skus)return;
  const keep=[];
  _zkAutoSentList.forEach(r=>{
    if(Date.now()-new Date(r.ts).getTime()>3*86400000)return;
    const e=P8.skus[r.sku]||(P8.skus[r.sku]={arrivals:[]});
    e.arrivals=e.arrivals||[];
    if(e.arrivals.some(a=>!a._local&&String(a.order_no)===String(r.po)))return;  // haqiqiy yozuv keldi
    keep.push(r);
    if(!e.arrivals.some(a=>a._local&&String(a.order_no)===String(r.po))){
      e.arrivals.push({_local:true,order_no:String(r.po),date:r.ts,status:"New",qty:0,expected:r.qty,supplier:r.sup});
    }
  });
  if(keep.length!==_zkAutoSentList.length){_zkAutoSentList=keep;_zkAutoSaveSent();}
}
// Belgi olib tashlanganda shu ta'minotchining vaqtinchalik "Open" va "Zakas berildi" izlari tozalanadi.
function _zkAutoClearMarks(sup){
  _zkAutoSentList.filter(r=>r.sup===sup).forEach(r=>{
    const e=P8&&P8.skus&&P8.skus[r.sku];
    if(e&&e.arrivals)e.arrivals=e.arrivals.filter(a=>!(a._local&&String(a.order_no)===String(r.po)));
  });
  _zkAutoSentList=_zkAutoSentList.filter(r=>r.sup!==sup);_zkAutoSaveSent();
  try{if(zkIsConfirmed(sup))zkSetConfirmed(sup,false);}catch(e){}
}
...
// ─── BELGISI OLIB TASHLANGAN TOVAR (2026-09-25, Bilol; 2-variant) ───
// Nazoratdagi firma sahifasida qatordan belgi olib tashlansa, shu tovar O'ZI SABABLI avtomat buyurtma chiqarmaydi
// (firmani "zakasga tushdi" qilmaydi), lekin firmaga boshqa tovar sababli buyurtma ketsa u bilan birga ketadi.
// Firmaga buyurtma berilgach belgi tugaydi (zakas/auto_control.js tozalaydi), unutilgan belgi 14 kundan keyin o'zi tugaydi.
// api/zakas-auto-firms.js "exclude" firmaning ro'yxatini ALMASHTIRADI; "exclude_get" sahifa ochilganda qaytaradi.
// UI bloklanmaydi, xato faqat konsolga yoziladi.
let _zkAutoExclTimer=null;
const _zkAutoExclPending=new Set(),_zkAutoExclSent={};   // firma -> oxirgi yuborilgan ro'yxat
...
async function _zkAutoExclFlush(){
  ...
  for(const sup of sups){
    ...
    try{
      ...
      const k=sup,sig=JSON.stringify(skus);
...
// Sahifa ochilganda serverdagi FAOL belgilarni (belgisi olib tashlangan tovarlar) qatorlarga qo'llaydi - belgi olingan tovar
// ertasi kuni ham (buyurtma berilguncha, ko'pi bilan 14 kun) belgisiz ko'rinadi va brauzerning kunlik tozalanishi uni
// yo'qotib yubormaydi (aks holda birinchi katakcha bosilganda server ro'yxati bo'sh holat bilan almashib ketardi).
// zkSaveManual chaqirilmaydi (qayta yuborish sikli bo'lmasin). Sahifa ochilishiga BIR marta.
let _zkAutoExclLoaded=false;
async function _zkAutoExclLoad(){
  if(_zkAutoExclLoaded)return;
  const j=await _zkAutoFirmsCall("exclude_get");
  _zkAutoExclLoaded=true;
  if(!j||!j.firms)return;
  let n=0;
  for(const sup in j.firms)(j.firms[sup]||[]).forEach(sku=>{zkRowChecked["normal:s:"+sku]=false;n++;});
  if(n){try{renderZakas();}catch(e){}}
}
...
// Firma sahifasi sarlavhasida badge + katakcha bilan yuborishda tasdiqlash oynasida ko'rsatiladi.
// Kesh 10 daqiqa (Invan'da raqam keyin qo'shilsa ko'rinishi uchun).
const _zkPhoneCache={};
const _zkPhoneTtl=10*60*1000;
function _zkPhoneGet(sup){const c=_zkPhoneCache[sup];return(c&&(c.state==="loading"||Date.now()-c.at<_zkPhoneTtl))?c:null;}
function _zkPhoneStateHtml(sup){
  const c=_zkPhoneGet(sup);
  const supJ=JSON.stringify(sup).replace(/"/g,'&quot;');
  const b="display:inline-flex;align-items:center;gap:5px;padding:3px 10px;border-radius:12px;font-size:12px;font-weight:600;white-space:nowrap;";
  if(!c||c.state==="error")return `<button type="button" onclick="zkPhoneCheck(${supJ})" style="${b}border:1.5px solid ${c?"#f3c1c1":"#e6e2f7"};background:#fff;color:${c?"#E24B4A":"#534AB7"};cursor:pointer;font-family:inherit">${c?"⚠ Tekshirib bo'lmadi - qayta":"📞 SMS raqamini tekshirish"}</button>`;
  if(c.state==="loading")return `<span style="${b}background:#f4f4f8;color:#888">📞 tekshirilmoqda...</span>`;
  if(c.state==="ok")return `<span style="${b}background:#e8f6f0;color:#1D9E75" title="Invan'dagi raqam - SMS shu raqamga ketadi">📞 SMS: ${esc(c.phones.join(", "))}</span>`;
  if(c.state==="none")return `<span onclick="zkPhoneCheck(${supJ})" style="${b}background:#fdecec;color:#E24B4A;cursor:pointer" title="Invan → Ta'minotchilar → shu firma → telefon raqami qo'shing. Qo'shgan bo'lsangiz - bosing, qayta tekshiradi">📵 Invan'da raqam YO'Q - SMS ketmaydi ↻</span>`;
  return `<span onclick="zkPhoneCheck(${supJ})" style="${b}background:#f4f4f8;color:#888;cursor:pointer" title="Bosing - qayta tekshiradi">❔ Invan'da firma topilmadi ↻</span>`;
}
function _zkPhoneBadgeHtml(sup){
  if(zkMode!=="detail"||sup!==zkSupFilter)return "";
  setTimeout(()=>_zkPhoneEnsure(sup),0);
  return `<span id="zk-phone-badge" style="margin-left:4px">${_zkPhoneStateHtml(sup)}</span>`;
}
function _zkPhonePaint(sup){const el=document.getElementById("zk-phone-badge");if(el&&sup===zkSupFilter)el.innerHTML=_zkPhoneStateHtml(sup);}
async function zkPhonesFor(sup,token){
  const c=_zkPhoneGet(sup);
  if(c&&c.state!=="loading"&&c.state!=="error")return c;
  _zkPhoneCache[sup]={state:"loading",phones:[],at:Date.now()};_zkPhonePaint(sup);
  let res={state:"error",phones:[]};
  try{
    const r=await fetch(_zkInvanEndpoint(),{method:"POST",headers:{"Content-Type":"application/json"},
      body:JSON.stringify({action:"phones",target:"prod",supplier_name:sup,invan_token:token})});
    const j=await r.json();
    if(j.token_expired)_zkInvanClearMy("prod");
    else if(j.ok){
      if(!j.found)res={state:"missing",phones:[]};
      else{const p=(j.phones||[]).filter(Boolean);res={state:p.length?"ok":"none",phones:p};}
    }
  }catch(e){}
  res.at=Date.now();_zkPhoneCache[sup]=res;_zkPhonePaint(sup);
  return res;
}
function _zkPhoneEnsure(sup){
  if(_zkPhoneGet(sup))return;
  const tok=_zkInvanMyToken("prod");
  if(tok)zkPhonesFor(sup,tok);
}
async function zkPhoneCheck(sup){
  const tok=await _zkInvanEnsureToken("prod");
  if(!tok)return;
  delete _zkPhoneCache[sup];
  await zkPhonesFor(sup,tok);
}
// ─── INVAN'DAGI YAQIN BUYURTMALAR (real vaqt) - takroriy zakas ochilib ketmasin (2026-09-26, Bilol) ───
// Sayt jonli ma'lumoti (Open PO) 15-20 daqiqa kechikadi; shu oraliqda (yoki avto/boshqa xodim ochgan buyurtma ustidan)
// shu ta'minotchiga yana buyurtma ochilmasligi uchun tasdiqlash oynasiga ogohlantirish qo'shiladi. Xato bo'lsa - to'sib
// QO'YMAYDI (faqat "tekshirib bo'lmadi" deb yozadi): qaror foydalanuvchida.
const ZK_RECENT_ORDER_MIN=30;
async function _zkRecentOrdersInfo(sup,token){
  const fail={line:"⚠️ Invan'dagi yaqin buyurtmalarni tekshirib bo'lmadi (takroriy zakas xavfi).",found:false,error:true};
  const ctrl=new AbortController();const timer=setTimeout(()=>ctrl.abort(),15000);
  try{
    const r=await fetch(_zkInvanEndpoint(),{method:"POST",headers:{"Content-Type":"application/json"},signal:ctrl.signal,
      body:JSON.stringify({action:"recent",target:"prod",supplier_name:sup,invan_token:token,minutes:ZK_RECENT_ORDER_MIN})});
    const j=await r.json();
    if(j&&j.token_expired){_zkInvanClearMy("prod");return fail;}
    if(!j||!j.ok)return fail;
    const o=Array.isArray(j.orders)?j.orders:[];
    if(!o.length)return{line:"",found:false,error:false};
    const x=o[0];
    return{line:`⛔ DIQQAT: bu ta'minotchiga ${x.mins_ago} daqiqa oldin Invan'da buyurtma ochilgan (№ ${x.no||"—"}, ${x.status||"—"}${o.length>1?`, jami ${o.length} ta`:""}). Sayt uni hali ko'rmagan bo'lishi mumkin — takroriy zakas bo'lib qolmasin.`,found:true,error:false,orders:o};
  }catch(e){return fail;}
  finally{clearTimeout(timer);}
}
async function zkAutoToggleSupplier(sup,checked,el){
  if(zkAutoSaving||_zkInvanSending){if(el)el.checked=!checked;return;}
  const undo=()=>{if(el)el.checked=!checked;};
  if(!checked){
    if(!confirm(`"${sup}" — belgi olib tashlansinmi?\n(Invan'da yaratilgan buyurtmaga tegilmaydi.)`)){undo();return;}
    zkAutoEnabled.delete(sup);_zkAutoSaveEnabled();_zkAutoClearMarks(sup);
    zkAutoMsg="";_zkRenderAutoPanel();try{renderZakas();}catch(e){}
    _zkAutoFirmsSync({[sup]:false}).then(ok=>{if(!ok){zkAutoMsg=`Diqqat: "${sup}" doimiy nazorat ro'yxatidan chiqarilmadi (server javob bermadi) - keyinroq belgini qayta qo'yib olib tashlang.`;try{_zkRenderAutoPanel();}catch(e){}}});
    return;
  }
  // YOQISH: shu ta'minotchining hozirgi zakasi DARHOL Invan'ga yuboriladi.
  if(!ZITEMS||!P8||!P8.skus){alert("Ma'lumot hali to'liq yuklanmadi - biroz kutib qayta urinib ko'ring.");undo();return;}
  // Qatordagi katakchalar hisobga olinadi (2026-09-25, Bilol topilmasi): belgisi OLIB TASHLANGAN tovar yuborilmaydi
  // (avval "zakas kerak" bo'lgan HAMMA tovar yuborilib, olib tashlangan ikki tovarga ham buyurtma ketgan edi).
  const _allRows=_zkAutoRowsFor(sup);
  const rows=_allRows.filter(r=>_zkIsChecked(r));
  const _skipN=_allRows.length-rows.length;
  if(!rows.length){alert(_allRows.length?`"${sup}" — belgilangan tovar yo'q (${_allRows.length} ta tovarning hammasidan belgi olingan). Hech narsa yuborilmadi.`:`"${sup}" — hozir zakas kerak bo'lgan tovar yo'q. Hech narsa yuborilmadi.`);undo();return;}
  const items=rows.map(r=>({sku:String(r.sku),qty:r.orderQty,cost:r.rcost||0,name:r.name,bc:r.bc||[]}));
  const sig=_zkInvanSig(sup,"prod",items);
  if(!_zkInvanCheckDup(sig)){undo();return;}
  const myToken=await _zkInvanEnsureToken("prod");   // shaxsiy Invan hisobi (kesh yoki login oynasi)
  if(!myToken){undo();return;}
  delete _zkPhoneCache[sup];   // yuborishdan oldin keshsiz, ENG YANGI holat (raqam yaqinda qo'shilgan bo'lishi mumkin)
  const _ph=await zkPhonesFor(sup,myToken);
  const _phLine=_ph.state==="ok"?`📞 SMS ketadi: ${_ph.phones.join(", ")}`
    :_ph.state==="none"?"⚠️ DIQQAT: Invan'da bu firmada telefon raqami YO'Q - buyurtma Open bo'ladi, lekin SMS KETMAYDI."
    :_ph.state==="missing"?"⚠️ DIQQAT: firma Invan xaritasida topilmadi."
    :"⚠️ Telefon raqamini tekshirib bo'lmadi - SMS ketishi noaniq.";
  const _rc=await _zkRecentOrdersInfo(sup,myToken);
  const _rcLine=_rc.line?_rc.line+"\n\n":"";
  if(!confirm(_rcLine+`"${sup}" — ${items.length} ta tovar${_skipN?` (belgisi olingan ${_skipN} ta tovar YUBORILMAYDI)`:""} bo'yicha zakas HOZIR Invan'da OCHILADI ("Open") va ta'minotchiga SMS yuboriladi.\n\n`+
      t("zk_invan_sending_as").replace("{name}",_zkInvanMyName("prod")||"")+"\n\n"+_phLine+"\n\nSMS qaytarib bo'lmaydi. Davom etasizmi?")){undo();return;}
  _zkInvanSending=true;zkAutoSaving=true;zkAutoMsg="Yuborilmoqda...";_zkRenderAutoPanel();
  const _mgr=_zkManagerName(),_nd=new Date();
  const _expIso=`${_nd.getFullYear()}-${String(_nd.getMonth()+1).padStart(2,"0")}-${String(_nd.getDate()).padStart(2,"0")}T00:00:00Z`;
  let ok=false;
  try{
    const r=await fetch(_zkInvanEndpoint(),{method:"POST",headers:{"Content-Type":"application/json"},
      body:JSON.stringify({target:"prod",supplier_name:sup,comment:"Avtomatik zakas: "+sup+(_mgr?(" - "+_mgr):""),expected_date:_expIso,items,invan_token:myToken,finalize:true})});
    const j=await r.json();
    if(j.ok){
      ok=true;
      _zkInvanLastSent={sig,at:Date.now(),po:j.po};
      const mapped=new Set((j.mapped||[]).map(m=>String(m.sku)));
      const sent=mapped.size?items.filter(it=>mapped.has(it.sku)):items;
      const ts=new Date().toISOString();
      sent.forEach(it=>_zkAutoSentList.push({sku:it.sku,sup,qty:it.qty,ts,po:String(j.po||("auto-"+Date.now()))}));
      _zkAutoSaveSent();_zkAutoReapplySent();
      zkAutoEnabled.add(sup);_zkAutoSaveEnabled();
      _zkAutoFirmsSync({[sup]:true}).then(ok=>{if(!ok){zkAutoMsg+=" | Diqqat: doimiy nazoratga qo'shib bo'lmadi (server javob bermadi).";try{_zkRenderAutoPanel();}catch(e){}}});
      try{zkSetConfirmed(sup,true);}catch(e){}
      const fz=j.finalize||{};
      const _base=`${sup}: buyurtma ${j.po||"—"}, ${j.added!=null?j.added:sent.length} ta tovar`;
      if(fz.opened&&fz.sms_sent)zkAutoMsg=`✓ ${_base} — Open qilindi, SMS yuborildi (${(fz.phones||[]).join(", ")})`;
      else if(fz.opened)zkAutoMsg=`Xato: ${_base} — Open qilindi, LEKIN ${fz.note||"SMS yuborilmadi"}`;
      else zkAutoMsg=`Xato: ${_base} — yaratildi (New), LEKIN ${fz.note||"Open qilib bo'lmadi"}`;
      if(j.unmapped&&j.unmapped.length)zkInvanShowUnmapped(j.unmapped,"normal");
    }else if(j.token_expired){
      _zkInvanClearMy("prod");
      zkAutoMsg="Xato: "+t("zk_invan_token_expired");
    }else{
      zkAutoMsg="Xato: "+(j.error||t("zk_file_unknown_error"));
      if(j.unmapped&&j.unmapped.length)zkInvanShowUnmapped(j.unmapped,"normal");
    }
  }catch(e){
    zkAutoMsg="Xato: "+t("zk_file_conn_error")+" "+(e&&e.message||e);
  }
  finally{
    _zkInvanSending=false;zkAutoSaving=false;
    if(!ok&&el)el.checked=false;
    _zkRenderAutoPanel();try{renderZakas();}catch(e){}
  }
}
// "Hammasini tanlash" - FAQAT belgilaydi/olib tashlaydi, HECH NARSA YUBORMAYDI (xavfsizlik).
function zkAutoSelectAll(checked,el){
  const all=_zkAutoAllSupplierNames();
  if(zkAutoSaving)return;
  const warnMsg=checked
    ?`BARCHA ${all.length} ta ta'minotchi belgilansinmi?\n\nBu FAQAT belgilaydi - hech narsa YUBORILMAYDI va doimiy nazoratga OLINMAYDI (sahifa yangilansa bu belgilar tushadi). Buyurtma yuborish va nazoratga olish uchun har bir firmani alohida belgilang.`
    :`BARCHA belgilar olib tashlansinmi? (Invan'da yaratilgan buyurtmalarga tegilmaydi.)`;
...
function _zkRenderAutoPanel(){
  const panel=document.getElementById("zk-autosup-panel");if(!panel)return;
  panel.classList.toggle("open",zkAutoOpen);
  const badge=document.getElementById("zk-autosup-badge");
  if(badge){badge.textContent=zkAutoEnabled.size;badge.style.display=zkAutoEnabled.size?"flex":"none";}
  if(!zkAutoOpen)return;
  const all=_zkAutoAllSupplierNames();
  const q=zkAutoQuery;
  const shown=q?all.filter(s=>s.toLowerCase().includes(q)):all;
  const allChecked=all.length>0&&all.every(s=>zkAutoEnabled.has(s));
  panel.innerHTML=`
    <div class="zk-settings-title">Avtomatik zakas — ta'minotchilar</div>
    <div class="zk-settings-hint">Ta'minotchini belgilasangiz - shu zahoti uning "Oxirgi kirim" bo'yicha kerakli tovarlari Invan'da buyurtma qilib OCHILADI (Open) va ta'minotchiga SMS yuboriladi (bir martalik; qayta yuborish uchun belgini olib, yana belgilang). Belgilangan firma DOIMIY NAZORATga olinadi: har ~3 soatda tekshiriladi va tovari tugashga yaqinlashganda (7 kundan kam qolganda) yangi zakas avtomatik yuboriladi - bu FAQAT "Avtomatik yuborish" kaliti yoqilgan bo'lsa. "Hammasini tanlash" faqat belgilaydi, hech narsa yubormaydi.</div>
    <div style="font-size:11.5px;margin:6px 0;padding:6px 8px;border-radius:6px;background:${_zkAutoMode.send?"#FDECEC":"#FFF6E5"};display:flex;align-items:center;gap:8px;flex-wrap:wrap">
      <span>${!_zkAutoMode.known?"Doimiy nazorat rejimi yuklanmoqda…":_zkAutoMode.send
        ?`<b style="color:#E24B4A">● AVTOMATIK YUBORISH YOQILGAN</b> — belgilangan firmalar tovari tugashga yaqinlashsa (7 kundan kam) zakas o'zi Open bo'lib SMS ketadi (har ~3 soatda tekshiriladi)${_zkAutoMode.max_sum?` · summa chegarasi ${Math.round(_zkAutoMode.max_sum).toLocaleString("ru-RU")}`:""}`
        :`<b style="color:#B7791F">● Faqat hisobot</b> — belgilangan firmalar har ~3 soatda tekshiriladi, lekin hech narsa yuborilmaydi`}</span>
      ${_zkAutoMode.known&&_zkIsAdmin()?`<button onclick="zkAutoModeToggle()" style="padding:3px 10px;border-radius:5px;border:1px solid #ccc;background:#fff;cursor:pointer;font-size:11.5px;font-weight:600">${_zkAutoMode.send?"O'chirish":"Yoqish"}</button>`:""}
    </div>
    <div class="zk-set-search"><input type="text" placeholder="Ta'minotchi qidirish..." value="${esc(q)}" oninput="zkAutoFilterInput(this.value)" autocomplete="off"></div>
    <label style="display:flex;align-items:center;gap:7px;padding:7px 2px;font-size:12px;font-weight:700;color:#333;border-bottom:1px solid #f0eef8;margin-bottom:2px;cursor:pointer">
      <input type="checkbox" ${allChecked?"checked":""} ${zkAutoSaving?"disabled":""} onchange="zkAutoSelectAll(this.checked,this)"> Hammasini tanlash (${zkAutoEnabled.size}/${all.length})
    </label>
    <div class="zk-set-ov-list" style="max-height:280px;overflow-y:auto;margin-top:0">
      ${shown.length?shown.map(sup=>{const supJ=JSON.stringify(sup).replace(/"/g,'&quot;');return `<label class="zk-set-ov-row" style="cursor:pointer"><input type="checkbox" ${zkAutoEnabled.has(sup)?"checked":""} ${zkAutoSaving?"disabled":""} onchange="zkAutoToggleSupplier(${supJ},this.checked,this)" style="flex-shrink:0"><span class="zk-set-ov-name" style="cursor:pointer;color:#333">${esc(sup)}</span></label>`;}).join(""):'<div style="padding:10px;text-align:center;color:#bbb;font-size:11.5px">Topilmadi</div>'}
    </div>
    ${zkAutoMsg?`<div style="font-size:11px;margin-top:6px;color:${zkAutoMsg.startsWith("Xato")?"#E24B4A":"#1D9E75"}">${esc(zkAutoMsg)}</div>`:""}
  `;
}
...
// ─── JONLI MA'LUMOT ESKIRGANLIGI OGOHLANTIRISHI (2026-09-26, Bilol) ───
// Jonli ma'lumot (invdata/kirimdata) har 15 daqiqada yangilanadi (raw CDN keshi bilan ~21 daq gacha). Bundan eski bo'lsa
// (GitHub job to'xtagan, tarmoq uzilgan, tab uzoq fonda qolgan) Open PO/kirim/stok eskirgan bo'lib, saytda allaqachon
// buyurtma berilgan tovarga yana zakas ko'rinishi mumkin - Buyurtma sahifasi tepasida sariq ogohlantirish chiqadi.
const ZK_LIVE_STALE_MIN=40;
function _liveDataInfo(){
  if(location.protocol==="file:")return null;
  const m=window.__liveMeta;
  if(!m)return null;
  if(!m.ok)return{failed:true};
  const age=Math.max(0,Math.round((Date.now()-m.publishedAt)/60000));
  return{age,stale:age>ZK_LIVE_STALE_MIN};
}
function _liveBannerText(info){
  if(!info)return "";
  if(info.failed)return "⚠️ Jonli ma'lumot yuklanmadi — sayt eski (saqlangan) ma'lumotni ko'rsatmoqda. Zakas ochishdan oldin Invan'da yaqin buyurtmalarni tekshiring.";
  if(info.stale)return `⚠️ Jonli ma'lumot eskirgan: oxirgi yangilanish ${info.age} daqiqa oldin (odatda har 15 daqiqada). Saytda ko'rinmagan yangi buyurtma/kirim bo'lishi mumkin — zakas ochishdan oldin Invan'da tekshiring.`;
  return "";
}
function _liveBannerPaint(){
  const host=document.getElementById("p7");if(!host)return;
  let el=document.getElementById("zk-live-banner");
  const txt=_liveBannerText(_liveDataInfo());
  if(!txt){if(el)el.style.display="none";return;}
  if(!el){
    el=document.createElement("div");el.id="zk-live-banner";
    el.style.cssText="margin:8px 0;padding:8px 12px;border-radius:8px;background:#FFF6E5;border:1px solid #EF9F27;color:#8a5a00;font-size:13px;font-weight:600;line-height:1.35";
    host.insertBefore(el,host.firstChild);
  }
  el.style.display="";el.textContent=txt;
}
function renderZakas(){
  try{_liveBannerPaint();}catch(e){}
  ...
  _zkInitInvanBtn();
  _zkSyncCalcModeBtns();
  _zkAutoReapplySent();_zkRenderAutoPanel();  // badge sonini panel ochilmasdan oldin ham to'g'ri ko'rsatish uchun
  if(zkMode==="list"){
  ...
  shownSups.forEach(s=>{
    ...
    visRows.forEach((r,i)=>{
      ...
      const dlTxt=(_kv&&!r.zkaStockOk)?(r.zkaDays!=null?Math.max(0,Math.floor(r.zkaDays)):"—"):(r.daysLeft!=null?r.daysLeft:"—");
      ...
      const sl=_kv?(r.zkaStockOk?["dot-normal","Stok yetarli: Invan va hisob qoldig'i "+ZKA_STOCK_GUARD_DAYS+"+ kunlik - zakas berilmaydi"]:(_zkaSl[r.zkaSignal]||null)):(sigLbl[r.signal]||["dot-normal",r.signal||"—"]);
...
// Zakas Excel'ini (ikkala eksport: `_zkExportInvanTemplate` va `zkFileExportXLSX`) yagona
// joyda quradi. Tovarlar Excel JADVALI (ListObject "Zakas") sifatida yoziladi, ИТОГО esa
// jadvalning O'Z jami qatori (2026-09-21, foydalanuvchi so'rovi). Sabab: menejer
// Excel'da Кол-во/Цена o'zgartirsa YOKI ИТОГО'dan oldin/tovarlar orasiga yangi qator
// qo'shsa, Сумма o'zi to'lishi va ИТОГО yangilanishi kerak - oddiy diapazonda Excel
// qator qo'shganda formulani ko'chirmaydi va SUM chegarasi kengaymaydi, jadvalda esa
// ikkalasi ham avtomatik (4 xil qo'shish usuli haqiqiy Excel'da sinalgan).
// items: [{name,bc,qty,price,qtyFmt,priceFmt}]. Qatorlar orasida bo'sh qator YO'Q.
const ZK_TBL="Zakas";
async function _zkBuildOrderBuffer(supName,items){
  const wb=new ExcelJS.Workbook();
  const ws=wb.addWorksheet("Заказ");
  const supRow=ws.addRow([supName||""]);
  ws.mergeCells(1,1,1,5);
  supRow.getCell(1).font={bold:true,size:12};
  supRow.getCell(1).alignment={horizontal:"left",vertical:"middle"};
  wb.calcProperties={fullCalcOnLoad:true};
  const rowF=`${ZK_TBL}[[#This Row],[Кол-во]]*${ZK_TBL}[[#This Row],[Цена]]`;
  let jami=0;
  const tblRows=items.map(i=>{const summa=(i.qty||0)*(i.price||0);jami+=summa;return [i.name,i.bc||"",i.qty,i.price||0,{formula:rowF,result:summa}];});
  ws.addTable({name:ZK_TBL,ref:"A2",headerRow:true,totalsRow:true,
    style:{theme:"TableStyleLight1",showRowStripes:false},
    columns:[
      {name:"Наименование",totalsRowLabel:"",filterButton:false},
      {name:"Штрих код",filterButton:false},
      {name:"Кол-во",filterButton:false},
      {name:"Цена",filterButton:false},
      {name:"Сумма",totalsRowFunction:"sum",totalsRowResult:jami,filterButton:false}],
    rows:tblRows});
  ...
  items.forEach((i,k)=>{
    const r=ws.getRow(3+k);
    r.getCell(3).numFmt=i.qtyFmt||"#,##0.##";
    r.getCell(4).numFmt=i.priceFmt||"#,##0.##";
    r.getCell(5).numFmt="#,##0";
  });
  const tot=ws.getRow(3+items.length);
  tot.getCell(1).value=null;
  tot.getCell(4).value="ИТОГО";
  tot.getCell(4).font={bold:true};tot.getCell(4).alignment={horizontal:"right"};
  tot.getCell(5).font={bold:true};tot.getCell(5).numFmt="#,##0";
  ws.columns=[{width:42},{width:18},{width:10},{width:12},{width:16}];
  return _zkPatchCalcColumn(await wb.xlsx.writeBuffer());
}
// ExcelJS jadval ustunining "hisoblanadigan ustun" (calculatedColumnFormula) belgisini
// yozmaydi - busiz Excel butun-qator qo'shilganda (o'ng tugma -> Insert) Сумма
// formulasini yangi qatorga o'zi qo'ymaydi. Faylni (zip) ochib, table1.xml'dagi
// "Сумма" ustuniga shu belgini qo'shamiz. Muvaffaqiyatsiz bo'lsa (JSZip yuklanmasa) fayl
// o'zgarishsiz qaytariladi - jadval va ИТОГО baribir ishlaydi, faqat qo'shilgan
// qatordagi Сумма'ni qo'lda pastga tortish kerak bo'ladi.
async function _zkPatchCalcColumn(buf){
  try{
    await _ensureJSZip();
    if(typeof JSZip==="undefined")return buf;
    const zip=await JSZip.loadAsync(buf);
    const p="xl/tables/table1.xml";
    if(!zip.files[p])return buf;
    const xml=await zip.files[p].async("string");
    const f=`${ZK_TBL}[[#This Row],[Кол-во]]*${ZK_TBL}[[#This Row],[Цена]]`;
    const out=xml.replace(/<tableColumn ([^>]*name="Сумма"[^>]*)\/>/,`<tableColumn $1><calculatedColumnFormula>${f}</calculatedColumnFormula></tableColumn>`);
    if(out===xml)return buf;
    zip.file(p,out);
    return await zip.generateAsync({type:"uint8array",compression:"DEFLATE"});
  }catch(e){console.warn("zakas Excel: calculatedColumnFormula qo'shilmadi",e);return buf;}
}
...
async function _zkExportInvanTemplate(sups){
  ...
  for(const [sup,rows] of bySup){
    ...
    const buf=await _zkBuildOrderBuffer(sup,rows.map(r=>({
      name:r.name,bc:(r.bc&&r.bc.length)?r.bc[0]:"",qty:r.orderQty,price:r.rcost||0,
      qtyFmt:r.kg?"#,##0.00":"#,##0",priceFmt:Number.isInteger(r.rcost||0)?"#,##0":"#,##0.##"})));
...
if(pid==="p7"){if(!P2||!ZITEMS){await new Promise(r=>requestAnimationFrame(()=>requestAnimationFrame(r)));}const _kirimP=_ensureKirimData();if(!P2){_ensureDailyDemand();let apiData=await _apiBoot();await _ensureP2Data(apiData);await initP2(apiData);}if(!ZITEMS)_buildZItems();await _kirimP;
  ...
  const _zkNeedSecondRender=!_stockOvLoaded||!_zkDraftLoaded||!_zkAutoFirmsLoaded;
  renderZakas();await Promise.all([_ensureStockOverrides(),_ensureZkDraft(),_zkAutoFirmsLoad(),_zkAutoModeLoad(),_zkAutoExclLoad()]);if(_zkNeedSecondRender)renderZakas();_zkDraftStartPoll();}
...
if(pid==="p8"){if(!P8){await _ensureKirimData();}initP8();if(!_p8FullTried)_ensureKirimFull().then(()=>{if(curPageId==="p8")renderP8();});}
if(pid==="p9"){oaInit();}
...
// Backend (history_archive.py: expand_hist_with_archives()) bilan AYNAN BIR
// XIL mantiq - GitHub 100MB chegarasi sababli eski oylar alohida arxiv
// fayllarga chiqarilgan (data_history_archive_YYYY-MM.json). Shu funksiya
// ularni qayta birlashtirib, frontend ham backend kabi TO'LIQ (masalan
// 2026-01-01'dan) tarixni ko'radi (2026-09-18, foydalanuvchi so'rovi - "1
// yanvardan bo'lishi kk, sotuvlar ko'rinmayapti"). Arxiv indeksi yo'q/bo'sh
// yoki bironta fayl yetishmasa/tutashmasa - xavfsiz ravishda mavjud `hist`
// o'zgarishsiz qaytariladi.
const HIST_ARCHIVE_FIELDS=["d","r","rc","wi","we","rt","rr","wri","wre","qf","rf"];
function _addDaysISO(iso,n){
  const [y,m,d]=iso.split("-").map(Number);
  return new Date(Date.UTC(y,m-1,d+n)).toISOString().slice(0,10);
}
async function _expandHistWithArchives(hist){
  // KESH: bu yerda `no-store` ATAYLAB YO'Q - arxivlangan oy fayllari bir marta
  // yozilgach HECH QACHON o'zgarmaydi (history_archive.py), indeks esa oyiga
  // bir marta. Oddiy fetch() brauzer keshini ishlatadi: fayl o'zgarmagan bo'lsa
  // server 304 (tanasiz) javob beradi - ya'ni bir marta yuklangach qayta
  // yuklanmaydi (foydalanuvchi so'rovi, 2026-09-19).
  let idx;
  try{
    const r=await fetch("data_history_archive_index.json");
    if(!r.ok)return hist;
    idx=await r.json();
  }catch(e){return hist;}
  const months=((idx&&idx.months)||[]).slice().sort();
  if(!months.length)return hist;
  let base=hist.base,days=hist.days||0;
  const maps={};
  for(const f of HIST_ARCHIVE_FIELDS)maps[f]=Object.assign({},hist[f]||{});
  // Oy fayllari PARALLEL olinadi (ketma-ket kutmaslik uchun), keyin eng
  // yaqinidan uzoqqa qarab tartib bilan qayta ishlanadi.
  const arcs=await Promise.all(months.map(mk=>
    fetch("data_history_archive_"+mk+".json").then(r=>r.ok?r.json():null).catch(()=>null)));
  for(let i=months.length-1;i>=0;i--){
    const arc=arcs[i];
    if(!arc)break;
    if(_addDaysISO(arc.base,arc.days)!==base)break;  // tutashmaydi - to'xtaymiz
    for(const f of HIST_ARCHIVE_FIELDS){
      const arcF=arc[f]||{},curF=maps[f];
      const merged={};
      for(const key of new Set([...Object.keys(curF),...Object.keys(arcF)])){
        const arcArr=arcF[key]||new Array(arc.days).fill(0);
        const curArr=curF[key]||new Array(days).fill(0);
        merged[key]=arcArr.concat(curArr);
      }
      maps[f]=merged;
    }
    base=arc.base;days+=arc.days;
  }
  return Object.assign({},hist,{base,days},maps);
}
function _histSetData(data){
...
// Arxivni MAVJUD HIST ustiga (bir marta) qo'shadi. Bazaviy sana orqaga surilgani
// uchun saqlangan indekslar (p2HistCustom) ham shuncha oldinga suriladi.
let _histArcPromise=null,_histArcDone=false;
function _ensureHistArchives(){
  if(_histArcDone||!HIST||!HISTMETA)return Promise.resolve(false);
  if(_histArcPromise)return _histArcPromise;
  _histArcPromise=(async()=>{
    const cur={base:HISTMETA.base,days:HISTMETA.days};
    for(const f of HIST_ARCHIVE_FIELDS)cur[f]=HIST[f]||{};
    const out=await _expandHistWithArchives(cur);
    const delta=out.days-HISTMETA.days;
    _histArcPromise=null;
    _histArcDone=true;                  // muvaffaqiyatli yoki yo'q - sahifa yangilanmaguncha qayta urinilmaydi (cheksiz sikl bo'lmasin)
    if(delta<=0)return false;           // arxiv yo'q/olinmadi
    _histSetData(out);
    if(p2HistCustom)p2HistCustom={from:p2HistCustom.from+delta,to:p2HistCustom.to+delta};
    _histSyncInputs();
    return true;
  })();
  return _histArcPromise;
}
function loadHistory(){
  ...
  _histLoadPromise=(async()=>{
    try{
      ...
      _histSetData(data);
      // Arxivlangan (eski) oylar HAR DOIM qo'shiladi - sayt 1-yanvardan boshlab
      // to'liq ko'rinsin (foydalanuvchi so'rovi, 2026-09-19). Arxiv fayllari
      // o'zgarmaydi, shuning uchun brauzer keshidan olinadi (qayta yuklanmaydi);
      // tarix o'zi ham faqat kerak bo'lganda (60 kundan uzoq ko'rinish) yuklanadi.
      await _ensureHistArchives();
...
function _histApplyRange(fromISO,toISO){
  ...
  if(fromISO<HISTMETA.base&&!_histArcDone){
    _ensureHistArchives().then(()=>_histApplyRange(fromISO,toISO));
    return;
  }
...
function setHistDays(d){
  ...
  if(HISTMETA&&!_histArcDone&&(d==='full'||+d>HISTMETA.days)){
    _ensureHistArchives().then(()=>setHistDays(d));
    return;
  }
  _histSyncInputs();
...
// KIRIM ma'lumoti IKKI qatlamli (2026-09-19, foydalanuvchi: "eskilari statik
// qolgani uchun turin, har yuklanganda kamroq yuklasin"):
//  1) JONLI qatlam (/api/v1/kirimdata, ~2.7MB) - Invan'dan so'nggi ~120 kun.
//     Har doim shu olinadi, har 15 daqiqada yangilanadi (_bgSilentRefresh).
//  2) STATIK qatlam (data_kirim.json, ~10MB) - TO'LIQ tarix. FAQAT kerak
//     bo'lganda (mahsulot kartochkasi / Kirim bo'limi) va KESHLANADIGAN qilib
//     (`no-store` YO'Q: fayl o'zgarmasa server 304 beradi, ya'ni faqat pipeline
//     yangilaganda - kuniga 2 marta - qayta yuklanadi) olinadi.
// Ikkalasi _mergeKirim() bilan birlashadi - shuning uchun 120 kundan oldingi
// tarix (masalan SKU 34552: 2026-03-10 dagi oxirgi kirim) "topilmadi" bo'lmaydi.
let _p8Static=null,_p8FullPromise=null,_p8FullTried=false,_p8SyncAt=0;
const KIRIM_DELTA_DAYS=21;   // Open->Received kechikishi haqiqiy ma'lumotda maks. ~19 kun (p99 ~7): 21 kun hech narsani o'tkazib yubormaydi
// SKU agregatlarini (backend_p8_kirim._finalize_sku bilan AYNAN bir xil mantiq)
// qatorlardan qayta hisoblaydi: (order_id,item_id) bo'yicha takrorlarni (eng katta
// qty) bitta qiladi, sanasi bo'yicha kamayish tartibida saralaydi.
// backend_shared_utils.rq bilan bir xil: butunga yaqin bo'lsa butun, aks holda 3 xona.
function _rq3(v){v=Number(v);return Math.abs(v-Math.round(v))<0.001?Math.round(v):Math.round(v*1000)/1000;}
function _kirimFinalize(arrivals){
  const best=new Map(),noItem=[];
  for(const a of arrivals){
    if(!a.item_id){noItem.push(a);continue;}
    const k=a.order_id+"|"+a.item_id,c=best.get(k);
    if(!c||(a.qty||0)>(c.qty||0))best.set(k,a);
  }
  const rows=[...best.values(),...noItem];
  rows.sort((x,y)=>{const dx=x.date||"",dy=y.date||"";return dx<dy?1:(dx>dy?-1:0);});
  const L=rows[0];
  return {name:L.name,barcode:L.barcode,total_received:_rq3(rows.reduce((t,a)=>t+(a.qty||0),0)),
    last_qty:L.qty,last_status:L.status,arrival_count:rows.length,last_date:L.date,last_cost:L.cost,arrivals:rows};
}
// `newer` (jonli/delta) qatorlari base'dagi SHU buyurtmaning (order_id) barcha
// qatorlarini almashtiradi (backend merge_kirimdata semantikasi: status
// Open->Received o'zgarsa dublikat bo'lmaydi, ko'p qatorli buyurtma yo'qolmaydi).
function _kirimOverlay(base,newer){
  const out={...base,skus:{...(base.skus||{})}};
  for(const sku of Object.keys(newer.skus||{})){
    const nRows=(newer.skus[sku].arrivals)||[];
    if(!nRows.length)continue;
    const ent=out.skus[sku]||{};
    const ids=new Set(nRows.map(a=>a.order_id));
    const kept=(ent.arrivals||[]).filter(a=>!ids.has(a.order_id));
    out.skus[sku]=Object.assign({},ent,_kirimFinalize(kept.concat(nRows)));
  }
  return out;
}
function _mergeKirim(stat,live){
  // Statikdan FAQAT tugallangan (Open/New bo'lmagan) yozuvlar olinadi: ochiq
  // buyurtma holati faqat jonli qatlamdan (Zakas "ochiq buyurtma" mantig'i
  // o'zgarmasligi uchun - u avval ham faqat jonli manbaga tayangan).
  const base={...stat,skus:{}};
  for(const sku of Object.keys(stat.skus||{})){
    const rows=(stat.skus[sku].arrivals||[]).filter(a=>a.status!=="Open"&&a.status!=="New");
    if(rows.length)base.skus[sku]=Object.assign({},stat.skus[sku],_kirimFinalize(rows));
  }
  return _kirimOverlay(base,live);
}
// Yangi jonli ma'lumotni qo'llaydi; statik qatlam allaqachon yuklangan bo'lsa
// qayta birlashtiradi (15 daqiqalik yangilanish eski tarixni o'chirib yubormasin).
function _kirimApply(live){return _p8Static?_mergeKirim(_p8Static,live):live;}
async function _ensureKirimData(){
  if(P8)return P8;
  if(window.TiinDataAPI){
    try{P8=_kirimApply(await window.TiinDataAPI.kirimdata());window._p8FromApi=true;_p8SyncAt=Date.now();return P8;}catch(e){/* pastga, statik zaxiraga */}
  }
  try{const _r=await fetch("data_kirim.json");P8=await _r.json();_p8Static=P8;window._p8FromApi=false;}catch(e){P8={skus:{}};window._p8FromApi=false;}
  return P8;
}
// To'liq (statik) tarixni bir marta qo'shadi. Chaqiruvchi tugagach o'zi qayta chizadi.
function _ensureKirimFull(){
  if(_p8Static||window._p8FromApi===false)return Promise.resolve(P8);   // allaqachon to'liq
  if(_p8FullPromise)return _p8FullPromise;
  _p8FullTried=true;
  _p8FullPromise=(async()=>{
    try{
      const r=await fetch("data_kirim.json");
      if(!r.ok)throw new Error("HTTP "+r.status);
      _p8Static=await r.json();
      if(P8)P8=_mergeKirim(_p8Static,P8);
    }catch(e){console.warn("data_kirim.json (to'liq tarix) yuklanmadi:",e);}
    _p8FullPromise=null;
    return P8;
  })();
  return _p8FullPromise;
}
...
async function _enrichWithInventory(arr,apiData){
  ...
  arr.forEach((v,i)=>{
    ...
    if(iv){
      ...
      // KRITIK: v.ovEffective=iv.ovEffective qatori (oxirida) - qo'lda tuzatilgan
      // stokning JONLI (sotuv/kirimga qarab) qiymati (backend/app.py::_live_invdata()
      // hisoblaydi). Bu qator o'chirilsa/tashlab ketilsa - qo'lda kiritilgan son
      // butun sessiya davomida QOTIB QOLGANDEK ko'rinadi (2026-08-19'da aynan shu
      // bug bo'lgan edi, ZITEMS'ga ko'chirilmagani uchun). Tekshirish uchun:
      // backend/verify_override_adjustment.py.
      v.iprice=iv.p;v.suprice=iv.sp;v.amt=iv.a;v.itype=iv.t;v.sub=iv.sb;v.sup=iv.su;v.lsd=iv.lsd||null;v.ld60=iv.ld60||null;v.pav=iv.pav||null;v.pavm=iv.pavm||null;v.avg30sa=iv.avg30sa||null;v.ck=iv.ck!=null?iv.ck:null;v.la=iv.la||null;v.bc=iv.bc||[];v.rcost=iv.rcost||0;v.rcostApprox=!!iv.rcost_approx;v.calcStock=iv.calcStock!=null?iv.calcStock:null;v.calcConf=iv.calcConf||null;v.calcEvidence=iv.calcEvidence!=null?iv.calcEvidence:null;v.calcAnchor=iv.calcAnchor||null;v.calcRule=iv.calcRule||null;v.lkQty=iv.lkQty!=null?iv.lkQty:null;v.lkSold=iv.lkSold!=null?iv.lkSold:null;v.lkDate=iv.lkDate||null;v.zabc=iv.zabc||"";v.ovEffective=iv.ovEffective!=null?iv.ovEffective:null;
    }
  });
}
// ─── P2: MAHSULOTLAR ───
...
function renderP2(idx){
  ...
  if(P8&&!_p8FullTried&&!_p8Static&&window._p8FromApi!==false){
    // To'liq (statik) kirim tarixi - faqat mahsulot kartochkasi ochilganda, bir marta (keshlanadi).
    _ensureKirimFull().then(()=>{if(window.p2ActiveIndex===idx)renderP2(idx);});
  }
...
async function _applyBgData(d){
  ...
  if(d.kirim){P8=(d.kirimDelta&&P8)?_kirimOverlay(P8,d.kirim):_kirimApply(d.kirim);_p8SyncAt=Date.now();}
  if(P2){
...
let _bgLastRefreshAt=0;
const _BG_REFRESH_MS=15*60*1000;
async function _bgSilentRefresh(){
  if(!window.TiinDataAPI||location.protocol==="file:"||_bgSilentBusy)return;
  // 2026-09-23 (Vercel "Fast Origin Transfer" limiti to'lib qolgani sababli
  // topildi): bu so'rov FONDA - foydalanuvchi boshqa tab/oynada bo'lsa ham -
  // har 15 daqiqada davom etardi. Ko'p qurilma/tab kun bo'yi ochiq (lekin
  // ko'rinmas) tursa, shu narsa oyiga 10GB'lik limitni bir necha kunda
  // tugatib qo'ygan. Endi FAQAT tab ko'rinib turganda ishlaydi.
  ...
  try{
    ...
    // 2026-09-23: kirimdata endi jsdelivr'dagi static fayldan (doim TO'LIQ)
    // olinadi - Vercel'ga kirmagani uchun delta-optimallashtirish shart
    // emas (_kirimApply, to'liq almashtirish yo'li ishlatiladi).
...
if(location.protocol!=="file:"){
  setInterval(_bgSilentRefresh,_BG_REFRESH_MS);
  setInterval(function(){if(curPageId==="p7"){try{_liveBannerPaint();}catch(e){}}},60000);   // sahifa ochiq turganda ham eskirish o'z vaqtida chiqsin
  // Tab fonda uzoq turib qaytib ko'ringanda - agar oxirgi yangilanishdan
  // beri interval muddati allaqachon o'tgan bo'lsa, darhol yangilaydi
  // (aks holda foydalanuvchi eskirgan ma'lumotni keyingi setInterval
  // tikigacha, ya'ni yana 15 daqiqagacha ko'rishi mumkin edi).
  document.addEventListener("visibilitychange",function(){
    if(document.visibilityState==="visible"&&Date.now()-_bgLastRefreshAt>_BG_REFRESH_MS)_bgSilentRefresh();
  });
}
...
if(typeof module!=="undefined"&&module.exports){
  module.exports={
    ...
    _zkAutoRowsFor,_zkAutoAllRowsMap,ZKA_MUST_ORDER_DAYS,ZKA_CAN_ORDER_DAYS,
    ZK_DEFAULT_TARGET,ZK_BUFFER,ZK_NO_SUPPLIER,ZK_MIN_ORDER,
    getZItems:()=>ZITEMS,
    getZkSupTargets:()=>zkSupTargets,
    setP8:(v)=>{P8=v;},
    _liveDataInfo,_liveBannerText,_liveBannerPaint,_zkRecentOrdersInfo,ZK_LIVE_STALE_MIN,
  };
}
