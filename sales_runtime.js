...
async function zkAutoToggleSupplier(sup,checked,el){
  ...
  const rows=_zkAutoRowsFor(sup);
  if(!rows.length){alert(`"${sup}" — hozir zakas kerak bo'lgan tovar yo'q. Hech narsa yuborilmadi.`);undo();return;}
  const items=rows.map(r=>({sku:String(r.sku),qty:r.orderQty,cost:r.rcost||0,name:r.name,bc:r.bc||[]}));
  ...
  if(!confirm(`"${sup}" — ${items.length} ta tovar bo'yicha zakas HOZIR Invan'da OCHILADI ("Open") va ta'minotchiga SMS yuboriladi.\n\n`+
