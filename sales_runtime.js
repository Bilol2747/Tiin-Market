...
const _ALL_TABS=["p1","p2","p3","p5","p7","p6","p_nazorat"];
...
(function(){
  try{
    ...
    if(us){const user=JSON.parse(us);const s=document.getElementById("login-screen");if(s)s.remove();document.body.classList.remove("locked");_applyUser(user);}
    else{const p=document.getElementById("lg-phone");if(p)p.focus();}
...
function zkSetConfirmed(sup,val){zkConfirmed[sup]=val;if(!val)delete zkConfirmed[sup];zkSaveConfirmed();}
...
function zkAutoRemoveConfirmed(allSups){
  allSups.forEach(s=>{
    if(zkIsConfirmed(s.sup)){
      const needCount=s.rows.filter(r=>r.orderQty>0).length;
      if(needCount===0){zkSetConfirmed(s.sup,false);}
    }
  });
}
...
function _zkBuildSuppliers(){
  ...
  const out=supNames.map(sup=>{
    ...
    const rows=bySup[sup].map(v=>{
      ...
      let orderQty=stock<0?v.dailyAvg*adj:v.dailyAvg*zakasDays;
...
if(pid==="p6"){if(!P2){P2=JSON.parse(document.getElementById("p2data").textContent);initP2(null);}if(!ZITEMS&&P2){_buildZItems();}if(!P6){P6=JSON.parse(document.getElementById("supplierdata").textContent);initP6();}setTimeout(_spFitTableHeight,0);}
...
function p6OpenSupplierDetail(r){
  ...
  const rows=_p6DetailProds.map((p,i)=>{
    ...
    const skuLine=p.sku?`<span style="font-size:9px;color:#ccc;font-weight:500;display:block">${esc(p.sku)}</span>`:"";
    return `<tr class="sp6-prod-row"><td style="color:#bbb;font-size:10px;text-align:center;width:32px;padding:5px 6px">${i+1}</td><td style="padding:5px 10px;max-width:280px;overflow:hidden"><span class="sp6-prod-link" onclick="p6GoToProduct(${i})" title="${esc(p.name)}">${esc(p.name)}</span>${skuLine}</td>${abcCells}</tr>`;
  }).join("");
...
function _p6EnsureDetailStyles(){
  ...
  st.textContent=`#sp-fullscreen,#sp-mz-page{position:fixed!important;top:0;bottom:0;left:195px;right:0;background:#fff;box-sizing:border-box;transition:left .18s ease}body.sb-collapsed #sp-fullscreen,body.sb-collapsed #sp-mz-page{left:64px}#sp-fullscreen{overflow-y:auto;overflow-x:auto;z-index:1500}#sp-mz-page{overflow-y:auto;overflow-x:hidden;padding:0 24px 60px;z-index:1600}#sp6-matrix-wrap{padding:0 14px 40px}.sp6-sup-row{cursor:pointer}.sp6-sup-row:hover .sp6-sup-link{color:#1D9E75;text-decoration:underline}.sp6-sup-link{font-weight:600;transition:color .15s}.sp6-prod-link{cursor:pointer;color:#1a1a2e;font-weight:600;font-size:11px;line-height:1.3;display:block;white-space:nowrap}.sp6-prod-link:hover{text-decoration:underline;color:#1D9E75}.sp6-prod-row:hover td{background:#f0faf6!important}.sp6-matrix{font-size:12px;width:auto;min-width:100%;border-collapse:collapse}.sp6-matrix th{font-size:10px;font-weight:800;text-transform:uppercase;letter-spacing:.3px;color:#888;border-bottom:1.5px solid #eee;padding:6px 6px;white-space:nowrap;position:sticky;top:0;z-index:1;background:#fafaf5}.sp6-matrix td{border-bottom:1px solid #f0f0ec;vertical-align:middle}.sp6-matrix tbody tr:hover td{background:#f0faf6!important}`;
...
function _p6ShowOverlay(name,detH,monthName,abc,mzCount,mzPageH){
  ...
  <div class="sp-det-wrap">${detH}</div>`;
  if(mzCount&&mzPageH){
    mzPage.innerHTML=`<div style="position:sticky;top:0;background:#fff;padding:14px 0 12px;border-bottom:1.5px solid #f0f0ec;margin-bottom:20px;z-index:2;display:flex;align-items:center;gap:12px">
...
// ─── Nazorat bo'limi ───
const NAZ_TABS=[{id:"p1",label:"Bosh sahifa"},{id:"p2",label:"Mahsulotlar"},{id:"p3",label:"ABC tahlili"},{id:"p5",label:"Stock"},{id:"p7",label:"Buyurtma"},{id:"p6",label:"Suppliers"}];
...
function _nazRender(){
  ...
  const t=document.getElementById("naz-total"),a=document.getElementById("naz-admins"),ac=document.getElementById("naz-active");
  if(t)t.textContent=total;if(a)a.textContent=admins;if(ac)ac.textContent=active;
...
function nazShowAdd(){
  _nazEditing=null;
  document.getElementById("naz-modal-title").textContent="Yangi xodim qo'shish";
  document.getElementById("naz-form").reset();
  document.getElementById("naz-pass-hint").textContent="";
  NAZ_TABS.forEach(t=>{const cb=document.getElementById("naz-tab-"+t.id);if(cb)cb.checked=t.id==="p1";});
...
function nazEdit(id){
  ...
  _nazEditing=id;
  document.getElementById("naz-modal-title").textContent="Xodimni tahrirlash";
  document.getElementById("naz-name").value=u.name||"";
...
async function nazSave(e){
  e.preventDefault();
  const name=(document.getElementById("naz-name").value||"").trim();
  ...
  const tabs=NAZ_TABS.filter(t=>{const cb=document.getElementById("naz-tab-"+t.id);return cb&&cb.checked;}).map(t=>t.id);
