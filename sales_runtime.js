...
async function _bgSilentRefresh(){
  ...
  try{
    ...
    // Kirim: bir marta to'liq olingan bo'lsa, endi FAQAT so'nggi KIRIM_DELTA_DAYS
    // kunlik yozuvlar so'raladi va mavjud ma'lumot ustiga order_id bo'yicha yoziladi
    // (~2.7MB -> ~0.5MB). Baza yo'q/juda eski (15+ kun tab ochiq turgan) bo'lsa - to'liq.
    const asDelta=!!P8&&window._p8FromApi!==false&&(Date.now()-_p8SyncAt)<15*86400000;
    const kirim=await window.TiinDataAPI.kirimdata(asDelta?KIRIM_DELTA_DAYS:0);
    d={inventory:inv,kirim:kirim||null,kirimDelta:asDelta};
