...
const ZKA_MUST_ORDER_DAYS=7,ZKA_CAN_ORDER_DAYS=14,ZKA_REF_MIN_DAYS=15,ZKA_REF_MAX_DAYS=60,ZKA_REF_RESET_DAYS=30;
...
const ZKA_LAST_KIRIM_MIN_DAYS=5,ZKA_LAST_KIRIM_MAX_DAYS=30;
...
async function zkAutoToggleSupplier(sup,checked,el){
  ...
  if(!confirm(`"${sup}" — ${items.length} ta tovar${_skipN?` (belgisi olingan ${_skipN} ta tovar YUBORILMAYDI)`:""} bo'yicha zakas HOZIR Invan'da OCHILADI ("Open") va ta'minotchiga SMS yuboriladi.\n\n`+
...
if(pid==="p7"){if(!P2||!ZITEMS){await new Promise(r=>requestAnimationFrame(()=>requestAnimationFrame(r)));}const _kirimP=_ensureKirimData();if(!P2){_ensureDailyDemand();let apiData=await _apiBoot();await _ensureP2Data(apiData);await initP2(apiData);}if(!ZITEMS)_buildZItems();await _kirimP;
  ...
  renderZakas();await Promise.all([_ensureStockOverrides(),_ensureZkDraft(),_zkAutoFirmsLoad(),_zkAutoModeLoad()]);if(_zkNeedSecondRender)renderZakas();_zkDraftStartPoll();}
