...
def build(records, business_client_ids=None):
    """business_client_ids: Invan `client.id` to'plami - Firmalar (p11, data_firmalar.json,
    client_type=Business) ro'yxatidagi RASMIY xaridor firmalar. Berilsa, har chekning
    `client_id`si shu to'plamda bo'lsa - "firma xaridi" deb belgilanadi (rev_d_firma/q_firma,
    pastda) - bu is_explicit_wholesale() (nom/TIN so'z-ohorati) dan MUSTAQIL, aniqroq signal,
    fetch_clients.py'dagi bilan bir xil `client.id` maydonidan (o'sha yerda ham DEBT
    cheklarini shu tarzda firmaga bog'laydi)."""
    business_client_ids = business_client_ids or set()
    receipts = {}
    ...
    for rec in records:
        ...
        if rec.get("client_id"):
            receipt["client_id"] = rec["client_id"]
    ...
    for receipt in receipts.values():
        day_index = (receipt["date"] - min_date).days
        explicit = is_explicit_wholesale(receipt["customer"], receipt["tin"])
        is_firma = receipt["client_id"] in business_client_ids if receipt["client_id"] else False
        customer_key = receipt["tin"] or normalize(receipt["customer"])
        for product_key, qty in receipt["items"].items():
            ...
            # Firmalar (p11) ro'yxatidagi RASMIY biznes-mijoz xaridi - qty<=0 (refund)
            # tekshiruvidan OLDIN, rev_d/revenue kabi - firma qaytargan tovar ham
            # to'g'ri hisobga olinishi uchun (Kategoriyalar "faqat chakana" rejimi).
            if is_firma:
                item["rev_d_firma"][day_index] += item_revenue
                item["q_firma"][day_index] += qty
            if qty <= 0:
                continue
            item["r"][day_index] += 1
            if explicit:
    ...
    for product_key, item in item_data.items():
        ...
        output_items[product_key] = {
            ...
            "revF": [round(value) for value in item["rev_d_firma"]],
            "qF": [round_quantity(value) for value in item["q_firma"]],
