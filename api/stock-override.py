...
# 2026-09-23: Vercel "Blob Advanced Operations" kvotasi (2000/oy, Hobby)
# to'lib qolgani sababli topildi - LIST har GET'da chaqirilardi (frontend
# har Zakas/Stock ochilganda GET qiladi -> ju'da tez-tez). PATHNAME va
# x-add-random-suffix=0 tufayli blobning URL manzili DOIM bir xil (fayl
# o'chirilib qayta yaratilmasa) - shuning uchun bir marta topilgach,
# funksiya "issiq" turgan davrda (bir nechta so'rov orasida) qayta
# so'ralmaydi. LIST endi faqat sovuq boshlanishda (yoki hali blob umuman
# yaratilmagan bo'lsa) chaqiriladi.
...
def _find_blob_url(token):
    """Keshdagi URL bo'lsa shuni qaytaradi - aks holda LIST API orqali
    `PATHNAME` bilan mos blobni topadi (topilmasa None - hali birorta
    tuzatish saqlanmagan degani, xatolik emas)."""
...
def _put_overrides(overrides, token):
    ...
    # PUT javobi ham blobning URL'ini qaytaradi - keshni shu yerdan ham
    # to'ldiramiz, keyingi GET LIST'ga umuman muhtoj bo'lmasin.
    global _cached_blob_url
    try:
        u = resp.json().get("url")
        if u:
