...
// ZAKAS OCHILGACH (Open) Telegram "Tiin Analitika" guruhiga SENING nomingdan PDF + xabar (2026-09-25, Bilol): GitHub Actions'dagi
// telegram_notify.yml ishga tushiriladi (Telegram sessiyasi GitHub secret'da; Vercel'da Python/Telethon yo'q). Faqat buyurtma raqami
// va SMS holati uzatiladi (telefon raqami loglarga tushmasin - repo ochiq). Xato buyurtmani BEKOR QILMAYDI. TG_NOTIFY=off - o'chirish.
async function dispatchTelegramNotify(po, finalize) {
  const pat = process.env.GITHUB_PAT;
  if (!pat || process.env.TG_NOTIFY === "off") return { dispatched: false, reason: "o'chiq" };
  const sms = finalize.sms_sent ? "sent" : (/telefon raqami yo'q/.test(finalize.note || "") ? "nophone" : "failed");
  const ctrl = new AbortController();
  const t = setTimeout(() => ctrl.abort(), 8000);
  try {
    const r = await fetch("https://api.github.com/repos/Bilol2747/Tiin-Market/actions/workflows/telegram_notify.yml/dispatches", {
      method: "POST", signal: ctrl.signal,
      headers: { Authorization: "Bearer " + pat, Accept: "application/vnd.github+json", "Content-Type": "application/json" },
      body: JSON.stringify({ ref: "main", inputs: { order_no: String(po), sms } }),
    });
    return { dispatched: r.ok, status: r.status };
  } catch (e) {
    return { dispatched: false, reason: String(e && e.message || e) };
...
module.exports = async function handler(req, res) {
  ...
  try {
    ...
    // Firma telefon raqamlari (SMS ketadimi?) - yuborishdan OLDIN ko'rsatish uchun (2026-09-24, Bilol).
    // FAQAT shaxsiy Invan tokeni bilan (login qilgan xodim, Invan ruxsatlari bilan tekshiriladi) -
    // endpoint ochiq bo'lgani uchun umumiy token bilan telefonlarni qaytarmaymiz.
    if (body.action === "phones") {
      if (env !== "prod" || !body.invan_token) { res.status(400).json({ ok: false, error: "shaxsiy Invan tokeni kerak" }); return; }
      const sid = body.supplier_id || SUPPLIER_ID_MAP[String(body.supplier_name || "").trim()];
      if (!sid) { res.status(200).json({ ok: true, found: false, phones: [] }); return; }
      const sup = await invan(cfg.base, `/api/v1/supplier/${sid}`, { method: "GET" }, String(body.invan_token));
      const d = (sup && sup.data) || sup || {};
      const phones = (Array.isArray(d.phone_number) ? d.phone_number : []).map(p => String(p || "").trim()).filter(Boolean);
      res.status(200).json({ ok: true, found: true, phones });
      return;
    }

    // YAQIN BUYURTMALAR (real vaqt) - 2026-09-26, Bilol: sayt jonli ma'lumoti (Open PO) 15-20 daqiqa kechikadi, shu oraliqda
    // (yoki avto/boshqa xodim ochgan buyurtma ustidan) takroriy zakas ochilib ketmasin. Katakcha bosilganda tasdiqlash
    // oynasida ko'rsatiladi. FAQAT O'QISH, faqat shaxsiy token bilan (Invan ruxsatlari tekshiradi). Qaytarish/bekor turlari hisobga olinmaydi.
    if (body.action === "recent") {
      if (env !== "prod" || !body.invan_token) { res.status(400).json({ ok: false, error: "shaxsiy Invan tokeni kerak" }); return; }
      const sid = body.supplier_id || SUPPLIER_ID_MAP[String(body.supplier_name || "").trim()];
      if (!sid) { res.status(200).json({ ok: true, found: false, orders: [] }); return; }
      const minutes = Math.min(240, Math.max(1, Number(body.minutes) || 30));
      const ctrl = new AbortController();
      const timer = setTimeout(() => ctrl.abort(), 12000);
      let list;
      try {
        list = await invan(cfg.base, "/api/v1/supplier_order?page=1&limit=300",
          { method: "POST", body: JSON.stringify({ filters: [] }), signal: ctrl.signal }, String(body.invan_token));
      } finally { clearTimeout(timer); }
      const rows = Array.isArray(list && list.data) ? list.data : null;
      if (!rows) { res.status(502).json({ ok: false, error: "Invan buyurtmalar ro'yxati kutilgan shaklda emas" }); return; }
      const now = Date.now(), orders = [];
      for (const o of rows) {
        if (!o || !o.supplier || o.supplier.id !== sid) continue;
        const t = Date.parse(o.created_at);
        if (!(t > 0)) continue;
        const mins = (now - t) / 60000;
        if (mins > minutes) continue;
        const st = (o.status && o.status.name) || "";
        if (/return|cancel/i.test(st)) continue;
        orders.push({ no: o.external_id || null, status: st, created_at: o.created_at, mins_ago: Math.max(0, Math.round(mins)), total: o.total_amount == null ? null : o.total_amount });
      }
      res.status(200).json({ ok: true, found: true, orders });
      return;
    }
    ...
    // 4) IXTIYORIY: "Open"ga o'tkazish + ta'minotchiga SMS (faqat prod va body.finalize).
    // Avtomatik zakas katakchasi so'raydi (2026-09-24, Bilol). Qadamlar zakas/order_finalize.js
    // bilan AYNAN bir xil (oldin real ishlatilgan): PUT supplier_order_status/{id} (tanasiz,
    // New -> Open), keyin ta'minotchi kartochkasidagi phone_number bo'yicha
    // POST supplier_order_sms/{id} {phone_number:[...]}. SMS FAQAT buyurtma haqiqatan Open
    // bo'lgandan keyin ketadi. Xato bo'lsa buyurtma o'zi yaratilgan qoladi (New) va sabab
    // `finalize`da qaytadi - yuborishni bekor qilmaydi.
    let finalize = null;
    if (body.finalize && env === "prod") {
      finalize = { opened: false, sms_sent: false, phones: [], note: "" };
      try {
        await invan(cfg.base, `/api/v1/supplier_order_status/${orderId}`, { method: "PUT" }, token);
        finalize.opened = true;
      } catch (e) {
        finalize.note = "Open'ga o'tkazib bo'lmadi: " + String(e && e.message || e);
      }
      if (finalize.opened) {
        try {
          const sup = await invan(cfg.base, `/api/v1/supplier/${supplierId}`, { method: "GET" }, token);
          const d = (sup && sup.data) || sup || {};
          const phones = (Array.isArray(d.phone_number) ? d.phone_number : []).map(p => String(p || "").trim()).filter(Boolean);
          if (!phones.length) {
            finalize.note = "SMS yuborilmadi: ta'minotchi kartochkasida telefon raqami yo'q";
          } else {
            await invan(cfg.base, `/api/v1/supplier_order_sms/${orderId}`, {
              method: "POST", body: JSON.stringify({ phone_number: phones }),
            }, token);
            finalize.sms_sent = true;
            finalize.phones = phones;
          }
        } catch (e) {
          finalize.note = "SMS yuborilmadi: " + String(e && e.message || e);
        }
      }
    }

    // Telegram xabari: faqat buyurtma haqiqatan Open bo'lgan bo'lsa.
    let telegram = null;
    if (finalize && finalize.opened) telegram = await dispatchTelegramNotify(po, finalize);

    res.status(200).json({
      ok: true, env, po, order_id: orderId, finalize, telegram,
...
module.exports.dispatchTelegramNotify = dispatchTelegramNotify;   // sinash uchun
