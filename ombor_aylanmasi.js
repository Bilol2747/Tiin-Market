...
let OA_SNAP=null,OA_BY_SKU=null,OA_ROWS=[],OA_PAGE_ITEMS=[],OA_P8=null;
...
async function oaEnsureData(){
  ...
  // OA_P8 — TO'LIQ kirim tarixi (data_kirim.json, 2025-02-26 dan beri). P8 ni
  // to'g'ridan-to'g'ri ISHLATIB BO'LMAYDI: boshqa sahifa (Mahsulotlar/Zakas/Kirim)
  // undan oldin ochilgan bo'lsa, P8 `/api/v1/kirimdata`dan kelgan bo'lishi mumkin
  // va u ATAYLAB faqat so'nggi 120 kunni qamraydi. O'sha holatda p9/p10 tarixiy
  // tannarxi 120 kundan eski tovarlarni "kirim narxi yo'q" deb ko'rsatardi
  // (2026-08-18 topildi: sku 14885 "Diplomat Trusik" — kirimi 2025-06 da bor,
  // lekin jadvalda tannarx "—" edi; natija sahifalarni ochish TARTIBIGA bog'liq edi).
  if(!OA_P8){
    if(P8&&!window._p8FromApi){OA_P8=P8;}
    else{
      try{const r=await fetch("data_kirim.json",{cache:"no-store"});OA_P8=await r.json();}catch(e){OA_P8=P8||{skus:{}};}
      if(!P8){P8=OA_P8;window._p8FromApi=false;}
    }
  }
...
//
// `qty>0` TALABI (2026-08-18 qo'shildi): "Open"/"New"/"Partially received"
// holatidagi hali TO'LIQ yetib kelmagan buyurtma qatorlari `qty:0` bilan
// keladi (10,000+ shunday yozuv butun bazada) - ularning narxi ko'pincha
// TAXMINIY/kelishilmagan bo'lib, haqiqiy qabul narxidan farq qiladi. Bunday
// qatorni "haqiqiy kirim" deb hisoblasa, tannarx tarixi noto'g'ri sana
// nuqtasidan sun'iy sakrab ketadi (isbot: SKU 50211 "Roshen Шоколапки" -
// 2026-06-23'dagi Open, qty:0, narx 48,500 - qo'shni Received narx 24,250
// bilan bir kunda - 2026-08-05'gacha BUTUN tannarxni ~2x oshirib yuborgan,
// haqiqiy sotuv foydasi manfiy ko'rinishiga sabab bo'lgan).
function _oaIsRealKirim(a){
  const st=a.status;
  return st!=="Returned"&&st!=="Custom Return"&&(a.qty||0)>0;
}
...
function _oaSkuMovement(sku,fromIso,toIso){
  ...
  let inQ=0;
  const kirimEntry=OA_P8&&OA_P8.skus?OA_P8.skus[sku]:null;
