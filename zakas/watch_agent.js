...
async function runCron() {
  ...
  for (const chat of chats) {
    ...
    // Telegram sendMessage 4096 harfdan uzun matnni RAD ETADI ("text is too
    // long") - keng sozlama (masalan "5 kun, hammasi kategoriya") bilan bir
    // safar 200+ ta'minotchi chiqishi mumkin (2026-09-08 aniqlandi, real
    // xato). Shu sabab bitta xabarda ko'pi bilan MAX_PER_ALERT ta - eng
    // shoshilinchi (kam kun qolgani) birinchi ko'rsatiladi, qolganlari
    // KEYINGI safar (hali "alert_sent" deb belgilanmagani uchun) chiqadi.
    const MAX_PER_ALERT = 25;
    const sorted = toAlert.slice().sort((a, b) => a.items[0].daysLeft - b.items[0].daysLeft);
    const shown = sorted.slice(0, MAX_PER_ALERT);
    ...
    if (restCount > 0) {
      lines.push('', `... va yana ${restCount} ta ta'minotchi (keyingi tekshiruvda ko'rsatiladi - chegara/kategoriya sozlamasini toraytirsangiz hammasi bir safar ko'rinadi: /chegara, /kategoriya).`);
    }
    ...
    console.log(`  chat ${chatId}: ${shown.length} ta ta'minotchi uchun xabar yuborildi${restCount ? ` (yana ${restCount} tasi keyingi safarga qoldi)` : ''}.`);
  }
}
