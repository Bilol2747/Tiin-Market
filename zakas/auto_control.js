...
//   belgisiz     - auto-data branch'idagi zakas_auto_exclude.json (2026-09-25, 2-variant): saytda firma sahifasida
//                  belgisi olib tashlangan tovar O'ZI SABABLI buyurtma chiqarmaydi (firmani "zakasga tushdi" qilmaydi),
//                  lekin firmaga boshqa tovar sababli buyurtma ketsa - u bilan BIRGA ketadi. Firmaga buyurtma
//                  berilgach (ledger / sayt katakchasi `at` / Invan real vaqt) belgi ishlatilgan bo'ladi va tovar oddiy
//                  tartibga qaytadi (fayldan tozalanadi); 14 kundan eskisi ham tugaydi. O'qib bo'lmasa - yuborilmaydi.
//
// Ishlatilishi:
...
function evaluateFirm(sup, allRows) {
  ...
  return {
    ...
    stockOkCount: allRows.filter(r => r.zkaStockOk).length,
    // TESKARISI (faqat ogohlantirish, zakas BERILMAYDI): kirim modeli "yetarli" (CAN chegarasidan ko'p) desa-yu, Invan VA hisoblangan qoldiq ikkalasi 3 kundan kam.
    lowStock: allRows.filter(r => r.zkaDays != null && r.zkaDays >= RT.ZKA_CAN_ORDER_DAYS && r.zkaSpeed > 0 && !(r.pendingQty > 0) && r.calcStock != null
      && Number.isFinite(Number(r.calcStock)) && Math.min(Number(r.invanStock || 0), Number(r.calcStock)) >= 0 && Math.min(Number(r.invanStock || 0), Number(r.calcStock)) < 3 * r.zkaSpeed)
      .sort((a, b) => b.zkaSpeed - a.zkaSpeed).slice(0, 5).map(r => ({ sku: String(r.sku), name: r.name, days: Math.round(r.zkaDays * 10) / 10 })),
  };
}
...
function buildReport(res, warnings, when, sendInfo, unresolved) {
  ...
  if (exF.length) {
    L.push('## 🚫 Belgisi olib tashlangan tovarlar (o\'zi sababli buyurtma chiqmaydi; firmaga boshqa tovar sababli buyurtma ketsa, u bilan birga ketadi)');
    exF.forEach(r => {
      L.push(`- ${r.sup}: ${r.excludedCount} ta tovar${r.excludedMust.length ? ` (${r.excludedMust.length} tasi MUST — 7 kundan kam qolgan!)` : ''}${r.allExcluded ? ' — zakasdagi HAMMA tovar belgisiz, buyurtma chiqmaydi' : ''}`);
  ...
  const lowF = res.filter(r => r.lowStock && r.lowStock.length > 0);
  if (lowF.length) {
    L.push('## ⚠️ Kirim bo\'yicha yetarli ko\'rinadi, lekin Invan VA hisob qoldig\'i 3 kundan kam (zakas berilmaydi - qo\'lda tekshiring)');
    lowF.forEach(r => L.push(`- ${r.sup}: ${r.lowStock.map(i => i.name).slice(0, 3).join('; ')}${r.lowStock.length > 3 ? '; …' : ''}`));
    L.push('');
  }
  const stockOkTotal = res.reduce((a, r) => a + (r.stockOkCount || 0), 0);
  if (stockOkTotal) {
    L.push(`## 📦 Stok yetarli deb zakas berilmagan tovarlar (Invan va hisob qoldig'i 30+ kunlik) — ${stockOkTotal} ta (${res.filter(r => r.stockOkCount > 0).length} firmada)`);
    L.push('');
  }
...
// Zakas ochilgach (Open) Telegram "Tiin Analitika" guruhiga sening nomingdan PDF + xabar: telegram_notify.yml (2026-09-25, Bilol).
// GITHUB_TOKEN bilan workflow_dispatch (zakas_auto.yml'da `actions: write` kerak). Xato buyurtmaga ta'sir qilmaydi. TG_NOTIFY=off - o'chirish.
async function dispatchTelegramNotify(po, sms) {
  if (process.env.TG_NOTIFY === 'off' || !process.env.GITHUB_TOKEN) return false;
  await ghApi(`/repos/${REPO}/actions/workflows/telegram_notify.yml/dispatches`, { method: 'POST', body: JSON.stringify({ ref: 'main', inputs: { order_no: String(po), sms } }) });
  return true;
}
...
async function sendPhase(res, mode, warnings, results, ctx) {
  ...
  for (let k = 0; k < targets.length; k++) {
    ...
    if (fz.opened) {
      try { await dispatchTelegramNotify(j.po, fz.sms_sent ? 'sent' : 'failed'); }
      catch (e) { warnings.push(`Telegram xabari ishga tushmadi (${f.sup}, buyurtma ${j.po}): ${e.message}`); }
    }
  }
  return results;
}

// Belgisi olib tashlangan tovarlar: {firma: Set(sku)} (faol) va ishlatilgan/eskirganlar ro'yxati (consumed).
// Belgi "ishlatilgan" - firmaga BELGIDAN KEYIN buyurtma berilgan bo'lsa: ledger (avtomat), firma yozuvidagi `at`
// (sayt katakchasi) yoki Invan real vaqt (`invanLatest`, qo'lda). 14 kundan eskisi ham tugaydi.
const EXCLUDE_TTL_MS = 14 * 86400000;
function activeExclusions(doc, ctx) {
  const active = {}, consumed = [];
  const firms = doc && doc.firms && typeof doc.firms === 'object' && !Array.isArray(doc.firms) && !doc.day ? doc.firms : {};
  for (const [sup, m] of Object.entries(firms)) {
    if (!m || typeof m !== 'object' || Array.isArray(m)) continue;
    const sid = supplierIdOf(sup);
    let lastOrder = 0;
    const fe = (ctx.firmsMap || {})[sup];
    if (fe && fe.at) lastOrder = Math.max(lastOrder, Date.parse(fe.at) || 0);
    for (const o of ctx.ledgerOrders || []) if (o && o.firm === sup && o.at) lastOrder = Math.max(lastOrder, Date.parse(o.at) || 0);
    if (ctx.invanLatest && sid && ctx.invanLatest.get(sid)) lastOrder = Math.max(lastOrder, ctx.invanLatest.get(sid));
    const set = new Set();
    for (const [sku, since] of Object.entries(m)) {
      const t = Date.parse(since);
      if (!(t > 0) || ctx.now - t >= EXCLUDE_TTL_MS || lastOrder > t) { consumed.push({ sup, sku, sinceMs: t }); continue; }
      set.add(String(sku));
    }
    if (set.size) active[sup] = set;
  }
  return { active, consumed };
}
// Ishlatilgan/eskirgan belgilarni fayldan olib tashlaydi (yangi qayta o'qib; foydalanuvchi shu orada QAYTA belgilagan bo'lsa tegmaydi).
async function pruneExclusions(consumed) {
  if (!consumed.length) return;
  const cur = await ghReadJson(EXCLUDE_FILE, () => ({ firms: {} }));
  const f = cur.data && cur.data.firms;
  if (!f || typeof f !== 'object' || cur.data.day) return;
  let changed = false;
  for (const c of consumed) {
    const m = f[c.sup];
    if (m && m[c.sku] !== undefined && (!(c.sinceMs > 0) || Date.parse(m[c.sku]) === c.sinceMs)) { delete m[c.sku]; changed = true; if (!Object.keys(m).length) delete f[c.sup]; }
  }
  if (changed) await ghWriteJson(EXCLUDE_FILE, cur.data, cur.sha, `Zakas nazorati: ishlatilgan/eskirgan belgilar tozalandi (${consumed.length} ta)`);
}
// Belgisi olingan tovar firmani "zakasga tushdi" qila OLMAYDI: holat (send/wait) faqat qolgan tovarlardan.
// Firmaga boshqa tovar sababli buyurtma ketsa, hamma zakas tovari (belgisizlar ham) birga ketadi.
// Belgisi olingan MUST tovarlar hisobotda alohida ko'rsatiladi ("tugab qolmasin" nazorati).
function evaluateWithExclude(sup, allRows, exSet) {
  const r = evaluateFirm(sup, allRows);
  r.excludedCount = 0; r.excludedMust = []; r.allExcluded = false;
  if (!exSet || !exSet.size) return r;
  const MUST = RT.ZKA_MUST_ORDER_DAYS;
  const isEx = x => x.sku != null && exSet.has(String(x.sku));
  const orderRows = allRows.filter(x => x.orderQty > 0 && x.sku);
  const exOrder = orderRows.filter(isEx), restOrder = orderRows.filter(x => !isEx(x));
  r.excludedCount = exOrder.length;
  r.excludedMust = exOrder.filter(x => x.zkaDays != null && x.zkaDays < MUST).sort((a, b) => a.zkaDays - b.zkaDays)
    .map(x => ({ sku: String(x.sku), name: x.name, qty: x.orderQty || 0, days: Math.round(x.zkaDays * 10) / 10 }));
  const restMust = restOrder.filter(x => x.zkaDays != null && x.zkaDays < MUST);
  r.status = restMust.length ? 'send' : (restOrder.length ? 'wait' : 'none');
  r.allExcluded = exOrder.length > 0 && restOrder.length === 0;
  return r;
}
...
async function main() {
  ...
  // Belgisi olingan tovarlar: o'qib bo'lmasa xato sifatida qaytadi (fail-closed: shu safar yuborilmaydi).
  ...
  // Invan'dagi eng yangi buyurtmalar (real vaqtda): belgilar "ishlatilgan"ligini bilish va yuborishda to'qnashuvdan saqlanish uchun.
  let invanRecent = null, invanErr = null;
  if (process.env.INVAN_API_TOKEN && !isAll) { try { invanRecent = await fetchRecentInvanOrders(process.env.INVAN_API_TOKEN); } catch (e) { invanErr = e; } }
  let ledgerOrders = [];
  if (haveGh) { try { ledgerOrders = (await readLedger()).data.orders; } catch (e) { warnings.push(`Ledger'ni o'qib bo'lmadi (belgilarning ishlatilganligi to'liq aniqlanmadi): ${e.message}`); } }
  const { active: exclude, consumed } = activeExclusions(excl.doc, { now: Date.now(), firmsMap, ledgerOrders, invanLatest: invanRecent && invanRecent.latestBySid });
  if (excl.err) warnings.push(`Belgisi olingan tovarlar ro'yxatini (zakas_auto_exclude.json) o'qib bo'lmadi: ${excl.err.message} — shu safar hech narsa yuborilmaydi.`);
  if (haveGh && !isAll && consumed.length) { try { await pruneExclusions(consumed); } catch (e) { warnings.push(`Ishlatilgan belgilarni tozalab bo'lmadi: ${e.message}`); } }
  const res = [];
  ...
  else if (excl.err) why = 'belgisi olingan tovarlar ro\'yxatini o\'qib bo\'lmadi';
  ...
  if (!why) {
    ...
    else if (invanErr) why = `Invan buyurtmalarini tekshirib bo'lmadi: ${invanErr.message}`;
    else if (!(invanRecent.oldest <= cutoffMs)) why = `Invan buyurtmalari (${invanRecent.count} ta) jonli ma'lumot vaqtini to'liq qamramadi`;
  }
...
module.exports = { evaluateFirm, evaluateWithExclude, activeExclusions, dispatchTelegramNotify, sendPhase, buildReport, productsNewestDay, main };
