...
function jwtPayload(t) {
  try {
    return JSON.parse(Buffer.from(String(t).split('.')[1]
      .replace(/-/g, '+').replace(/_/g, '/'), 'base64').toString('utf8'));
  } catch { return null; }
}

// Buyurtma yaratishdan OLDINGI qat'iy tekshiruv (2026-08-04, foydalanuvchi
// talabi: "agar umumiy hisobdan zakas ketadigan bo'lsa, zakas bermasin").
// Ikki xato holatni ushlaydi:
//   1. shaxsiy token umuman yo'q  -> avval umumiy hisobga tushib ketardi
//   2. shaxsiy token o'rniga UMUMIY token qo'yilgan (masalan GitHub sirига
//      noto'g'ri qiymat nusxalangan) - fayl bor, lekin hisob baribir umumiy.
//      Buni faqat foydalanuvchi identifikatorini solishtirib bilish mumkin.
function requirePersonalToken() {
  const my = myToken();
  if (!my) {
    throw new Error(
      'Shaxsiy Invan tokeni topilmadi (zakas/my_token.txt) — buyurtma YARATILMADI. '
      + 'Umumiy hisob nomidan zakas berish taqiqlangan. '
      + 'GitHub Actions\'da INVAN_MY_TOKEN sirini tekshiring; lokalda: '
      + 'node zakas/order_agent.js --login "<telefon>" "<parol>"');
  }
  const mp = jwtPayload(my);
  const sp = jwtPayload(token());
  if (mp && sp && mp.user_id && mp.user_id === sp.user_id) {
    throw new Error(
      'Shaxsiy token aslida UMUMIY hisobniki (foydalanuvchi identifikatori bir xil) — '
      + 'buyurtma YARATILMADI. INVAN_MY_TOKEN siriga o\'zingizning tokeningizni qo\'ying.');
  }
  return my;
}
...
async function createOrder(g, supplierId, opts = {}) {
  ...
  // QAT'IY QOIDA (2026-08-04, foydalanuvchi talabi): buyurtma FAQAT shaxsiy
  // hisobdan yaratiladi. Ilgari shaxsiy token topilmasa umumiy hisobga
  // (Murodjon Tursunov) "tushib" ketardi va buyurtma boshqa odam nomidan
  // yozilardi - aynan shu 2026-08-04 da sodir bo'ldi. Endi bunday holatda
  // HECH NARSA YARATILMAYDI, aniq xato qaytariladi.
  const writeTok = requirePersonalToken();
...
async function verifyOrder(orderId, sent) {
  ...
  return {
    ...
    // Invan'da buyurtma KIM nomidan yozilgani (2026-08-04). Shaxsiy token
    // (zakas/my_token.txt) topilmasa umumiy hisob ishlatiladi va buyurtma
    // boshqa odam nomidan chiqadi - buni har safar jurnaldan qidirmaslik
    // uchun natijaga qo'shamiz, bot shuni xabarida ko'rsatadi.
    createdBy: [got.user && got.user.first_name, got.user && got.user.last_name]
      .filter(Boolean).join(' ') || '',
  };
}
...
async function main() {
  ...
  else console.log(`\n  ⛔ Shaxsiy login qilinmagan — buyurtma YARATILMAYDI (umumiy hisobdan zakas taqiqlangan).`
    + `\n      Avval kiring: node zakas/order_agent.js --login "<telefon>" "<parol>"`);
