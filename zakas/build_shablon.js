...
// Soni ustunida birlik yozilgan bo'lishi mumkin ("10 шт", "4 kg", "4sht") —
// birlik olib tashlanib faqat raqam olinadi (2026-09-21, foydalanuvchi qoidasi:
// "4 kg bu 4, 4sht bu 4"; Надежда Мирзоян "Заказ тиин.xlsx" shunday keldi va
// parseNum("10 шт") NaN bo'lib butun fayl "soni yozilgan qator topilmadi" bergan edi).
// Faqat ro'yxatdagi birliklar: "2 коробки" kabi qadoq so'zlari ATAYLAB yo'q —
// quti soni dona emas, uni raqamga aylantirib yuborish jimgina xato buyurtma beradi.
const QTY_UNIT_RE = /^(\d[\d\s.,]*?)\s*(?:шт|штук|sht|pcs|dona|дона|кг|kg)\.?$/i;
function parseQty(v) {
  const s = String(v == null ? '' : v).replace(/ /g, ' ').trim();
  const m = s.match(QTY_UNIT_RE);
  return parseNum(m ? m[1] : s);
}
