...
| p7 | Buyurtma / Zakas | `build_prev_avg.py` — UCHTA mustaqil hisob: `pav` ("chuqur zakas", `.pav_cache.json`), **`avg30sa`** ("muntazam zakas" kunlik o'rtachasi, `_compute_avg30_stock_aware()` — stok-asosli, xom nisbat `tot/effective` + `AVG30_CAP_MULT` cheklov, 2026-07-22 pav-yumshatish/`K` olib tashlandi) va **`rcost`/`rcost_approx`** (Narx/Summa ustunlari uchun eng ishonchli tannarx, `recompute_current_cost()` — `backend_p6_suppliers.py`dagi `kirim_cost_breakpoints`/`cost_at` qayta ishlatadi, har build qayta hisoblanadi). Qo'shimcha: **`zabc`** — [backend_p_zakas_abc.py](backend_p_zakas_abc.py), butun do'kon bo'yicha global `abc`dan MUSTAQIL, har ICHKI kategoriya (`cat`) o'z ichida mustaqil ABC (2026-08-06, foydalanuvchi so'rovi — "qaysi firmaning tovari o'z turkumida yaxshiroq"). Xavfsizlik zaxirasi frontendda: `ZK_BUFFER` @ [sales_runtime.js:699](sales_runtime.js#L699) — endi `v.zabc` bo'yicha (`v.abc` emas), chunki `_winArr()` sana oralig'ida `v.abc`ni qayta yozib yuboradi. Asosiy ro'yxat p5 invdata'dan quriladi | `renderZakas()` @ [sales_runtime.js:1096](sales_runtime.js#L1096), `renderZakasPag()` @ :992. Boshqa funksiyalar `zk*`/`_zk*` prefiksi bilan (~636-1920 oralig'ida, p5/Stock bilan chatishgan) | `ZITEMS` (p5 invdata asosida) + `.pav_cache.json` + `data_kirim.json` (`krPendingQty()` orqali ochiq buyurtma tekshiruvi) |
...
| p11 | Firmalar — 2 tab: **Xaridorlar** (bizga qarzdor B2B mijozlar, qarz muddati bo'yicha) va **Ta'minotchilar** (2026-08-11 qo'shildi — biz qarzdor bo'lgan yetkazib beruvchilar) | **Xaridorlar:** [fetch_clients.py](fetch_clients.py) — Invan'ning **ICHKI** API'sидан (`https://api.7i.uz/api/v1`, `integration/v1` EMAS) `POST /clients` orqali `client_type=Business` mijozlar reyestri (328 firma: nom, STIR, telefon, balans) + `GET /client/{id}` shartnoma raqami/sanasi (**inkremental** — faqat yangi firmalar uchun) + Turso `orders`dan `DEBT` to'lovli cheklar. Turso `clients_business` jadvaliga **faqat o'zgargan qatorlar** yoziladi (`row_hash`). ⚠️ `POST /api/v1/client` (BIRLIKDA) yangi mijoz YARATADI — ro'yxat uchun faqat KO'PLIK `clients` ishlatiladi. **Ta'minotchilar:** [fetch_supplier_debt.py](fetch_supplier_debt.py) — bir xil ICHKI API oilasi, lekin `POST /api/v1/suppliers` (1600+ ta'minotchi: nom, STIR, telefon, shartnoma, `balance`). Belgi: `balance<0` = BIZ qarzdormiz (haqiqiy raqamlar bilan tasdiqlangan — `invan_tahlili.md`dagi qo'lda topilgan "16.2 mlrd" bilan mos). ⚠️ Bu endpoint pagination'da **takrorlangan `id`** qaytaradi (1643 qatordan ~1451 unikal) — skript `id` bo'yicha DEDUP qiladi, aks holda summa shishadi. Tranzaksiya darajasida (aging/"necha kunlik qarz") API topilmadi — faqat joriy balans bor. Turso `suppliers_debt` jadvaliga `clients_business` bilan bir xil `row_hash` mulohazasi bilan yoziladi | `fmSwitchTab()` — ikkala tab orasida almashtiradi. Xaridorlar: `fmInit()`/`fmRender()`/`fmExportXLSX()`. Ta'minotchilar: `fmsInit()`/`fmsRender()`/`fmsExportXLSX()` — barchasi [sales_runtime.js](sales_runtime.js) oxiri, prefiks `fm*`/`_fm*` (xaridor) va `fms*`/`_fms*` (ta'minotchi). Xaridorlar guruhlari buxgalteriyaning "Horeca tahlil" Excel formatiga mos: 15 kungacha / 30 kungacha / 30 kundan oshgan / 45 kundan oshgan / Jami / Puli bor (`balans>0`), qatorni bosganda qarz cheklari ochiladi. Ta'minotchilar tab'ida aging YO'Q (sabab yuqorida) — sodda jadval, qatorga bosish/overlay yo'q | `data_firmalar.json` (xaridorlar) + `data_ta_qarz.json` (ta'minotchilar) — ikkalasi ham lazy-fetch, tab birinchi ochilganda. Sana oralig'i (faqat xaridorlar tab'ida) — p10/p9 kabi alohida `<input type="date">` |
...
### p7 (Buyurtma/Zakas) — 2026-09-24 da qo'shilgan qismlar

| Nima | Qayerda |
|---|---|
| "Oxirgi kirim" formulasi | `sales_runtime.js`: `_zkaCompute()` (kirimdan beri sotuv tezligi `vCheck`, 3x chegara `ZKA_MAX_ORDER_MULT`, reset-maqsad `_zkaBuildSupplierResetDays()`), karobka yaxlitlash `_zkaBoxSize()`; lokal nusxa: `backend_p_zakas_auto.py` (repo'da YO'Q) |
| "Qabul boshlangan buyurtma" qoidasi (buyurtmadan kamida bitta tovar kelsa, kelmaganlari Open emas) | `krPendingQty()` + `_krOrderHasReceipt()` |
| C tovar filtri (kam sotiladigan C ga zakas yo'q) | Hisob: `build_prev_avg.py` `c_keep_decision()`/`recompute_c_keep()`/`build_c_keep()` (chegaralar `C_KEEP_*`), `build_all_from_api.py` [10/8b] qadami -> `data_inv_new.json` `ck`=1/0; jonli ma'lumotga o'tishi: `backend/app.py` `_HEAVY_FIELDS`, `backend/publish_metrics.py` `HEAVY_FIELDS`; sayt: `v.ck` (ZITEMS) -> `_zkBuildSuppliers()` qator hisobida `orderQty=0` |
| Avtomatik zakas paneli (katakcha = darhol Invan'ga) | `zkAutoToggleSupplier()`, `_zkAutoRowsFor()`, `_zkAutoReapplySent()` (vaqtinchalik "Open"), `zkAutoSelectAll()` (yubormaydi); belgilar `localStorage` `zk_auto_enabled` + serverdagi nazorat ro'yxati (pastdagi qator) |
| Firma telefon raqami (SMS ketadimi) | `sales_runtime.js`: `_zkPhoneBadgeHtml()`, `zkPhonesFor()`; server: `api/invan-order.js` `action:"phones"` |
| Invan buyurtma ko'prigi | `api/invan-order.js`: `action:"login"` (shaxsiy token), `"phones"`, buyurtma yaratish (+`finalize:true` -> Open + `supplier_order_sms`) |
| Avto-belgi va "zakasga muhtoj" | `_zkIsChecked()` (faqat Muntazam, har kuni yangidan), ro'yxat sarlavhasi `withNeed.filter(s=>s.needN>0)` |
| Jonli ma'lumot (15 daq) | `.github/workflows/live_data.yml` (`live-data-latest` TAG), ishga tushirish: cron-job.org "Jonli ma'lumot (15 daqiqa)" vazifasi |
| Doimiy zakas nazorati (Toshkent 08/11/14/17/20; sukut FAQAT hisobot, yuborish KALITI o'chiq) | Skript: `zakas/auto_control.js` (MUST/CAN qoidasi `evaluateFirm()`, yuborish `sendPhase()`, ledger); workflow: `.github/workflows/zakas_auto.yml`; `auto-data` branch fayllari: `zakas_auto_firms.json` (nazorat ro'yxati), `zakas_auto_mode.json` (kalit `send`, `max_sum`), `zakas_auto_ledger.json` (kuniga 1 buyurtma); API: `api/zakas-auto-firms.js` (`get/set/mode_get/mode_set`, mode_set faqat admin); frontend: `_zkAutoFirmsLoad()/_zkAutoFirmsSync()`, `_zkAutoModeLoad()/zkAutoModeToggle()` (panel), qatorga `zkaDays`, `_zkAutoAllRowsMap()` |
| Lokal zaxira tizimi (repo'da YO'Q) | `local_zakas_server.py` (8798-port), `zakas_auto_run.py`, `zakas_auto_enable.py`, `zakas_auto_send.js`, `local_zakas_inject.js`, `backend_p_zakas_auto.py` |
