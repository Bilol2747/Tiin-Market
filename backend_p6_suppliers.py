...
def kirim_cost_breakpoints(kirim_skus):
    ...
    [(sana, dona_narx), ...] ro'yxatini quradi - FAQAT haqiqatan qabul
    qilingan ("Received"/"Partially received") yozuvlardan.

    2026-08-19 TUZATISH (Bilol topilmasi, real misol: SKU 1428 "Saqich
    Meller Shokolad" - Invan'da 3 marta 5,835 so'mdan HAQIQATAN kelgan,
    lekin eng so'nggi yozuv 30 dona kutilayotgan "Open" (hali kelmagan)
    buyurtma bo'lib, unda 15,830 so'mlik TAXMINIY/kelishilayotgan narx
    yozilgan edi - sana bo'yicha eng so'nggi bo'lgani uchun shu noto'g'ri
    narx "joriy tannarx" sifatida ishlatilib kelingan). Ilgari faqat
    "Returned"/"Custom Return" chiqarib tashlanardi - "Open"/"New" (hali
    tasdiqlanmagan, tovar kelmagan) holatlar esa xato ravishda haqiqiy
    kirim sifatida hisoblanardi. Live tekshiruvda: 25,158 SKU'dan 1,450
    tasi (5.8%) aynan shu sababdan noto'g'ri tannarxga ega ekani
    aniqlandi. Bu funksiya `unit_cost` (sotuv tannarxi, build_all_from_api.py)
    va `rcost` (build_prev_avg.py) - ikkalasi uchun ham yagona manba,
    shuning uchun tuzatish butun tizim bo'ylab (Kategoriyalar/ABC/
    Ta'minotchilar marja hisoblari) to'g'irlanishini ta'minlaydi."""
    breakpoints = {}
    for sku, entry in (kirim_skus or {}).items():
        events = []
        for a in entry.get("arrivals", []) or []:
            if a.get("status") not in ("Received", "Partially received"):
                continue
...
def kirim_supplier_breakpoints(kirim_skus):
    ...
    2026-08-19: kirim_cost_breakpoints()dagi bilan bir xil tuzatish - faqat
    "Received"/"Partially received" hisobga olinadi, "Open"/"New" (hali
    tasdiqlanmagan) buyurtma ta'minotchisi noto'g'ri "joriy" deb
    qo'llanmasin."""
    breakpoints = {}
    for sku, entry in (kirim_skus or {}).items():
        events = []
