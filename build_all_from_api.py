...
# 2026-08-15: `turso_sync_supplier_orders` o'rniga `invan_orders` — kirim ham
# to'g'ridan-to'g'ri Invan'dan (sotuv bilan bir xil sabab; ikkalasi ham AYNAN
# bir xil xom Invan obyektlarini qaytaradi).
from invan_orders import fetch_supplier_orders, fetch_orders_by_ids
...
# 2026-09-22: avval bu 5 kunlik "cursor" oynasi edi (oxirgi _synced_until'dan
# orqaga) - Invan'dagi ikkita real muammoni ushlay olmasdi: (1) buyurtma
# yaratilib "Received"ga o'tishi 5 kundan ko'proq vaqt olsa (yoki sahifalashda
# bir marta o'tkazib yuborilsa), created_at cursor undan uzoqlashib ketgach
# o'sha buyurtma boshqa hech qachon qayta so'ralmasdi; (2) allaqachon
# "Received" bo'lgan buyurtmaning Invan tomonida keyinroq tuzatilgan
# miqdori/narxi umuman ushlanmasdi (PO#20465, SKU 1270: 1000x xato yozuv
# oylab tuzalmay qoldi). Endi cursor'ga bog'liq bo'lmagan, har build'da
# TO'LIQ qayta so'raladigan sirpanuvchi oyna - shu ikkalasini ham yopadi.
# 15 kun = buyurtma yaratilganidan keyingi 15 kun ichida bo'lgan har qanday
# o'zgarish (holat yoki miqdor/narx tuzatishi) keyingi build'da ushlanadi -
# amaliy kelishuv (arzon, ~550-600 buyurtma/build), 15 kundan keyingi juda
# kech tuzatishlarni ushlamaydi.
KIRIM_RECHECK_DAYS = 15
...
def build_history_incremental(orders, daily_full=None):
    ...
    # Mavjud faylni o'qi yoki yangi boshlang'ich yaratamiz. 2026-08-21: fayl
    # arxivlash tizimi (history_archive.py) tufayli `base` endi HAR DOIM
    # HISTORY_BASE_DATE emas - eski oylar arxivga chiqarilgani sayin oldinga
    # siljib boradi. Massiv indekslari shuning uchun FAYLNING O'ZIDA saqlangan
    # `base`ga nisbatan hisoblanadi.
    if hist_path.exists():
        try:
            hist = json.loads(hist_path.read_text(encoding="utf-8"))
            stored_base = date.fromisoformat(hist.get("base", HISTORY_BASE_DATE.isoformat()))
            maps = {f: hist.get(f, {}) for f in FIELDS}
            print(f"      Mavjud tarix o'qildi: {len(maps['d']):,} mahsulot, base={stored_base}")
        except Exception:
            maps = {f: {} for f in FIELDS}
            stored_base = HISTORY_BASE_DATE
    else:
        maps = {f: {} for f in FIELDS}
        stored_base = HISTORY_BASE_DATE
        print("      data_history.json yo'q — to'liq yaratilmoqda")

    total_days = (today - stored_base).days + 1

    # DIQQAT (2026-09-10): `daily_full`ning oxirgi kuni ba'zan `today`dan
    # KEYINGI sanaga chiqib ketishi mumkin (masalan Invan so'rovi UTC/Toshkent
    # farqi sabab bir kun oldinga cho'zilsa) - shunda pastdagi massiv
    # yozuvi (`maps[f][prod_key][day_idx] = ...`) `total_days`dan tashqariga
    # chiqib "IndexError: list assignment index out of range" berardi (2 kun
    # ketma-ket sync.yml shu sababdan qulagan). Massivni ATAYLAB shu ehtimoldagi
    # eng katta kunga yetadigan qilib oldindan kattalashtiramiz - ma'lumot
    # yo'qolmaydi, faqat yetarlicha joy ajratiladi.
    if daily_full:
        _meta = daily_full.get("__meta__", {})
        try:
            _dstart = date.fromisoformat(_meta.get("start", today.isoformat()))
            _ddays = int(_meta.get("days", 0) or 0)
            if _ddays > 0:
                _needed = (_dstart + timedelta(days=_ddays - 1) - stored_base).days + 1
                total_days = max(total_days, _needed)
        except Exception:
            pass

    # Barcha mavjud massivlarni total_days ga uzaytir, oxirgi N kunni 0 ga reset
    #
    # DIQQAT (2026-09-10, 2-chi tuzatish): ilgari faqat `maps["d"].keys()`
    # bo'yicha yurilardi - agar biror mahsulot biror SABABDAN "d"da yo'q,
    # lekin boshqa maydonda (masalan "rc") mavjud bo'lsa, o'sha maydon
    # HECH QACHON uzaytirilmasdi (eski, qisqa uzunligicha qolib ketardi) -
    # keyinroq pastdagi yozuv shu eski/qisqa massivga urinib IndexError
    # berardi. Endi BARCHA maydonlardagi kalitlar birlashtirilib, hammasi
    # bir xil uzunlikka keltiriladi.
    reset_from = max(0, total_days - HISTORY_UPDATE_DAYS)
    all_hist_keys = set()
    for f in FIELDS:
        all_hist_keys.update(maps[f].keys())
    ...
    if daily_full:
        ...
        for prod_key, item in daily_full.get("items", {}).items():
            ...
            for di in range(daily_days):
                day_idx = (daily_start + timedelta(days=di) - stored_base).days
                if not (reset_from <= day_idx < total_days):
                    continue
                for f, src in src_fields.items():
                    src_arr = item.get(src, [])
                    # Himoya (2026-09-10): yuqoridagi ikkita uzaytirish bosqichi
                    # nazariy jihatdan yetarli, lekin bu yerda YANA bir marta
                    # o'zini-o'zi tuzatadigan tekshiruv qo'shildi - qanday
                    # sabab bilan bo'lmasin, massiv `day_idx`dan qisqa bo'lib
                    # qolsa, IndexError bilan BUTUN pipeline qulashi o'rniga
                    # shu yerda kengaytirib, davom etadi.
                    dest = maps[f][prod_key]
                    if day_idx >= len(dest):
                        dest.extend([0] * (day_idx + 1 - len(dest)))
                    dest[day_idx] = src_arr[di] if di < len(src_arr) else 0
    else:
        # Eski usul: raw orderlardan faqat d va r (birinchi marta uchun fallback,
        # faqat fayl hali mavjud bo'lmagan holatda ishga tushadi - stored_base
        # shu payt doim HISTORY_BASE_DATE ga teng)
        for order in orders:
            sign = -1 if order.get("type") != "sale" else 1
            sale_date = parse_local_date(order.get("create_time"))
            if sale_date is None or sale_date < stored_base:
                continue
            day_idx = (sale_date - stored_base).days
    ...
    # 2026-08-21: GitHub'ning 100MB fayl chegarasi sabab (data_history.json
    # 100.88MB'da push butunlay rad etilgan, "pre-receive hook declined") -
    # endi HAR build'da issiq oynadan (HOT_WINDOW_DAYS) eski, TO'LIQ o'tgan
    # kalendar oylar avtomatik alohida arxiv fayllariga chiqariladi, faol
    # fayl doim kichik qoladi. Batafsil/tekshirilgan: history_archive.py.
    maps, hist_base, total_days, newly_archived, _ = history_archive.archive_old_months(
        maps, total_days, stored_base)
    if newly_archived:
        print(f"      Arxivga chiqarildi: {', '.join(newly_archived)}")

    hist = {"base": hist_base.isoformat(), "days": total_days}
    hist.update(maps)
    ...
    # ABC tahlili "Oylik hisobot" grafigi uchun - faol oynaning tushumi +
    # oldin arxivga chiqarilgan barcha oylarning muzlatilgan tushumi
    # birlashtiriladi (aks holda arxivlangan eski oylar grafikdan yo'qolib
    # qolardi).
    monthly_rev = history_archive.load_all_archived_monthly_rev() + \
        _compute_monthly_revenue_rollup(maps["r"], total_days, hist_base)
...
def build(orders, products_path, html_path=None, last_sale_60=None, monthly=None, month_keys=None, products=None):
    ...
    # Firmalar (p11) ro'yxatidagi RASMIY biznes-mijozlar (client_type=Business, STIR bilan) -
    # Kategoriyalar (p10) "faqat chakana" rejimi uchun, chekning client.id shu to'plamda
    # bo'lsa "firma xaridi" deb ajratiladi (build_sales_demand.build()ga uzatiladi).
    try:
        _firmalar_raw = json.loads((ROOT / "data_firmalar.json").read_text(encoding="utf-8"))
        business_client_ids = {f["id"] for f in _firmalar_raw.get("firmalar", []) if f.get("id")}
    except Exception:
        business_client_ids = set()
    dailydata = build_dailydata_improved(api_records(orders), business_client_ids=business_client_ids)
    print(f"      {len(dailydata['items']):,} mahsulot, {len(business_client_ids):,} firma ID bilan taqqoslandi")
    # 2026-08-18 (Bilol so'rovi): SKU Invan'da vaqti bilan qayta ishlatilishi
    # mumkinligi (eski tovar o'chirilib, o'sha SKU BOSHQA tovarga
    # biriktirilishi) topildi - shu sabab frontend'da SKU orqali topilmasa
    # NOM bo'yicha zaxira moslashtirish ikkita HAQIQIY boshqa tovarni
    # chalkashtirib qo'yishi mumkin edi (masalan eski "coca-cola" SKU'si
    # keyinchalik "pishloq"qa berilgan). build_dailydata_improved()ning
    # ICHKI hisob-kitobiga TEGILMAYDI - faqat NATIJASIGA joriy katalogdan
    # (yuqorida `products` allaqachon o'qilgan) shtrix-kod biriktiriladi.
    # Frontend endi SKU topilmasa NOMGA emas, avval SHTRIX-KODGA qaraydi -
    # bu ancha ishonchli (fizik/global identifikator, SKU kabi qayta
    # ishlatilmaydi).
    for _dd_item in dailydata["items"].values():
        _dd_prod = products.get(_dd_item.get("sku") or "")
        if _dd_prod and _dd_prod.get("bc"):
            _dd_item["bc"] = _dd_prod["bc"]
    ...
    # Cursor'ga (_synced_until) bog'liq bo'lmagan sirpanuvchi oyna - har build'da
    # so'nggi KIRIM_RECHECK_DAYS kunni Invan'dan TO'LIQ qayta so'raydi (izoh
    # yuqorida, konstantaning ta'rifida). existing_kirim umuman yo'q bo'lsagina
    # (birinchi marta) butun tarix bootstrap qilinadi.
    since = None
    if existing_kirim is not None:
        since = (datetime.utcnow() - timedelta(days=KIRIM_RECHECK_DAYS)).isoformat()
    new_supplier_orders = fetch_supplier_orders(since=since)
    # Qo'shimcha zaxira: KIRIM_RECHECK_DAYS oynasidan HAM eski, lekin hali
    # "Received"ga o'tmagan (yoki endigina o'tgan, hali grace davrida)
    # buyurtmalarni ID bo'yicha alohida qayta so'raymiz - juda sekin
    # yetkazib beriladigan buyurtmalar ham ushlanadi.
    ...
    max_created = max((o.get("created_at", "") for o in new_supplier_orders), default="")
    kirimdata["_synced_until"] = max_created or datetime.utcnow().isoformat() + "Z"
...
def main():
    ...
    # 2026-08-15: `turso_sync` o'rniga `invan_orders` — sotuvlar endi Invan
    # API'dan TO'G'RIDAN-TO'G'RI o'qiladi. Ikkalasi ham AYNAN bir xil xom
    # `order` obyektlarini qaytaradi (Turso faqat vositachi edi), shuning
    # uchun bu yerdan pastdagi hech qanday hisob o'zgarmaydi.
    # Sabab: Turso yozish kvotasi tugab, "Sotuvlarni Turso'ga sinxronlash"
    # qadami 90 daqiqa osilib qolar edi -> butun pipeline bekor bo'lib,
    # sayt 13-avgustda muzlab qolgan edi.
    from invan_orders import fetch_window, fetch_range
    ...
    print(f"[0/6] Invan API'dan so'nggi {SITE_WINDOW_DAYS} kunlik buyurtmalar o'qilmoqda...")
    orders = fetch_window(SITE_WINDOW_DAYS)
    print(f"      {len(orders):,} ta buyurtma olindi")

    # 2026-08-15: calcStock JONLI TUZATISH uchun bazaviy vaqt (Bilol so'rovi -
    # zakas real vaqtga yaqin ishlasin, lekin butun tarixni har 15 daqiqada
    # qayta yurish imkonsiz). Vercel backend shu vaqtdan buyon Invan'dan
    # sotilgan/kelgan miqdorni olib, calcStock'ga QO'SHIB-AYIRIB qo'yadi.
    #
    # DIQQAT (Bilol tuzatishi - aniqlik: bo'shliq ham, takror ham bo'lmasin):
    # ATAYLAB SOAT (datetime.now()) EMAS, ENG SO'NGGI BUYURTMANING O'ZI
    # (max create_time) ishlatiladi. Sabab: `fetch_window()` bir necha
    # daqiqa davom etadi (30 kunlik, ko'p sahifa) - agar bazaviy vaqt
    # so'rov BOSHLANISHIDAN OLDIN qayd etilsa, so'rov DAVOMIDA kelgan
    # buyurtmalar ham tarixiy hisobga kiradi, HAM jonli tuzatishda yana
    # bir marta hisoblanib, TAKROR bo'lardi. Eng so'nggi `orders`dagi
    # create_time esa aynan tarixiy hisobga KIRGAN so'nggi buyurtma - undan
    # KEYINGI har bir buyurtma jonli tuzatishda albatta bor (bo'shliq yo'q),
    # undan OLDINGI birortasi ikki marta hisoblanmaydi (takror yo'q).
    _cts = [o.get("create_time") for o in orders if o.get("create_time")]
    calc_baseline_at = max(_cts) if _cts else datetime.now(timezone.utc).isoformat(timespec="seconds").replace("+00:00", "Z")
    if not calc_baseline_at.endswith("Z") and "+" not in calc_baseline_at:
        calc_baseline_at += "Z"
    (ROOT / "data_calc_baseline.json").write_text(
        json.dumps({"at": calc_baseline_at}), encoding="utf-8")
    ...
    # 2026-09-26 (Bilol: "hisob ancha aniqroq"): avg30sa va ck ENDI calcStock'DAN KEYIN hisoblanadi - "javonda bor edimi"
    # tiklash Invan qoldig'i emas, hisoblangan qoldiqdan (`calcStock`) boshlanadi (build_prev_avg.recompute_avg30_from_history).
    # Ikkalasi ham data_inv_new.json'ni o'qib-yozadi va <script id="invdata"> embed'ini o'zi yangilaydi (calcStock maydoniga tegmaydi).
    # Muntazam zakas uchun stok-asosli avg30sa (60 kunlik oyna) - har build'da qayta hisoblanadi (arzon, keshlanmaydi).
    print(f"\n[10/10] Muntazam zakas stok-asosli 'avg30sa' (calcStock anchor) data_inv_new.json ga qo'llanmoqda...")
    try:
    ...
    # C tovarlar uchun zakasda qolish belgisi `ck` - zabc (yuqoridagi qadam) va avg30sa'ning 30 kunlik tafsilotlari asosida
    print(f"\n[10/11] Zakas: C tovarlar uchun 'ck' (zakasda qolish) belgisi...")
    try:
        from build_prev_avg import build_c_keep
        build_c_keep(ROOT)
    except Exception as e:
        print(f"      ! ck qo'shishda xato: {e}")
