...
def _walk_clamp_ext(k_by_day, sales_arr, start_idx, end_idx, clamp=False):
    ...
    for i in range(start_idx, end_idx):
        ...
        if bal < min_bal:
            min_bal = bal
        if clamp and bal < 0:
            bal = 0.0        # ISHONCHLI-yangi yo'l (pastdagi _ext_reliable_new izohiga qarang)
    sold_before = 0.0
    for i in range(0, min(start_idx, len(sales_arr))):
        sold_before += sales_arr[i] or 0
...
def _walk_with_pauses_ext(sales, k_by_day, first_k, natural_max, total_days,
                      is_food=True, track_gap_kirim=False, clamp_neg=False):
    ...
    while i < total_days:
        if sales[i]:
            bal += k_by_day.get(i, 0) - sales[i]
            if clamp_neg and bal < 0:
                bal = 0.0    # manfiyga tushmaydi (ishonchli-yangi yo'l)
            if k_by_day.get(i, 0):
                last_k_idx = i
                sold_since_k = 0.0
        ...
        if p_kun <= 0:
            ...
            p_rand = (1 - p_kun) ** gap
        ratio = gap / max(cover, 0.5) if rate > 0 else 0.0
        # !!! "gap > natural_max" sharti ATAYLAB OLIB TASHLANGAN (2026-08-04) !!!
        # U AYLANMA (o'zini o'zi tasdiqlaydigan) xato bo'lib chiqdi: natural_max
        # tovarning O'Z tarixidagi eng uzun kirimsiz tanaffusi edi, LEKIN o'sha
        # tanaffusning o'zi ham tekshirilmagan - u ham aslida tugash bo'lishi
        # mumkin. Shu holda eski (aniqlanmagan) tugash kelajakdagi HAQIQIY
        # tugashni DOIMIY bloklab qo'yardi.
        #
        # Isbot (SKU 2876919, foydalanuvchi jismonan ko'rsatgan): 2025-08-27..
        # 10-01 orasidagi 35 kunlik tanaffus "tabiiy" deb qabul qilingan. Keyin
        # 2026-06-18..07-16 orasidagi 29 kunlik HAQIQIY tugash (Invan mustaqil
        # ravishda 0/3 ko'rsatgan) 35 dan qisqa bo'lgani uchun o'tkazib
        # yuborilgan va hisob 27 da qotib qolgan.
        # Qolgan uchta shart (ratio>=2, p_rand<0.25, rate>0) himoya uchun
        ...
        if fire:
            ...
            bal = 0.0
        # PAUZA ICHIDAGI ESKI KIRIM (2026-08-04, foydalanuvchi topilmasi).
        # Pauza tugash deb tan olingan bo'lsa (fire), o'sha o'lik davr ICHIDA
        # kelgan, LEKIN pauza oxirigacha yana KIRIM_PAUZA_ESKI_MAX kundan ortiq
        # sotilmay turgan kirim ham ARVOH: agar tovar javonda bo'lganda edi,
        # shuncha oy ichida kamida bir marta sotilardi.
        #
        # Isbot (SKU 2881511 "kusachka komplekt"): 2025-10-14 da 24 dona kelgan,
        # 9 OY davomida bitta ham sotilmagan; 2026-07-04 da 9 dona kelib, sotuv
        # boshlangan. Eski 24 ham sanalgani uchun hisob 29 chiqardi - Invan esa
        # mustaqil ravishda 5 ko'rsatgan (=9-4, ya'ni faqat yangi kirim).
        # SKU 2875064 (monster energy) ham xuddi shunday: 2025-12-29 dagi 19
        # dona 199 kun sotilmay turgan.
        #
        # VAQT-MUVOFIQLIGI: chegara pauza OXIRIGA nisbatan o'lchanadi, shuning
        # uchun yangi kelgan kirim hech qachon tashlanmaydi (u pauza oxiriga
        # yaqin bo'ladi) - sinovda tasdiqlangan.
        for d in range(a, min(j, total_days)):
            q = k_by_day.get(d, 0)
            if q and fire and (j - d) > KIRIM_PAUZA_ESKI_MAX:
                continue          # o'lik davrda kelib, o'zi ham sotilmay qolgan
            if q and track_gap_kirim:
                # SOTUVSIZ kunda kelgan kirim ham "oxirgi partiya" (SKU 48868):
                # avval u e'tiborsiz qolib, eski partiya bilan solishtirilardi va
                # yangi kelgan kirim noto'g'ri 0'ga tushirilardi.
                last_k_idx = d
                sold_since_k = 0.0
            bal += q
        i = j
...
def recompute_calc_stock_from_history(root=ROOT, verbose=True):
    ...
    # ─── OXIRGI KIRIMDAN QOLGAN "6/45" (2026-08-04, foydalanuvchi so'rovi) ──
    # Zakas berishda eng foydali oddiy ko'rsatkich: ENG OXIRGI kirimda necha
    # dona kelgan va o'shandan buyon nechtasi sotilgan.
    #   lkQty  - eng oxirgi Received kirimdagi dona
    #   lkSold - o'sha kirim KUNIDAN boshlab (shu kun ham kiradi) sotilgan dona
    #   lkDate - o'sha kirim sanasi
    # Frontend "qolgan/kelgan" ko'rinishida ko'rsatadi: 45 kelib 39 sotilsa 6/45.
    #
    # ANIQLIK SHARTLARI (foydalanuvchi alohida ta'kidladi):
    #  - FAQAT eng oxirgi kirim sanasi olinadi; o'sha kunda bir nechta hujjat
    #    bo'lsa, ular QO'SHILADI (bitta yetkazib berish, bir necha qator).
    #  - Faqat status="Received" (kutilayotgan buyurtma kirim emas).
    #  - Sotuv ikkala manbadan (2025h2 statik + 2026 jonli) yig'iladi, ustma-ust
    #    tushmasligi uchun tarixiy manba QAT'IY offset'gacha - asosiy hisobdagi
    #    bilan bir xil chegara.
    #  - Kirim oynadan oldin (2025-07-01 dan avval) bo'lsa, sotuvni to'liq
    #    sanab bo'lmaydi -> ko'rsatkich BERILMAYDI (noto'g'ri son ko'rsatgandan
    #    ko'ra ko'rsatmagan yaxshi).
    lk = {}
    for sku in active_skus:
        k_full = _kirim_by_day(kirim_skus, sku, new_base, total_days)
        if not k_full:
        ...
        for ds, q in (old_daily.get(sku) or {}).items():
            di = (date.fromisoformat(ds) - new_base).days
            if last_kd <= di < offset:
                sold += q
        hist_arr = d_map.get("sku:" + sku) or []
        start_i = max(0, last_kd - offset)
        for i in range(start_i, min(len(hist_arr), hist_days)):
            sold += hist_arr[i] or 0
        lk[sku] = {
            "qty": round(qty, 2),
            "sold": round(sold, 2),
            "date": (new_base + timedelta(days=last_kd)).isoformat(),
        }
    # MUHIM (2026-08-04, foydalanuvchi topilmasi): "oxirgi kirimdan qolgan"
    # calcStock'dan MUSTAQIL ko'rsatkich - u faqat kirim+sotuv yozuvidan
    # hisoblanadi va hech qanday model/qoidaga bog'liq emas. Shuning uchun
    # calcStock chiqmagan tovarda ham ko'rsatiladi (masalan SKU 7758: 2025-07-30
    # da 40 dona kelgan, 2026 da kirim yo'q - asosiy model uni ko'rmaydi, lekin
    # "oxirgi kirimdan nechta qolgani" baribir ma'lum va zakas uchun foydali).
    # Bunday yozuvda stock=None - _apply_calc_stock() calcStock'ni YOZMAYDI,
    # faqat lk maydonlarini yozadi (saytda Hisob "—" bo'lib qolaveradi).
    n_lk_only = 0
    for sku, v in lk.items():
        if sku in result:
            result[sku]["lk"] = v
        else:
            result[sku] = {"stock": None, "conf": None, "evidence": None,
                           "anchor": None, "rule": None, "lk": v}
            n_lk_only += 1
    ...
    if verbose:
        ...
        print(f"  {n_calc:,} mahsulotga calcStock hisoblandi (yuqori={n_hi:,}, o'rta={n_mid:,}, eskirgan={n_stale:,})")
    ...
    if TARGETED_EXTENDED_SKUS:
        ...
        for sku in TARGETED_EXTENDED_SKUS:
            ...
            if (result.get(sku) or {}).get("rule") == "qolda-tuzatish":
                continue    # menejer sanagan qo'lda tuzatish (yuqorida qo'llangan) HAR DOIM ustun - bu blok uni bosib ketmasin
            k = {}
            ...
            if len(sold_idx) >= 2:
                ...
                cands = [_walk_with_pauses_ext(fs, k, fk, nat, ext_total, is_food=is_food,
                                               track_gap_kirim=True, clamp_neg=cl)
                         for cl in (False, True)]
            else:
                final, resets, _ = _walk_clamp_ext(k, fs, fk, ext_total)
                last = None
                _c = _walk_clamp_ext(k, fs, fk, ext_total, clamp=True)
                cands = [(_c[0], _c[1], None)]
            # ISHONCHLI-YANGI (2026-09-19, Bilol: "manfiyga tushmasin, yaqinda kelgan
            # kirim yo'qolmasin - lekin faqat ishonchlilarini o'zgartir"): yangi hisob
            # (kunlik clamp + sotuvsiz kundagi kirimni ham "oxirgi partiya" deb
            # hisoblash) FAQAT shunda qabul qilinadi, agar (1) eskisidan KO'P bo'lsa
            # (ya'ni yaqinda kelgan kirim noto'g'ri yo'qolgan) VA (2) butunlay OXIRGI
            # KIRIMDAN QOLGAN miqdor bilan tushuntirilsa (kelgan - shundan beri
            # sotilgan). Aks holda eski hisob saqlanadi - shu tufayli katta hujjatsiz
            # sotuvlar (2819 shokolad: Invan 573, to'liq clamp 5,576) yoki eski
            # arvoh partiyalar (sosiskalar 4 -> 400) stokni sun'iy shishirib yubormaydi.
            # Nomzodlar: (a) faqat xato tuzatish, (b) xato + clamp. Qoidaga mos
            # keladiganlardan ENG KATTASI olinadi.
            _lk = lk.get(sku)
            if _lk:
                _rem = max(0.0, _lk["qty"] - _lk["sold"])
                _ok = [c for c in cands if final < c[0] <= _rem + 0.5]
                if _ok:
                    final, resets, last = max(_ok, key=lambda c: c[0])
                    n_reliable += 1
            ...
            n_ext += 1
    if verbose and n_reliable:
        print(f"  {n_reliable:,} mahsulotda yaqinda kelgan kirim tiklandi (ishonchli-yangi hisob, oxirgi kirimdan qolgan bilan tushuntirildi)")
...
def _apply_calc_stock(calc_by_sku, root=ROOT, verbose=True):
    ...
    for iv in inv.values():
        ...
        if res:
            # stock=None - "faqat oxirgi-kirim ko'rsatkichi bor" yozuvi:
            # calcStock maydonlari YOZILMAYDI (saytda Hisob "—" bo'lib qoladi),
            # lekin lk maydonlari yoziladi. Ular mustaqil hisoblanadi.
            if res["stock"] is None:
                for k in ("calcStock", "calcConf", "calcEvidence", "calcAnchor", "calcRule"):
                    iv.pop(k, None)
            else:
